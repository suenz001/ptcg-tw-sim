...
<details open>
        <summary><span class="ver-badge">v5.542</span> 對戰修正：「多龍奇｜偵查指令」「探險家的嚮導」這類『查看牌庫上方數張卡、強制選擇指定張數加入手牌』的效果，先前錯誤地多顯示了【不選】按鈕。已修正——卡面強制選擇的牌庫 picker 不再出現【不選】，必須選滿規定張數（同類「搜尋牌庫可能找不到／最多 N 張／任意數量」的效果仍可不選）。同類強制選的牌庫效果（辛俐、暗碼迷的解讀、賽吉 等）一併修正。</summary>
      </details>

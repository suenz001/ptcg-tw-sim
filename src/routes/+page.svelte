...
<details open>
      ...
      </details> 對戰修正：「多龍奇｜偵查指令」「探險家的嚮導」「辛俐」這類『查看牌庫上方數張卡、強制選擇指定張數加入手牌』的效果，先前錯誤地多顯示了【不選】按鈕。已修正——這類『看到牌之後必須選擇』的牌庫效果不再出現【不選】，必須選滿規定張數。（注意：「搜尋牌庫找出特定卡」的效果，依官方規則即使牌庫中有也可選擇「找不到」，因此仍保留【不選】。）</summary>
      </details>

...
<details open>
        <summary><span class="ver-badge">v5.696</span> 一般對戰：對手閒置超過設定時間改為自動判定獲勝</summary>
        <ul>
          <li>一般線上對戰中，當對手閒置超過房主設定的「對手閒置判定獲勝時間」後，系統會自動判定你獲勝（先前需手動點按鈕）。判定仍以伺服器最新盤面再驗證，若對手其實已行動則改為重新同步、不會誤判。仍保留手動「立即宣告對手棄權獲勝」按鈕當後備。</li>
        </ul>
      </details>

...
<details open>
        <summary><span class="ver-badge">v5.963</span> 修正一批「搜尋牌庫」型卡在看完牌庫後選擇不拿（跳過）時忘了重洗牌庫的漏洞——這會讓玩家看過整副牌庫排列卻沒洗牌、之後可利用已知順序。已修正:賽吉、赤松、大吾的巨金怪ex「X啟動」、克雷色利亞「充溢之光」、穿著熊「力量充能」、謝米「親送花朵」、信使鳥「急速之禮」——這些在跳過／找不到符合卡時現在都會正確重洗牌庫（其中賽吉、赤松之前訊息還誤寫「已重洗」但實際沒洗，一併修正）。另外:克雷色利亞「充溢之光」可選能量從『任何基本能量』收窄為卡面的『基本【超】能量』；信使鳥「急速之禮」的「任意選1張」改為必選1張（比照同類卡）。</summary>
      </details>
      <details>

...
<main>
  ...
  <section class="changelog-section">
    ...
    <div class="changelog-list">
      ...
      <details open>
        <summary><span class="ver-badge">v5.265</span> 重試徽章附加限制澄清 + 道具 audit</summary>
        <ul>
          <li>玩家反映：重試徽章只有【無】屬性寶可夢可附加。經 audit 確認，本站重試徽章在<b>附加端沒有限制</b>，可附在任何寶可夢身上（除非該寶可夢已附有道具）。</li>
          <li>真正限制是<b>效果端</b>：依卡面「附有這張卡的無屬性寶可夢可在自己的回合使用 1 次，重擲招式擲幣結果」，因此擲幣後的「保留/重擲」詢問視窗僅對【無】屬性寶可夢觸發。</li>
          <li>本次修補：非【無】寶可夢持有重試徽章且擲過幣時，戰鬥訊息會明確顯示「本次效果不觸發（卡面僅對【無】屬性寶可夢生效）」，避免玩家誤判為附加失敗。</li>
          <li>全道具 audit 確認：所有寶可夢道具的附加端皆無錯誤的屬性限制；「電氣球」「赫普的講究頭帶」「驅勁能量 古代」「竹蘭的力量負重」等 holder-specific 道具皆採「可附但對非條件 holder 不生效」設計，符合 PTCG 規則。</li>
        </ul>
      </details>
      ...
      <details>
        ...
        <ul>
          ...
          <li>本次修補：詢問視窗的標題與說明改成讀取本次招式名稱動態顯示；「（停止）」標籤只對「擲幣直到反面為止」類招式（機關槍合擊、滾球、連續舞步、奔進、螺旋衝刺、連續火焰）才顯示，其他招式（飛翔等只擲 1 次的）不會誤標。</li>
        </ul>
      </details>

      <details>
        <summary><span class="ver-badge">v5.262</span> 重試徽章「保留剛才結果」實際還是重新隨機修正</summary>
        <ul>
          <li>玩家反映：擲反面後選「不重擲」，但訊息卻顯示又擲了一次變正面，等於「保留」不生效。</li>
          <li>根因：v5.165 重試徽章機制用「撤回狀態 + 重跑攻擊 + 從動作參數注入結果」，但用共用工具函式擲幣的招式（飛翔、咬碎、破壞之頭錘等 30+ 張）沒辦法接到注入的結果。</li>
          <li>本次修補：注入機制改放在遊戲狀態裡的 queue，由擲幣工具自己每次取一個用，所有招式（含飛翔等用共用工具的）都會正確使用剛才的擲幣結果。</li>
        </ul>
      </details>
      ...
      <details>
        ...
        <ul>
          ...
          <li>本次先加上診斷訊息：使用輪番狂攻時會在戰鬥訊息列出「場上其他古代寶可夢及其上回合使用的招式」，方便玩家／開發者直接從訊息看出未觸發的原因（例如猛雷鼓ex 被 KO、被替換離場、或未在場上等）。</li>
        </ul>
      </details>
      ...
      <details>
        ...
        <ul>
          <li>玩家反映：探險家的嚮導（查看牌庫頂 6 張，選 2 張加入手牌）可以選 0 張略過，違反卡面「選擇 2 張」。本次修補：強制選 2 張，僅在牌庫剩 1 張時才允許少選。</li>
          <li>玩家反映：重試徽章詢問視窗的文字「前次擲幣結果」會被誤解為「上一回合的擲幣」。實際指的是本回合招式剛擲完的那一次。本次修補：四處文字統一改為「剛才擲幣結果」與「本次擲幣結果」，避免誤解。</li>
        </ul>
      </details>
      ...
      <details>
        ...
        <ul>
          <li>玩家反映：N的索羅亞克ex 用「暗黑底牌」借「亂暴閃電」攻擊後，撤退再上場仍然不能攻擊。</li>
          <li>根因：撤退時雖然有清狀態，但若某些換位路徑漏走「清狀態」流程，備戰寶可夢殘留的「下回合無法攻擊」標記在新上場時仍會生效。</li>
          <li>本次修補：補場（送出新戰鬥寶可夢）時統一清一次「無法攻擊／傷害加成／鎖招」等戰鬥位專屬標記。同類撤退-再上場場景全部會自然修復。</li>
        </ul>
      </details>
      ...
      <details>
        ...
        <ul>
          <li>v5.165 重試徽章原本只對「機關槍合擊」這類動態次數擲幣的招式完整支援。其他用 helper 計算的固定次數擲幣招式（連續攻擊、雙重戲法、亂抓、舔舔颶風、二連擊…共 60+ 張）雖然能觸發詢問視窗，但「保留前次結果」會重新隨機，使用者體驗不一致。</li>
          <li>本次修補：把擲幣結果記錄與「保留前次結果」回放機制下放到擲幣共用工具，所有用此工具的招式自動受惠，現在每張固定次數擲幣招式都能正確支援重試徽章的「保留/重擲」二選一。</li>
        </ul>
      </details>
      ...
      <details>
        ...
        <ul>
          ...
          <li>本次修補：巨型花束計算自身【草】能量時加上繁茂判斷，自方場上有大竺葵繁茂時，基本【草】能量改算 2 個，傷害公式回到 70+50×6=370。</li>
        </ul>
      </details>
      ...
      <details>
        ...
        <ul>
          ...
          <li>本次修補：擲 3 幣後，先彈出牌庫挑選介面讓玩家自己挑能量（任何屬性、最多 正面數×2 張），再彈出分配介面讓玩家把這些能量自由分配給自己場上任何一隻寶可夢（含戰鬥場與備戰）。仿超級花葉蒂ex「永生綻放」既有兩階段介面，但範圍擴大到含戰鬥場。最後依卡面重洗牌庫。</li>
        </ul>
      </details>
      ...
      <details>
        ...
        <ul>
          ...
          <li>本次修補：戰鬥寶可夢滿血時超大冰淇淋按鈕變灰不可選；同類治療道具（傷藥、好傷藥、密阿雷格雷派餅、派帕的三明治、龍之秘藥、寶可生機劑A、西餐廚師、真菰、由紫、希特隆的機智、寶可夢中心的姐姐 等）原本就已有滿血禁用判定，本次只補上超大冰淇淋這張漏網之魚。</li>
        </ul>
      </details>
      ...
      <details>
        ...
        <ul>
          ...
          <li><b>大劍鬼「激流旋渦」</b>：自方戰鬥↔備戰互換完成、對手戰鬥↔備戰互換也跑完之後，跳出大劍鬼方新戰鬥位的上場特性詢問。</li>
          <li><b>直衝熊「激動衝刺」</b>：自方場上有超級進化ex時，把直衝熊與戰鬥位互換後，跳出新戰鬥位的上場特性詢問。</li>
          <li><b>魔幻假面喵「表演時間」</b>：把魔幻假面喵與戰鬥位互換後，跳出新戰鬥位的上場特性詢問。</li>
          <li>3 張卡之前都漏設「本回合換上來」標記，疾風直撞類條件招式對它們也失效，本次一併修好。</li>
        </ul>
      </details>
      ...
      <details>
        ...
        <ul>
          ...
          <li>根因：我之前的 audit 漏看了「烏栗」「衝浪手」這兩張支援者卡（它們的實作位置在跟其他卡不同的檔案內），所以換位後既沒設「本回合換上來」的內部標記，也沒跳出詢問視窗。</li>
          <li>本次修補：烏栗的兩種換位路徑（備戰只有 1 隻直接換 / 多隻時挑一隻換）跟衝浪手的換位都補上機制，現在這 3 條路徑跟其他換位卡一樣會自動詢問。</li>
          <li>順便修了一個衍生 bug：烏栗 / 衝浪手換位之前都漏設「本回合換上來」標記，導致疾風直撞類「本回合從備戰區放置於戰鬥場 +N 傷害」的招式對它們失效，現在一併修好。</li>
        </ul>
      </details>

      <details>
        <summary><span class="ver-badge">v5.245</span> 上場特性詢問補完 — 火箭隊的坂木（自換+迫換對手）</summary>
        <ul>
          <li>承 v5.244 補上火箭隊的坂木這張支援者卡：先把自方戰鬥場與備戰互換，再強迫對手戰鬥場與備戰互換。在這套流程跑完後，若自方新戰鬥位有「振翅高飛」等上場特性會自動詢問。</li>
          <li>機制設計上保證安全：「老大的指令」等只換對手的卡（自方戰鬥場沒換）<b>不會誤觸發詢問</b>，因為機制要求「自方戰鬥位這回合剛從備戰區上來」才會跳出。</li>
        </ul>
      </details>

      <details>
        <summary><span class="ver-badge">v5.244</span> 上場特性詢問補完 4 處（瞬間移動者 / 天空搬運 / 急進開關 / AZ的平和）</summary>
        <ul>
          <li>承 v5.243，本次再補上 4 個自方換位入口：凱西「瞬間移動者」、超級快龍ex「天空搬運」、急進開關（物品卡）、AZ的平和（支援者卡）。換位完成後若新戰鬥位有「振翅高飛」等上場特性會自動詢問。</li>
          <li>急進開關特別處理：因為換位後還要挑能量轉移，<b>能量挑完之後才會跳特性詢問</b>，避免兩個介面互衝。</li>
          <li>AZ的平和順便補上一個漏設的標記，之前疾風直撞之類「本回合從備戰區放置於戰鬥場 +N 傷害」的條件招式對它無效，現在也修好了。</li>
          <li>規則更嚴：詢問機制本身加上「<b>必須真的剛從備戰區換上來</b>」的判斷，避免任何邊角情況誤觸發。</li>
          <li>尚未涵蓋：火箭隊的坂木（自換+強迫對手換 兩階段，結構較複雜，留下一版處理）；直衝熊「激動衝刺」、魔幻假面喵「表演時間」、大劍鬼「激流旋渦」、桃歹郎ex「支配鎖鏈」、衝浪手、烏栗 等卡尚未實作底層效果，將來實裝後可即時納入詢問機制。</li>
        </ul>
      </details>

      <details>
        <summary><span class="ver-badge">v5.243</span> 上場特性詢問 — 自方換位路徑全面納入</summary>
        <ul>
          <li>承 v5.240 / v5.242，本次把「上場時自動詢問特性」機制擴大到所有自己回合內的自方換位路徑：</li>
          <li><b>已涵蓋（v5.243）：</b>撤退、撤退需挑能量丟棄的版本、寶可夢交替類道具卡、衝浪海灘場地卡。</li>
          <li><b>不涵蓋（依規則設計）：</b>戰鬥位被擊倒後選新戰鬥位（補位不算換位）、對手強制我方換場（卡面要求「在自己的回合」、跨回合不觸發）、我方招式強制對手換場（換場目標是對手，特性屬於對手）。</li>
          <li>其餘特定卡的自方換位招式（雀躍、瞬間移轉、瞬間移動者、天空搬運、急進開關等）會在後續版本陸續補上。實作上把共用 helper 集中放到底層檔（_shared），所有換位 caller 都能直接呼叫不會再有 v5.241 那種循環引用問題。</li>
        </ul>
      </details>
      ...
      <details>
        ...
        <ul>
          ...
          <li>1. <b>撤退時跳出能量選擇視窗的情況</b>：撤退費用涉及多種屬性能量，需要玩家手動挑哪幾張丟棄。挑完之後新戰鬥位上場時同樣會自動詢問是否使用特性。</li>
          <li>2. <b>對手用招式強制我方換場</b>（如風扇呼喚、玻璃喇叭等將寶可夢從備戰換到戰鬥場的招式）：被換上場的寶可夢同樣會被自動詢問。</li>
          <li>仍排除「<b>戰鬥位被擊倒後選新戰鬥位</b>」(因為這是被迫補位、不算換位行為)。</li>
          <li>其他「招式效果強制換場」「特性效果換場」「道具效果換場」等較少見的路徑會在後續版本逐步補上，在那之前玩家仍可手動點戰鬥位的特性按鈕使用。</li>
        </ul>
      </details>

      <details>
        <summary><span class="ver-badge">v5.240</span> 遠古巨蜓ex「振翅高飛」等「上場時」特性 — 撤退換場時自動詢問</summary>
        <ul>
          <li>玩家反映：遠古巨蜓ex 從備戰區撤退換到戰鬥場時，「振翅高飛」特性應該主動詢問玩家是否使用（從牌庫附最多 3 張基本【草】能量），就像殺手鐧捕捉、精神抽出那樣。</li>
          <li>本次新增「上場時自動詢問特性」機制 — 凡卡面寫「<b>從備戰區將這隻寶可夢放置於戰鬥場時，可使用 1 次</b>」的特性，撤退新戰鬥位上場時會自動跳出視窗詢問使用。第一波涵蓋：遠古巨蜓ex「振翅高飛」、勾帕路翁ex「金屬之路」、拉帝歐斯「潔淨支援」。鐵武者ex「超光速位元」、鐵毒蛾「熱流反應者」尚未實作，先納入詢問清單，後續實裝後即可生效。</li>
          <li>目前只在「玩家自願撤退」時自動詢問。其他換場路徑（KO 後選新戰鬥位、招式強制換場、玻璃喇叭、紅放大鏡風扇等）後續版本陸續補上。在這之前，玩家換場後仍可手動點擊戰鬥位的特性按鈕使用。</li>
        </ul>
      </details>

      <details>
        <summary><span class="ver-badge">v5.239</span> 預設牌組土龍弟弟改 I 標版本 + 章魚桶墨汁噴射尊重薄霧能量免疫 + 文字校正</summary>
        <ul>
          <li><b>預設牌組更新</b>：原本部分預設牌組裡的「土龍弟弟」用了 G 標的舊版本（SVM 095/175），現役賽制已退標。本次全部改成 I 標的 SV9 078/100 版本，玩家拿來開戰不會再被 G 標警告擋住。</li>
          <li><b>章魚桶「墨汁噴射」vs 薄霧能量</b>：「墨汁噴射」會讓被擊中的寶可夢下回合使用招式前要擲 2 次硬幣（出現反面則招式失敗）。但被擊中的寶可夢若附有「薄霧能量」（不受對手招式效果影響），規則上不該被套上這個擲幣干擾。本次修補後墨汁噴射、潑沙等同類招式遇上附薄霧能量（或抵抗之幕、球形盾牌、化隱、太晶、全能硬殼等招式效果免疫機制）的目標時，跳過設定擲幣干擾，符合卡面語意。</li>
          <li><b>文字校正</b>：前一版（v5.238）的更新說明跟對戰 log 把「阿塞蘿拉的惡作劇」誤寫成「阿賽蘿拉」，本次全部更正回正確卡名。</li>
        </ul>
      </details>

      <details>
        <summary><span class="ver-badge">v5.238</span> 完全免疫招式類也擋下放置傷害指示物的招式效果</summary>
        <ul>
          <li>玩家反映：喇叭啄鳥／咕咕鴿招式「飛翔」擲幣正面後，下回合受胡地「手之力量」（招式效果是放置傷害指示物）攻擊時，傷害指示物還是會被放上，沒符合卡面「不受招式的傷害和效果」。</li>
          <li>本次修補：依照卡面，凡卡面寫「不受招式的傷害<b>和效果</b>」的免疫效果（飛翔、要害斬、阿塞蘿拉的惡作劇 vs ex 招式），同時擋下對手招式的後置效果（放置傷害指示物、附加異常狀態、拔能量等），不再只擋傷害數值。</li>
          <li>不變的部分：卡面只寫「不受招式的<b>傷害</b>」的免疫（中立中心、精神防護、閃光屏障、塗層攻擊）依然只擋傷害，不擋招式效果，符合卡面語意。</li>
        </ul>
      </details>

      <details>
        <summary><span class="ver-badge">v5.237</span> 蟲甲聖「球形盾牌」對幻影奇襲類招式同樣有效</summary>
        <ul>
          <li>玩家反映：蟲甲聖在戰鬥位時，被「幻影奇襲」類同時擊倒戰鬥位＋對備戰放傷害指示物的招式攻擊後，因為蟲甲聖已昏厥退場，備戰寶可夢就失去「球形盾牌」的保護，仍會被放上指示物。</li>
          <li>本次修補：依照規則「招式效果同時結算」，攻擊宣告當下若有球形盾牌生效，即使蟲甲聖被同招式擊倒，備戰寶可夢仍受球形盾牌保護，不會被放上指示物或承受招式效果。修法跟先前火箭隊的急凍鳥「抵抗之幕」、謝米「花之帷幔」同步。</li>
        </ul>
      </details>

...
<details open>
        <summary><span class="ver-badge">v5.520</span> 對戰修正：「直接昏厥／招式效果擊倒」類招式（伊裴爾塔爾ex｜死亡靈魂、超級阿勃梭魯ex｜死亡終局、三重強念、擊斃）改用統一的效果昏厥機制處理。先前是用「把傷害灌到極大值」的方式強制昏厥，萬一昏厥被某些效果擋下，會殘留異常的傷害數值（可能造成「?／HP 0／0」之類的顯示異常或卡住）。現在統一改為標準的效果昏厥（傷害設為剛好等於最大HP、並正確尊重各種HP加成道具），行為一致且更穩定。</summary>
      </details>

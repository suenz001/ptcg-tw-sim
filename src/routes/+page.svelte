...
<details open>
        <summary><span class="ver-badge">v5.468</span> 兩項修正：（1）狐大盜「技能大盜」現在會跳出選單，讓你選擇要複製對手場上哪一隻寶可夢的哪個招式（先前是自動挑印刷傷害最高的招式、無法自選）。（2）修正「身上附有『古舊能量』的寶可夢，被奧利瓦ex『油之機關槍』擊倒時，對手少拿 1 張獎賞卡的效果沒有生效」的問題——油之機關槍先前用簡化的獎賞計算、沒套用古舊能量等調整；已改走統一的獎賞計算（同時補正「同命戰鬥」「深淵之瞳」「反彈傷害擊倒」等少數手動擊倒路徑的同類調整：莉莉艾的珍珠／豪華斗篷／影藏等）。</summary>
      </details>

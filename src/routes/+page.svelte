...
<details open>
      ...
      <details>
        <summary><span class="ver-badge">v5.429</span> 修正（擲幣全面整理）：將所有擲硬幣的招式／特性改走統一的擲幣流程，讓寶可夢道具「重試徽章」對所有擲幣招式都能正常觸發重擲（先前部分招式因擲幣方式不同而被偵測不到）；涵蓋約 40 個擲幣點、20 個檔案。另順手修正蒂蕾喵「魔法葉」原本「加傷」與「回血」用兩次獨立擲幣（脫鉤）的問題，改為依同一次擲幣結果決定</summary>
      </details>

      <details>
        <summary><span class="ver-badge">v5.428</span> 修正：高傲雉雞「高速飛翔」等部分擲幣招式，裝備寶可夢道具「重試徽章」時沒有效果（這些招式的擲幣沒走正規擲幣流程，使重試徽章偵測不到擲幣）。現已修正：高速飛翔、深處潛水、潛水、躲藏、咕咕鴿的飛翔、偷襲、踹、暴動、雜技 等招式可正常觸發重試徽章重擲</summary>
      </details>

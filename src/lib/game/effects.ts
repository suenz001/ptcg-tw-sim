...
import {
  ...
  ATTACK_USE_PRECONDITION,   // ⭐v6.350 per-attack 使用前提（engine 兩處共用同一份）
...
export { ATTACK_USE_PRECONDITION };   // ⭐v6.350
...
//   (d) 不計算對手身上附加效果（skipDefEffects，Session 33 已正式實作）— 故勒頓|撕裂
//   (e) 自身中毒則增傷 — 夠讚狗ex|瘋狂連鎖
//   (f) 攻擊 + 抽 N 張 — 貓頭夜鷹|鉤爪搜尋（v2.159 升級為 deck-search 玩家自選）
//   (g) 對對手任一寶可夢造成傷害 — 皮卡丘|電磁電光（10 傷害，opp-poke-choose）
//
// 補完紀錄（此段原標「已知未補完項」，經核對均已實作，v5.843 更正過時註解）：
//   - 地盤崩壞「古代支援者」+3 張：已用 ancientSupporterPlayedThisTurn 追蹤（共 4 張）
//   - 撕裂「不計算身上附加效果」：已用 skipDefEffects 實作（Session 33）
//   - 鉤爪搜尋：v2.159 已升級為 deck-search 任選最多 2 張
// ══════════════════════════════════════════════════════════════════════════════
...
export function applyAttackerActiveDamageBonuses(
  ...
  let d = dmg;
  let s = state;
  // >>> v6409-clamp-at-the-end
  // ⭐⭐⭐v6.409 站長裁定（2026-09-20）：**攻擊方的所有加減先算完，最後才不能低於 0。**
  //   逐字：「傷害應該是 50+40-100=-10 然後等於 0」。
  //   ⇒ 中途**不**夾 0：招致削傷之後即使變成負的，後面的加項照樣累加。
  //   例：基礎 50、受「吠」-50、身上 2 張伏特【雷】能量
  //       舊：50-50=0 ⇒ 逐段閘擋掉後面 ⇒ **0**
  //       新：50-50+40 = **40**
  //   例：基礎 50、受「大聲咆哮」-100、2 張伏特
  //       50-100+40 = -10 ⇒ 最後 clamp ⇒ **0**（新舊同值）
  //
  // ⚠ v6.408 曾在這裡放一組 `_perTermGate` 逐段閘，逐字保留 engine inline 版的
  //   「中途 clamp 之後不再加」。那是**未裁定前不夾帶行為改變**的權宜；裁定下來之後拆掉。
  // ⚠ 兩條路徑（engine 主管線 / 延後・狙擊）現在**共用同一套算法**，不再分歧。
  // ⚠ `_engineMainline` 只剩一個用途：旗標 `_attackerActiveBonusDone` 的設定時機
  //   （engine 主管線沿用「最終 > 0 才標記」，讓最終 0 的招式仍能由中央 helper 補套）。
  const _engineMainline = !!opts?.attackerSnapshot;
  // <<< v6409-clamp-at-the-end
  const _colorlessBlocked = (card: Card | undefined): boolean => {
    if (!card || card.pokemonType !== 'Colorless') return false;
  ...
  // ⭐⭐v6.409：**唯一的 clamp 點**（站長裁定：所有加減算完才不能低於 0）。
  //   ⚠ 這一行是本函式對外的保證：回傳的 damage 永遠 ≥ 0，呼叫端不必自己再夾一次。
  if (d < 0) d = 0;
  // ⚠v6.408：engine 主管線沿用「**加成後** > 0 才設這個旗標」（最終 0 時不設，
  //   讓中央 helper 之後還能補套）。延後／狙擊路徑照舊無條件設。
  if (!_engineMainline || d > 0) s = { ...s, _attackerActiveBonusDone: true } as GameState;
...
//
//   skipWeakRes    — 傷害不計算弱點與抵抗力（引擎 v2.260 已實作抵抗力計算；此旗標同時跳過兩者）
...
regG('珍寶配件', (st, idx) => st.players[idx].deck.length > 0);
reg('珍寶配件', (st, idx) => {
...
regG('能量輸送PRO', (st, idx) => st.players[idx].deck.length > 0);
reg('能量輸送PRO', (st, idx) => {
...
export function isLazyTraitBlockingAttack(
  ...
  // ⚠ v6.113：這裡**只判卡面條件**（對手場上有沒有 ex/V）。
  //   「特性有沒有被消除」一律交給 engine 的 selfAttackPreconditionBlock →
  //   isAbilityHolderEffective 統一處理（火箭隊的監視塔／傳說的熔岩洞／鐵荊棘ex 初始化／
  //   招式版暗夜羽擊／振翼髮 passive 全都涵蓋）。
  //   舊版在這裡自己 gate 了監視塔一種，結果**漏掉其餘四種**——那正是 v6.113 修的 bug。
  const aIdx = state.activePlayerIndex;
  const dIdx = (1 - aIdx) as 0 | 1;
  const opp = state.players[dIdx];
...
export const ON_EVOLVE_FROM_HAND_ABILITIES = new Set([
  ...
  '全滿旋律',     // v5.950 西獅海壬 — 進化(糖果/Stage1)時可用1次:恢復自方1隻全部HP。原漏收→進化不自動彈窗、時好時壞(玩家報)
]);

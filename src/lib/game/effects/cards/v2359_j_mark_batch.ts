...
// v5.843：卡面「將對手中毒」無擲幣 → 直接中毒(原誤用 coinStatusFn 擲幣才中,且尖括號 key 版失效)。
regPost('莉佳的臭臭花|噴毒', statusPost('poisoned'));
...
// 火狐狸（M4 Basic Fire 70HP）｜呼朋引伴：0 傷，從牌庫選最多 2 隻基礎寶可夢放備戰
// v4.63 修：原本誤寫 1 隻，卡面 JSON 確認是「最多 2 張」（同火狐狸 SV M4 011/083）
// 卡面：「從自己的牌庫選擇 1 張【基礎】寶可夢卡，放置於備戰區。並且重洗牌庫。」
regPre('火狐狸|呼朋引伴', (state) => ({ state, damage: 0 }));
regPost('火狐狸|呼朋引伴', benchBasicFn(2, '呼朋引伴'));

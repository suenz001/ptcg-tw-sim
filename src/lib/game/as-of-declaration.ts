...
export const AS_OF_DECLARATION_ABILITIES: readonly string[] = [
  ...
  // <<< v6375-a1-field-wide-reduce
  // >>> v6376-a1-max-hp
  // ⭐⭐v6.376 站長裁定 A-1／A-3（「凡是有這種類似的狀況，請你都比照花之帷幔」）：
  //   樂天河童｜生機森巴 是**最大 HP** 型，消費點是 engine.ts 的 getEffectiveMaxHp，
  //   而 getEffectiveMaxHp 正是**昏厥判定**讀的那一支 ⇒ 與 v6.375 五張 field-wide 減傷
  //   同一條路徑（備戰那一段排在戰鬥位的 KO 分支之後，讀不到已昏厥離場的持有者）。
  //   ⭐ v6.376 行為端實測（__m6a/probe376c.mjs，真卡 三首惡龍ex｜黑曜石 130 ＋ 對手 2 隻
  //     備戰各 130；樂天河童 HP140＋40＝180，damage 60 ⇒ 190 ≥ 180 在戰鬥位被同一招打死）：
  //       持有者活著 ⇒ 備戰兩隻 HP130 的最大 HP 是 170、受 130 **存活**，獎賞 0 張；
  //       持有者被同一招打死 ⇒ 最大 HP 掉回 130、兩隻**全部被 KO**，獎賞 3 張。
  //   ⚠⚠ 這一條會**改變獎賞卡數量**（進而改變勝負），是本版唯一會改變線上既有行為的卡。
  //   ⚠ 卡面明文「無論有多少隻…也不會重複」：消費點只問「有沒有算數的持有者」，

...
export const HIDDEN_FROM_PLAYERS: Readonly<Record<string, HiddenCardInfo>> = {
  ...
  // ⚠⚠⚠ v6.328：以下三張是「傳說」競技場**右半的舊 id**。
  //   v6.093 拆卡時自己造右半 id，卻捏到了 19624/19625/19626 ——
  //   那三個號碼**其實是台灣官方 M-P 209~211**（膽小蟲／超級米立龍ex／麻麻小魚），
  //   repo 自己的 `scripts/data/official-set-manifest.json` 早就把它們列在 M-P 底下，
  //   ⇒ 那三張官方卡永遠補不進來（補了就 id 重複），其中超級米立龍ex 是標準賽可用卡。
  //   v6.328 把右半改成「左半 id ＋ `-1`」（官方 id 一律純數字 ⇒ 結構上不可能再撞號）。
  //
  // ⭐ 為什麼舊 id 的**卡片資料要留著**、只是玩家選不到：
  //   部署當下**進行中的對局**與**部署前拍下的錦標賽報名快照**裡仍帶著舊 id，
  //   資料一旦拿掉，那些盤面上的競技場會變成「查不到的卡」。留著就完全無感。
  //   舊牌組則由 `migrateCardId`（走本表推導出的 RETIRED_DUP_TO_TW_ID）在載入時換成新 id。
  //
  // 🔨 v6.329 待辦：確認雲端牌組都被載入改寫過之後，把這三筆連同 `static/cards/M6.json`
  //   裡那三筆停用卡一起刪掉，**同版**才可以把 19624/19625/19626 讓給官方那三張卡。
  //   ⚠ 順序反了會讓玩家的傳說場地卡變成一張長相與類別都不同的寶可夢。
  '19624': {
    replacementId: '19621-1',
    setCode: 'M6',
    reason: '傳說的海溝 072/076 右半的舊 id：19624 其實是官方 M-P 209（膽小蟲），v6.328 起改用 19621-1',
  },
  '19625': {
    replacementId: '19622-1',
    setCode: 'M6',
    reason: '傳說的山頂 074/076 右半的舊 id：19625 其實是官方 M-P 210（超級米立龍ex），v6.328 起改用 19622-1',
  },
  '19626': {
    replacementId: '19623-1',
    setCode: 'M6',
    reason: '傳說的熔岩洞 076/076 右半的舊 id：19626 其實是官方 M-P 211（麻麻小魚），v6.328 起改用 19623-1',
  },
};

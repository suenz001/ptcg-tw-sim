 ...
 *
 * 當時台灣官方在 M-P 215/M-P（I 標，id 19630）的印刷，站內收成「老大的指令（烏羽）」：
 * `rulesText` 與「老大的指令」**逐字相同**，括號內是卡圖上的角色。
 * ⇒ 牌組「同名卡最多 4 張」必須**共用同一份額度**（合計 4 張，不是各 4 張）。
 *
 * ⚠ v6.193：站長裁定把那張卡直接改名為「老大的指令」⇒ **卡庫目前一個括號卡名都沒有**。
 *   這支函式與例外表**保留不動**：它是通則守衛，官方下次再發括號卡名時要立刻擋得住。
 *   （守衛 `scripts/test-v6192-same-name-art-variant.mjs` 已改用合成卡驗機制，
 *   同時繼續枚舉卡庫裡的括號卡名 —— 有新卡就紅燈逼人判讀。）
 *
 ...
 *   2. 全站卡庫的卡名裡，帶括號的一個都沒有（見下方守衛），沒有反例。
 ...
 */
/**
 * ⚠⚠ v6.328 右半 id 換號：舊的右半 id（19624/19625/19626）**其實是官方 M-P 209~211 的 id**
 *   （膽小蟲／超級米立龍ex／麻麻小魚）—— v6.093 造右半 id 時佔用了官方 id 空間。
 *   新格式是「左半 id ＋ `-1`」：官方 id 一律純數字，所以**結構上不可能**再被官方佔用。
 * ⚠ 這裡**刻意不收舊的右半 id** —— 舊 id 已在 `$lib/cards/visibility` 登記為停用卡，
 *   舊牌組載入時由 `migrateCardId` 換號；配對邏輯只認新 id，避免兩套判準並存。
 *   守衛：scripts/test-v6328-two-card-stadium-id-renumber.mjs
 */
export const TWO_CARD_STADIUM_PAIR_IDS: Readonly<Record<string, string>> = {
  '19621': '19621-1', '19621-1': '19621',   // 傳說的海溝   071/076 ↔ 072/076
  '19622': '19622-1', '19622-1': '19622',   // 傳說的山頂   073/076 ↔ 074/076
  '19623': '19623-1', '19623-1': '19623',   // 傳說的熔岩洞 075/076 ↔ 076/076
};

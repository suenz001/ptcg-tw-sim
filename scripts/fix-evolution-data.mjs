...
const FIXES = {
  ...
  // === v2.62 Leon 回報：火箭隊的操陷蛛（Stage1）缺 evolvesFrom → 被誤當 Basic ===
  // SV10 009/098 + 099/098 + M2a 016/193 + 197/193 全部缺欄位，爬取時
  // findIndex 對 `<火箭隊的>` 尖括號名匹配失敗 → 整條 scraper heuristic 沒寫入。
  // 正確前階：<火箭隊的>團珠蛛（同 set 皆 Basic）。此欄位修完，引擎的
  // isBasicPokemonCard 以及進化鏈檢查都會恢復正常。
  '火箭隊的操陷蛛': '火箭隊的團珠蛛',
  '<火箭隊的>操陷蛛': '火箭隊的團珠蛛',
};

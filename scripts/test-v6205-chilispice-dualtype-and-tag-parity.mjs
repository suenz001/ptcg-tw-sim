...
const ADJUDICATED_IMPLEMENTED=new Map(Object.entries({
  ...
  // ⭐ v6.358 移除（比照上面 v6.257 鬥志戰吼／下面 v6.207 威嚇之顎 的同型處理）：
  //   v6.358 的 EFFECT_SOURCE_IMMUNITY（effects.ts）那一筆同時寫著
  //   `card: '陳舊的背蓋化石'` 與 `member: '背蓋守護'`（站長裁定 D-9 的卡面逐字錨）
  //   ⇒ 候選判準（「特性名的出現點有沒有綁到自己的卡名」）不再把它列為候選，
  //     留在表裡就是 7e 的死條目。實作本身沒有變，逐字錨改由
  //     scripts/test-v6358-effect-source.mjs 的【G】G2 釘住。

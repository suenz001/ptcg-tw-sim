...
//   ⭐⭐⭐v6.326【真正擋住事故 2 的是「規則順序」，不是任何一道護欄】——
//       事故 2 的形狀是「`//` 註解裡的 `/*` 一路吃掉真碼」。擋住它的是**規則 (a) 排在區塊判斷之前**：
//       不在區塊內時先看行首 `//`，命中就 `continue` 丟整行 ⇒ 那一行裡的 `/*` 根本沒有機會開區塊。
//       ⚠ 護欄①（留存率）③（區塊長度）⑤（連續丟棄行數）對這個形狀**都攔不到**，實證：
//         · M11b  只把 `t.startsWith(open)` 換成 `line.includes(open)` ⇒ anti-pattern-lint **rc=0（不紅）**，
//           因為規則 (a) 先把 `//` 行丟掉了，區塊開不起來。
//         · M11b′ 再把規則 (a) **搬到區塊判斷之後** ⇒ anti-pattern-lint **rc=1**，
//           紅在 Check Y 的洞內正對照（`[Y] 正對照失敗`）。
//       ⇒ 這兩條由 test-lib-strip-comments【7】M11a／M11b′ 實跑撐著（跑的是真的 lint 子行程，不是字串比對）。
//       ⚠ 這是**實話不是保證**：它說的是「今天擋住事故 2 的機制是規則順序」，
//         不是「這個順序絕不會被改壞」—— 會不會被改壞，只由那兩條突變決定。
...
//   ⚠⚠ ⭐v6.325 明寫護欄①的**盲帶**：`minRatio` 降到 0.02 之後，「剝除器吃掉 10%~50% 實碼」這一整段
//      **沒有任何護欄看得到**。實測（engine.ts，每 100 行丟掉前 N 行）：吃掉 70% ⇒ 留存 29.3%、吃掉 90% ⇒ 留存 10.0%，
//      兩者在 0.02 之下**都是綠的**；舊的 0.5 兩個都紅。⇒ 0.02 買到的是「多檔掃描不誤紅」（全站正常下限 8.5%），
//      代價就是這條盲帶。**這一段只能靠 ②mustKeep／③maxBlockLines／⑤maxDropRun 與呼叫端的已知答案表兜住**，
//      單檔守衛若擔心這一段，請自己明寫更嚴的 minRatio。
//   ⚠ 本檔頭已經被改過八版，每一版的「方向保證」都被後續突變推翻過 ⇒ 這一段**不寫任何「絕不會／保證」**：
//      上面每一句都對應 test-lib-strip-comments 的一條實跑突變，沒有突變撐著的宣稱一律不寫。

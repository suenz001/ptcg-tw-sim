...
import { stripV6421Engine } from './lib/engine-strip-v6421.mjs';   // ⭐v6.421 自傷同時昏厥判平手＋KO 獎賞歸屬（5 組）
...
const TOURN_TAIL_SHA256_V6276 = '9b234e690ec261cafc40031b0730821042ef625e4383c8e46b9df857cb2df4d1';
...
//   剝除器全部是純函式（沒有斷言、沒有副作用）⇒ 一律上移到模組層，五條共用同一份。
  ...
  // ⭐ v6.350：engine.ts 合法新增「per-attack 使用前提」的三個區塊（import ＋ ATTACK handler
  ...
  // ⭐ v6.354：engine.ts 的合法改動**全部是純新增**（v3001_g3_wave3 的 import 一行、
  //   markHealsByDamageDecrease 裡的 healBlocked 宣告、「禁止恢復HP」中央閘、中央揭示 log）
  ...
  // ⭐ v6.356：engine.ts 的合法改動**全部是純新增**（撼盪拳中央閘 tremorPunchTrainerGate、
  //   PLAY_TRAINER／PLAY_FOSSIL 兩個呼叫點、END_TURN 的 clear 條件一行＋delete 一行＋promote 區塊）
  //   ⇒ 一律 v6356- 哨兵框住，泛用剝除器一次剝掉，不需要逐字還原。
  //   那一塊的守備由 scripts/test-v6356-tremor-punch.mjs 全面接管。
  const stripV6356Engine = (src) => stripSentinelBlocks(src, 'v6356-');
  // ⭐ v6.357：engine.ts 的合法改動**全部是純新增**（effects 的 import 一行、ATTACK 宣告點的
  //   field-wide 反擊持有者快照、attack flow ＋ applyAction wrapper 兩個 clear 區塊）
  ...
  // ⭐ v6.361：engine.ts 合法新增「中央終局判定（含平手）＋ 先結算 on-KO 再判勝負的延後」
  //   —— 三段**全部是純新增**，一律 `>>> v6361-…` 哨兵框住 ⇒ 泛用剝除器一次剝掉，
  //   不需要逐字還原。那一塊的守備由 scripts/test-v6361-draw-and-ko-order.mjs 全面接管。
  const stripV6361Engine = (src) => stripSentinelBlocks(src, 'v6361-');
  // ⭐ v6.362：engine.ts 合法新增「跨 take-prize-choose picker 之後補洗手牌」（站長裁定 A-1）
  //   —— 三段（import／RESOLVE_SELECTION 的補跑／END_TURN finalize 的清除）**全部是純新增**，
  ...
  for (const [p, sha] of [['src/lib/game/engine.ts', BASE_SHA_V6309]]) {
    ...
    const cur = p === 'src/lib/game/oracle-client.ts' ? stripV6270(raw)
      : (p === 'src/lib/game/engine.ts' ? (() => {
        ...
        // ⭐⭐ Rule 54（由新到舊）：v6.420 必須排在 v6.419 **之前**。
        // ⭐⭐ Rule 54（由新到舊）：v6.421 必須排在 v6.420 **之前**。
        // ⭐⭐ Rule 54（由新到舊）：v6.422 必須排在 v6.421 **之前**。
        // ⭐⭐ Rule 54（由新到舊）：v6.427 必須排在 v6.422 **之前**（它是最新的一版）。
        const rawA27 = stripV6427Engine(raw);
        ok(rawA27 !== raw, 'v6.427 的還原器過期（配對區塊的字面對不上）');
        const rawA22 = stripV6422Engine(rawA27);
        ok(rawA22 !== rawA27, 'v6.422 的還原器過期（哨兵區塊的字面對不上）');
        const rawA21 = stripV6421Engine(rawA22);
        ok(rawA21 !== rawA22, 'v6.421 的還原器過期（哨兵區塊的字面對不上）');
        const rawA20 = stripV6420Engine(rawA21);
        ok(rawA20 !== rawA21, 'v6.420 的還原器過期（哨兵區塊的字面對不上）');

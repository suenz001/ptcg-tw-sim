...
// ⚠⚠ v6.327 訂正註記（7-a 的定位）：這一條斷言的是「某個突變**必須是綠的**」——
//   它是**實話存證**（記錄「今天的護欄①③⑤ 對事故 2 的形狀攔不到」這個事實），
//   **不是設計目標**。⇒ 日後若把護欄做強到連 M11a 都能攔下來，那是**改進**，
//   要改的是**這一條**（連同檔頭那句話一起重寫），**不是**回頭把護欄調弱去迎合它。
//   （型態：把「抓不到」鎖成規格，會讓「變強」看起來像「回歸」。）

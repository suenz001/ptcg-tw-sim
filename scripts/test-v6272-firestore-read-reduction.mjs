...
const PREV_SHA = 'c4d07e4b0a3db164258cb471e98566e37f23c3be';   // v6.413（上一版）
...
// ⭐v6.318：純守衛修正（strip-markup-sections 改單趟行級狀態機：註解只在模板層認、區段只在行首開／收 ⇒ 方向 A／B 兩種假綠一起收；
//   test-v6190 反對照打錯字／test-v6267 E3b 掃全部 import／test-v6288・v6296・v6297 的 import 抽取改行級剝註解）—— 玩家端零改動，只有 version.ts；不動首頁 changelog。
// ⭐v6.319：純守衛修正（strip-markup-sections 第六版：OPEN 認檔首 BOM／「行首」＝本行到此只有空白或已剝掉的註解與區段／護欄⑦殘留開頭標籤字面
...
// ⭐v6.323：純守衛修正（strip-comments.mjs 加等長留白渲染＋mustDrop／對照字串先斷言在原檔存在；新自驗檔 test-lib-strip-comments；
//   test-v6179 期望值收緊（歸零只數 _tResetResSeg 本體）；game 頁六支守衛的區塊正則改走中央狀態機；五處 slice(indexOf) fail-open 加 anchor 斷言）
//   —— 玩家端零改動，只有 version.ts；不動首頁 changelog。
// ⭐v6.324：純守衛修正（strip-comments 的 renderDropped 尾巴空白定義改用 WS_RE／護欄①預設降到 0.02＋新增護欄⑤ maxDropRun／
//   等價性斷言改成報行號欄號不 dump 大字串；test-v6210 的自寫區塊正則整支改走中央留白版；test-v6158:266 的 slice(indexOf) fail-open）
//   —— 玩家端零改動，只有 version.ts；不動首頁 changelog。
...
const PREV_ALLOWED = [
  // ⚠⚠ v6.414 前移：PREV_SHA 從 v6.413（c4d07e4b）起算。
  //   ⚠ PREV_SHA 必須是「留在 main 上的那一顆」（IRON_RULES Rule 45）。
  // ⭐v6.414：「在下個自己的回合…傷害 +N」依卡面分成**招式限定**與**通用**兩種。
  //   站內先前把 5 張招式限定的卡也寫成通用 ⇒ 下回合用**別的招式**也吃到加傷
  //   （巨金怪｜彗星拳 +60 外溢到潔淨爆破、路卡利歐ex｜龍捲風猛攻 +100 外溢到波動衝天）。
  //   步哨鼠｜聚氣的「傷害**改為** 240」也改成真正的覆寫（原本用 +160 逼近）。
  //   6 個手刻寫入點一起收斂到中央 setter（Rule 38）。這是**玩家看得到的修正**。
  ...
  'src/lib/game/effects/cards/v2670_i_wave17_complex2.ts',

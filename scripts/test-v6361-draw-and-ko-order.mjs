...
console.log('\n【G】中央判定函式 judgeEndgameV6361 直接單元測（官方 §14 的編號順序）');
{
  ...
  // ⭐⭐⭐v6.420（站長裁定，IRON_RULES Rule 40：守的意圖沒變，只有「同時成立」那一格改了）
  //   原本這一條寫「即使該方場上已空，也**不是**平手」。站長裁示（逐字）：
  //   「單方取完6張獎賞、而同一瞬間該方自己也沒有寶可夢可上場…應該判定為雙方平手」。
  //   ⚠ 官方規則 §14 只列了三條勝敗條件（①取完獎賞 ②沒有寶可夢可上場 ③抽不到牌），
  //     **沒有明文**規定「兩條同時成立」怎麼辦 ⇒ 這是設計裁定，不是規則事實。
  //     與 v6.361 自己定的 D-10／D-11（雙方同時取完 ⇒ 平手）內部一致。
  chk('G4 ⭐§14-① 只有一方取完、而且**該方還有寶可夢** ⇒ 該方獲勝',
    J(st(1, 1, 1, 1, 0, 3), true).winner === 0 && J(st(1, 1, 1, 1, 3, 0), true).winner === 1,
    JSON.stringify([J(st(1, 1, 1, 1, 0, 3), true), J(st(1, 1, 1, 1, 3, 0), true)]));
  chk('G4b ⭐⭐v6.420 站長裁定：取完獎賞的那一方**自己**場上已空 ⇒ 兩個條件同時成立 ⇒ 平手',
    J(st(0, 0, 1, 1, 0, 3), true).winner === null && J(st(1, 1, 0, 0, 3, 0), true).winner === null,
    JSON.stringify([J(st(0, 0, 1, 1, 0, 3), true), J(st(1, 1, 0, 0, 3, 0), true)]));
  chk('G4c ⭐ 零回歸：沒寶可夢的是**對手**時，兩個條件都指向同一方 ⇒ 仍然是該方獲勝',
    J(st(1, 1, 0, 0, 0, 3), true).winner === 0 && J(st(0, 0, 1, 1, 3, 0), true).winner === 1,
    JSON.stringify([J(st(1, 1, 0, 0, 0, 3), true), J(st(0, 0, 1, 1, 3, 0), true)]));

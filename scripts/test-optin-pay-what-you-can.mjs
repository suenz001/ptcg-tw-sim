...
writeFileSync(E, `export { ATTACK_PRE, ATTACK_POST, OPTIN_NO_PAYMENT, ATTACK_PRE_DISCARD_CHOICE, flushAttackEnergyPayment } from './src/lib/game/effects';\nimport './src/lib/game/effects';`);
await build({ entryPoints:[E], outfile:O, bundle:true, format:'esm', platform:'node', target:'node20', alias:{ '$lib':join(ROOT,'src/lib'), '$app/paths':S }, logLevel:'error' });
const _M = await import(pathToFileURL(O).href);
const { ATTACK_PRE, ATTACK_POST, OPTIN_NO_PAYMENT, ATTACK_PRE_DISCARD_CHOICE } = _M;
// ⭐⭐⭐v6.407：「付出」已經從 PRE 延後到 engine 的「傷害造成後」（官方三段順序：
//   傷害 → 招式效果（含付出）→ 受傷時的特性／道具；依據：伏特【雷】能量 Q&A
//   ＋PTCG_RULES §17.46.D）。這支守衛直呼 PRE，所以要自己補跑 flush 才看得到結果。
// ⚠ 哨兵：舊版沒有這支 helper（PRE 就直接付出）⇒ 回原 state 即可，新舊兩版斷言都成立。
const _flush = (typeof _M.flushAttackEnergyPayment === 'function') ? _M.flushAttackEnergyPayment : ((st) => st);
/** 跑完「付出」之後的 state（守衛的觀測點往後移一步，IRON_RULES Rule 40）。 */
const FL = (st) => _flush(st, pool);
...
T('忍者飛旋:選否([])→120', () => {
  ...
  assert.equal(FL(out.state).players[0].active.energyAttached.length,1,'不付出');
});
T('忍者飛旋:有1水 opt-in([iid])→200+水回手', () => {
  ...
  assert.equal(FL(out.state).players[0].active.energyAttached.length,0,'水應離身');
  assert.ok(FL(out.state).players[0].hand.some(c=>c.iid===w.iid),'水應回手');
});
...
T('忍者飛旋:2水 opt-in 只付1(payMax=1)', () => {
  ...
  assert.equal(out.damage,200);
  assert.equal(FL(out.state).players[0].active.energyAttached.length,1,'只放回1張');
});
T('忍者飛旋:有雷無水 opt-in(sentinel)→200 且雷不動(filter)', () => {
  const l=en(LIGHTNING); const st=mk(inst(GRENINJA,[l]),inst(BUD));
  const out=ninjaPre(st,0,pool,{discardedEnergyIids:[OPTIN_NO_PAYMENT]});
  assert.equal(out.damage,200);
  assert.equal(FL(out.state).players[0].active.energyAttached.length,1,'雷能量不是水,不可付');
});
// ── 皮可西 揮指 borrow ──
...
T('揮指借忍者飛旋:皮可西有1水→200+回手', () => {
  ...
  assert.equal(out.damage,200);
  assert.ok(FL(out.state).players[0].hand.some(c=>c.iid===w.iid),'皮可西的水應回手');
});
T('揮指借忍者飛旋:選否→120', () => {
...
T('災難衝擊:選否([])→不丟不麻痺', () => {
  ...
  const out=FL(disasterPost(st,0,pool,{discardedEnergyIids:[]}));
...
T('災難衝擊:0雷 opt-in(sentinel)→麻痺(裁定2)', () => {
  ...
  const out=FL(disasterPost(st,0,pool,{discardedEnergyIids:[OPTIN_NO_PAYMENT]}));
  assert.ok(paralyzedOf(out),'0雷 opt-in 應麻痺');
});
T('災難衝擊:1雷 opt-in→丟1+麻痺(裁定2)', () => {
  const l=en(LIGHTNING); const st=mk(inst(EEL,[l]),inst(BUD));
  const out=FL(disasterPost(st,0,pool,{discardedEnergyIids:[l.iid]}));
...
T('災難衝擊:2雷 opt-in→丟2+麻痺', () => {
  ...
  const out=FL(disasterPost(st,0,pool,{discardedEnergyIids:[l1.iid,l2.iid]}));
...
T('災難衝擊:3雷 opt-in只給1 iid→auto-top-up丟2(公平性)', () => {
  ...
  const out=FL(disasterPost(st,0,pool,{discardedEnergyIids:[l1.iid]}));
...
T('時間爆炸:2能量 opt-in→160+全回牌庫', () => {
  ...
  assert.equal(FL(out.state).players[0].active.energyAttached.length,0);
  assert.equal(FL(out.state).players[0].deck.filter(c=>c.iid===e1.iid||c.iid===e2.iid).length,2,'能量應進牌庫');
});
...
T('狂暴噴射:2能量 opt-in→330+全丟(PRE即付)', () => {
  ...
  assert.equal(FL(out.state).players[0].active.energyAttached.length,0,'應全丟');
  assert.equal(FL(out.state).players[0].discard.filter(c=>c.iid===e1.iid||c.iid===e2.iid).length,2);
});
T('狂暴噴射:選否→200保留能量', () => {
  const e1=en(LIGHTNING); const st=mk(inst(EELEK_M,[e1]),inst(BUD));
  const out=eelekPre(st,0,pool,{discardedEnergyIids:[]});
  assert.equal(out.damage,200);
  assert.equal(FL(out.state).players[0].active.energyAttached.length,1);
});
...
T('叢林鞭打:2能量 opt-in→160+全回手(PRE即付)', () => {
  ...
  assert.equal(out.damage,160);
  assert.equal(FL(out.state).players[0].active.energyAttached.length,0);
  assert.equal(FL(out.state).players[0].hand.filter(c=>c.iid===e1.iid||c.iid===e2.iid).length,2,'能量應回手');
});
// ── 金屬之錘 auto-top-up ──
T('金屬之錘:4鋼 opt-in只給1 iid→auto-top-up丟3', () => {
  ...
  assert.equal(FL(out.state).players[0].active.energyAttached.length,1,'4鋼應強制丟3留1');
});
...
T('忍者飛旋:平穩境地擋回手 opt-in→仍+80=200且水留身', () => {
  ...
  assert.equal(FL(out.state).players[0].active.energyAttached.length,1,'水留身(平穩境地擋回手)');
  assert.ok(!FL(out.state).players[0].hand.some(c=>c.iid===w.iid),'水未回手');
});
T('叢林鞭打:平穩境地擋回手 opt-in→仍+80=160且能量留身', () => {
  ...
  assert.equal(FL(out.state).players[0].active.energyAttached.length,1,'能量留身');
});

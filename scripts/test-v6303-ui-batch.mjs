...
import { revertCasualIdleV147 } from './lib/sap-revert-casual-idle-v147.mjs';   // ⭐v6.425 鏈又長一節（Rule 54：由新到舊）
...
let hasPw = false;
// ⭐⭐v6.409：spawn measure-*.mjs 型 ⇒ 必須用 pwUsable（真的 launch 一次），
//   不能用只檢查模組的 pwChromium —— 理由與 test-v6297 【F】那一格相同。
hasPw = await pwUsable('v6.303 【G】四種尺寸的 DOM 量測');
if (!hasPw) {
...
await T('H3 ⭐ 沒有動到不該動的檔：oracle-admin/server_admin_patch.js 與 BASE **逐位元相同**', () => {
  ...
  const SAP_REVERTED = revertV6381(revertAdminV146(revertCasualIdleV147(SAP_RAW)));   // 由新到舊（Rule 54）

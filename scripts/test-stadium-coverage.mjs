...
writeFileSync(E,"export { PASSIVE_STADIUMS } from './src/lib/game/effects';\nimport './src/lib/game/effects';");
await build({entryPoints:[E],outfile:O,bundle:true,format:'esm',platform:'node',target:'node20',alias:{'$lib':join(ROOT,'src/lib'),'$app/paths':S},logLevel:'error'});
const { PASSIVE_STADIUMS } = await import(pathToFileURL(O).href);
...
const uncovered=[...stadiums].filter(n=> !PASSIVE_STADIUMS.has(n) && !ACTIVE_STADIUMS.has(n));
chk('每張 live HIJ 競技場都有掛 hook(∈ PASSIVE 或 ACTIVE 觸發型)', uncovered.length===0);

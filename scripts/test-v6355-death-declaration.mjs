...
console.log('\n【F】game-over 順序（獎賞先給攻擊方，再輪到死亡宣告）');
{
  ...
  // F2 ⭐⭐ v6.361 站長裁定 D-10／D-11 已下 ⇒ 依 **Rule 40 上移到意圖層**（不放寬、不刪除）。
  //    原本這一條釘的是「攻擊方先取完就直接獲勝、死亡宣告不再結算」的**現行行為**
  //    （v6.355 自己就寫明那是待站長裁示的暫時錨）。站長裁定逐字：
  //      「應該先結算死亡宣告再判勝負，有可能雙方一起昏厥(剛好都是場上最後一隻寶可夢)，
  //        就判定為平手請另開一版處理」
  //    ⇒ 意圖層＝① 死亡宣告**仍然結算**（攻擊方真的昏厥、離場）
  //              ② 雙方同時取完所有獎賞卡、且雙方都還放得出戰鬥寶可夢
  //                 ⇒ 依 PTCG_RULES L622／L728／L894／L1668 逐字「若雙方皆可以放置…則為平手」
  //                 ⇒ **平手**（winner 這個 key 不存在、isDraw===true），不是任一方獲勝。
  const F2 = runIce(boardIceVsGengActive({}, 1, 1));
  chk('F2 ⭐⭐雙方都剩 1 張獎賞 ⇒ 死亡宣告**仍然結算**（攻擊方昏厥離場）、雙方同時取完 ⇒ **平手**',
    F2.phase === 'game-over' && F2.winner == null && F2.isDraw === true
    && A0(F2) == null && PZ(F2, 0) === 0 && PZ(F2, 1) === 0
    && nLog(F2, '「死亡宣告」啟動') === 1,
    JSON.stringify([F2.phase, F2.winner, F2.isDraw, !!A0(F2), PZ(F2, 0), PZ(F2, 1),
      nLog(F2, '「死亡宣告」啟動')]));

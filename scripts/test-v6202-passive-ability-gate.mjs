...
// ⭐v6.326【A 類：收斂就會掉】92（slack 1）→ 90（slack 3，實測 93）。
//   理由：`sites` 數的是**程式碼消費點**，把手抄 dispatch 收斂成中央 helper 會讓它**合法下降**
//   —— 上面那行註解記著的 v6.258 就是實例（90 → 88，淨減 2）。slack 1 會讓那次合法收斂翻紅，
...
T('20b. 掃描器下限：兩種 pattern 都要抓得到東西（少一種＝那一族又隱形了）',()=>{
  ...
  // ⭐v6.326【A 類：收斂就會掉】58/33（slack 1）→ 56/31（slack 3，實測 59/34）。
...
//   理由：`gated` 是 `sites` 的**子集** —— sites 因為收斂而下降時它一定跟著下降。
//   ⚠ 掉 1~3 多半是合法收斂，確認後改這一行。

...
// ══════════════════════════════════════════════════════════════════════════════
console.log('\n【4】⚠ 本批**未實裝**的 4 個特性 —— 卡面逐字錨（待站長裁示；卡面若改版這裡會紅）');
{
  // ⚠ 這一段**不是**「宣告永遠不做」。它只做兩件事：
  //   ① 把 4 張待裁示卡的 abilities[].effect 逐字釘住（卡面改版／抓錯卡時會紅）；
  //   ② 讓交接的人一眼看到「哪 4 個沒做、為什麼」。實作了之後把該條移到自己的章節即可。
  const FACE = [
    ['甜甜螢', '絕佳費洛蒙',
    ...
    ['弱丁魚', '群聚反擊',
      '只要這隻寶可夢在場上，自己戰鬥場的「弱丁魚（包含『寶可夢【ex】』）」受到對手的寶可夢招式的傷害時，在使用招式的寶可夢身上放置3個傷害指示物。',
      '「持有者在場上、觸發者是戰鬥場的另一隻」＝ field-wide 反擊；站上唯一同型（花岩怪｜怨恨旋渦）'
      + '是在 engine 的 KO／非 KO 兩個分支 + effects.fireDefenderOnDamaged 共 3 處 inline 手寫的，'
      + '要加第二張必須先把那 3 處收斂成中央 helper ＝ 改 engine 的 ATTACK 核心流程'],
    ['耿鬼ex', '死亡宣告',
...
// ══════════════════════════════════════════════════════════════════════════════
console.log('\n【4B】⚠⚠ 上面 4 個待裁示特性「目前確實沒有作用」—— 一律用**行為端數字**釘住');
{
  ...
  // ── (b) 弱丁魚｜群聚反擊：卡面要在攻擊方身上放 3 個傷害指示物；現況 0 ──────────
  ...
  chk('4B-b ⭐行為端：攻擊方（急凍鳥）身上 0 點 ⇒ 群聚反擊目前確實無作用',
    A0(rFe)?.damage === 0, String(A0(rFe)?.damage));

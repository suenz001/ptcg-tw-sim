...
站長裁定：「依你的建議處理」。本版**一個位元組的站台程式碼都沒動**，只改守衛腳本＋版本號。

### 【零】事故：連續兩版被噪音判準擋下來，原樣 rerun 就綠

| 版本 | 被擋的判準 | 量到的值 | 真因 |
|---|---|---|---|
| v6.334 | `test-v6234` ⑦ `maxFlips < 20` | 最多 22 次 | 3000 次真隨機擲幣，P(max ≥ 20) ≈ **0.57%／次 CI** |
| v6.335 | `test-v6242` ⑤ `shipped.ms < bare.ms * 1.3` | 多了 34% | 拿同樣有噪音的牆鐘當分母取比值，兩邊各只量一發 |

⚠ **假紅最大的傷害不是浪費一次 rerun，而是訓練大家「看到紅先 rerun」**；
下一步就是有人幫它加 `skip`，守備靜悄悄地沒了。

### 【一】v6234 ⑦：取樣改固定種子，判準改到「官方上限碰不到」

「擲到反面為止」⇒ flips ≥ k ⇔ 前 k−1 次全正面 ⇒ P(f ≥ 20) = 2⁻¹⁹ ≈ 1.91e-6；
3000 次獨立攻擊 ⇒ P(max ≥ 20) ≈ 3000 × 1.91e-6 ≈ **0.57%**，約每 175 次 CI 紅一次。

照 IRON_RULES **Rule 40**（改判準到意圖級，不是放寬）做三件事：

1. 取樣改吃固定種子 PRNG（新檔 `scripts/lib/seeded-rng.mjs`，單一來源）⇒ 同一顆 commit 的結果完全可重現，**結構上不可能再隨機翻紅**。
2. 判準改成這一段標題真正要守的那件事 —— **官方上限 30 碰不到**，而不是隨手留的安全邊際 20。
3. 用 3 顆種子各跑 3000 次 ⇒ 證明結論不是「剛好挑到一顆會過的種子」。

⭐ 加了兩條自證，否則種子化本身會變成新的安慰劑：
- **可重現**：同一顆種子再跑一輪，`maxFlips` 與平均逐欄相同（種子哪天被拿掉，這一條會紅）。
- **反安慰劑（Rule 33）**：把硬幣灌成 95% 正面，同一段必須量到 `maxFlips === 30`（撞上限）⇒ 證明這個量測真的量得到「碰到上限」，不是恆真放行。

⚠ 種子化不削弱守備力：擲幣的正／反判準在出貨碼裡，硬幣一旦被改偏或上限被動過，
同一顆種子跑出來的平均值與 `maxFlips` 就會變 ⇒ 兩條斷言照樣紅。

### 【二】v6242 ⑤：不對噪音分母取比值，改成兩條零噪音判準

⚠ 諷刺的是，作者自己在那條斷言上面十行就寫了正確原則：

> 「門檻取『相對』為主、『絕對』放寬：沙盒 CPU 會被鄰居干擾，絕對值抓太緊會變成**隨機翻紅的守衛**（而隨機紅的守衛下一步就是被人加 skip）。」

`shipped.max` / `p99` 那幾條都照做了（`/3`、`<60`、`<25`，餘裕 5~10 倍），
**唯獨最後這條總耗時比值 1.3× 沒照做** —— 它是單次牆鐘量測的比值，分子分母都有噪音。
實測比值在不同機器上從 **1.06 到 1.35** 游走。

改成兩條，而且比舊判準**更嚴**：

1. **讓路次數 = 節拍算得出來的值**（`floor(60000 / 200) = 300`）—— 完全確定性，零牆鐘噪音。
   真正會把耗時炸掉的旋鈕就是讓路節拍；節拍從 200 改成 10 會立刻紅，舊的比值判準未必抓得到。
2. **總耗時的絕對上界** `shipped.ms < 5000`（意圖本身：admin 不可以等很久）。
   沙盒實測 753～1419 ms ⇒ 餘裕 3.5～6.6 倍；正式 VM 約 1/10，且端點另有 60 秒 TTL 快取。

⭐ 反安慰劑：把**出貨碼裡的節拍常數**突變成「每 1 筆」（其餘一字未動，仍用出貨碼那一份實作），
讓路次數必須跟著變成 4000 —— 沒有這一條，計數斷言可能只是個常數。
`extractYield()` 因此多一個 `overrideEvery` 參數：**只換常數、不重寫 helper**（Rule 38）。

### 【三】新守衛 `scripts/test-v6336-flaky-guard-criteria.mjs`

⚠ 守衛的守衛最容易變成「判準第二份」。分工寫死在檔頭：

- 「怪顎龍｜亂暴 的上限碰不到」這個**引擎行為**判準只住在 v6234 ⑦，本檔不重抄。
- 「讓路次數對不對」這個**出貨碼行為**判準只住在 v6242 ⑤，本檔不重抄。
- 本檔守**元性質**：

【A】`seeded-rng.mjs` 本身：同種子同序列、離開時一定還原（含丟例外）、偏態硬幣真的偏。
　　**A4 直接把假紅率量出來**：1000 顆固定種子 × 3000 次的純數學模擬 ——
　　新判準 `<30` 最差只到 **23**（全過）；舊判準 `<20` 在同一批裡**紅了 4 顆（0.4%）**。
　　這條同時是反安慰劑：模擬器壞掉的話舊判準也不會紅。
【B】【C】那兩段真的搬到意圖級了、不會被無聲改回去（否定型掃描，**先用中央 helper 剝註解**
　　—— 本版註解裡原封不動引用了舊寫法，不剝就誤紅）。
【D】HEAD-FAIL：**同一份掃描器**餵 BASE 的那兩段，【B】【C】必須全部判成舊寫法；
　　`seeded-rng.mjs` 在 BASE 不存在，用 `readBaseBlob().ok === false` 的哨兵斷言（Rule 41，不靠 throw）。

### 【四】為什麼兩支一起修，不拆兩版

`feedback-one-version-per-fix`：一個修法只能 bump 一個版本號，重點是**禁止把一件事拆成兩版**
（code 一版、changelog 一版）。這兩支是同一件事的兩個實例（消除 CI 隨機假紅），
判準與反安慰劑各自獨立，拆兩版只是浪費版本號池與一輪 CI。

### 【五】首頁 changelog：不放

純 CI／守衛層面的改動，對玩家沒有任何可見差異 ⇒ 依 changelog 規格「純 meta 不放首頁」。
`test-v6264` 的【F】會走 **F0**（三檔與 BASE 逐位元相同）。

## v6.335 M6「綠寶石風暴」補上 37 張高編號印刷（077/076 ～ 113/076）

BASE `813fd407990cdfde217c7190dccb5c4d227450e8`（v6.334，遠端 main）。
站長交辦：「綠寶石風暴 m6 已經有高版本（SAR、AR 等）的卡片了，功能完全比照 m6 原本版本的卡片
…請你 audit 還沒放到我們網站上的 m6 剩餘卡片」。

### 【零】為什麼會漏：發售前抓的

站上 M6 的 `scrapedAt` 是 **2026-07-31**，而 M6 的 `releaseDate` 是 2026-08-07 ——
官方在發售前只公開到 076 號，高編號（異畫／SAR／AR／UR）是發售後才陸續上官網。
這與 M5 那次「玩家報的卡沒實裝，其實是印刷版本漏收」是同一個坑。
⇒ **通則：發售前抓的卡包，發售後要回頭再對一次官方張數。**

### 【一】⚠ WebFetch 的摘要說「73 張」，是錯的

用 WebFetch 問官方 list 頁，小模型回答「Total Results: 73」——剛好等於我們已有的 73 張，
非常有說服力。實際用瀏覽器頁內 same-origin fetch 讀 DOM，頁面白紙黑字寫 **「搜尋結果 110 個 第 1 頁 / 共 6 頁」**。
⇒ 記憶裡那條「子代理／摘要的結論一律自行查證」再次成立；這次差點少收 37 張。

### 【二】官方清單（瀏覽器 same-origin fetch 六頁）

110 張，detail id `19551`～`19667`，其中：

- `19551`~`19623`（73 張）＝ 001/076 ～ 073/076，站上早就有，連號無缺。
- `19624`~`19630` **不屬於 M6**（19624~19626 是官方 M-P 209~211 —— 正是 v6.093 自造 id 撞號、
  v6.328/329 才拆兩版修好的那三張）。
- `19631`~`19667`（37 張連號）＝ **074/076 之後的 077/076 ～ 113/076**，就是這一版要補的。

⚠ 站上的 074~076 是「兩張合一場地卡」的右半（自造 id `19621-1`/`19622-1`/`19623-1`），
不是官方編號，所以官方 73 張 ＋ 自造 3 張 ＝ 站上 76 筆。補完之後 M6 共 **113 筆**。

### 【三】精配來源卡：不是靠名字，是靠**卡面雜湊**

IRON_RULES 明訓「新編號/同名絕不可用同名第一張 clone」。作法：

1. 逐張讀官方 detail，抽出 `.collectorNumber` / `.alpha` / `.mainInfomation`（HP、屬性 icon 檔名）/
   `.skillName`（剝零寬字元）/ `.skillDamage` / `.skillCost` icon / `.skillEffect` / `.weakpoint` / `.resist` / `.escape`。
2. 把「特性 ＋ 招式 ＋ 規則文字」組成正規化字串取 **SHA-256**，與站上同名卡逐張比對。
3. 36/37 唯一命中；5 張出現「M6 本體」與「M-P-J promo」兩個**完全相同**的候選（不是歧義，取 M6）。
   剩下 `098/076 寶可夢捕捉器` 全站有 8 張卡面完全相同的印刷、且 M6 本體沒有它 ⇒ 取 id 最小者。
4. 校驗和對帳：新 id 總和 727013、編號總和 3515（＝77+…+113），兩邊都對得上。

⚠ 三張特殊能量（104~106）與 `098 寶可夢捕捉器` 的來源**不在 M6**，分別來自 M3／M4／MC ——
這是 M6 對舊卡的重印，不是漏收。

### 【四】clone 只動允許的欄位（有零回歸斷言）

`id` / `setCode` / `collectorNumber` / `regulationMark` / `imageUrl` / `sourceUrl` / `scrapedAt` / `illustrator`，
其餘欄位序列化後必須與來源**逐字相同**，否則整支中止。
`imageUrl` 依**新 id** 重算（`tw{id 補零8位}.png`），不是抄來源那張。
`regulationMark` 一律取官方該印刷的 `.alpha`（實查 37 張全部是 **J**）。
⚠ 三張特殊能量的官方頁面**沒有繪師欄**，所以 clone 時把 `illustrator` 刪掉，不從來源印刷抄一個假的。

### 【五】⚠ 差點漏掉：`static/card-set-map.json`

`test-card-db-integrity` 的「card-set-map.json 與實際卡片檔零落差」當場翻紅 ——
新卡不只要進 `M6.json` 與 `index.json`，還要進 `card-set-map.json`（漏了會讓那些卡載不進來）。
補法是**手術式插入**（錨在 `"19623": "M6", ` 後面），不重生整檔。
`index.json` 同樣只改 M6 那一筆的 `cardCount`/`count`/`supertypeCounts`，並斷言其他卡包逐字未動。

### 【六】新守衛 `scripts/test-v6335-m6-high-number-printings.mjs`（22 條）

- 【A】資料面：編號連號、id 落在 19631~19667、全站唯一、imageUrl 依自己的 id、
  card-set-map／index.json 同步（A4 就是那條當場抓到漏改的）。
- 【B】卡面逐字等於某張既有同名卡 ＋ **突變測試**（改掉一張的招式傷害 ⇒ B1 必須紅）。
- 【C】⭐ 行為端：真的跑 `applyAction('ATTACK')`，089/076（新）與 009/076（來源）打出來的傷害必須相同。
  ⚠ 第一版用 60HP 的鬼斯當防守者，兩邊都回 `KO` ⇒ 那是「反正都相等」的恆真式（安慰劑）。
  改用 340HP 的 009/076 當防守者後才真的在比數字（160 vs 160），正對照用同一張卡的另一招（60 ≠ 160）。
- 【D】牌組合法性：37 張都可組牌；**M6a 仍必須被擋**（正對照，證明 D2 不是恆真式）。
- 【E】HEAD-FAIL：走中央 `scripts/lib/base-blob.mjs`，對 BASE 斷言「一張高編號都沒有」
  ＋ 哨兵「001/076 在 BASE 找得到」（哨兵紅 ⇒ 代表 BASE blob 根本沒讀到，整批紅不算數）。

### 【七】部署注意

⚠ 動到 `static/cards` ⇒ **必須跑 `update-tournament.bat`**（IRON_RULES B-6），
否則錦標賽與線上對戰的伺服器端不認得這 37 張。
...
改動檔：changelog 三檔／**新增** `scripts/lib/changelog-policy.mjs`／
守衛五支（`test-v6264`／`test-changelog-size-and-archive`／`test-v6223`／`test-v6244`／`test-v6246`／`test-v6248`）／
`src/lib/changelog-lazy.ts`／`src/service-worker.ts`（過期註解）／
`src/lib/version.ts`／`oracle-admin/admin.html`／`scripts/test-v6272-*`／本檔。
⚠ **首頁 changelog 不加條目** —— 這是「更新記錄自己」的 meta 題材，依撰寫規範②整則不放。

### 【零】為什麼現在做：實測的成長率

`changelog-bodies.html` 是 v6.264 把「展開才看得到的內文」搬出去之後的落腳處。
首頁不再長大了，但**成長換到 bodies 繼續**：
...
最近 10 個有動 changelog 的版本平均 **+117 bytes/版**；剩餘 3,843 bytes ⇒ **約 33 版就撞 40KB**。
以最近的出版節奏（8 天 25 版）＝ **約 10 天後會直接擋住 deploy**。

⚠ 我上一輪跟站長說「不急」是**低估**，已當面更正。
⭐ 通則：**講「還很久」之前先把成長率量出來**，不要用直覺估。

### 【一】為什麼只能這樣解

bodies 裝的是「首頁第 13～50 則」的內文。要讓它變小，只能**減少首頁保留的則數**。
其他路都不通：
- 把 40KB 上限調高 ＝ 放寬判準讓守衛變綠（違反紀律 F）。
- 把內嵌的 12 則再減少 ⇒ 懶載入變多 ⇒ **bodies 反而更大**。
- 拆成 bodies-1／bodies-2 ⇒ 要改 SW、fetch、全部守衛，而且只是把問題延後。

**站長裁定：保留 35 則。** 一次把第 36～50 則（v6.258 ~ v6.239，共 15 則）搬進封存頁。

實測結果：
- `changelog.html`  32,056 → **26,312** bytes（50 → 35 則）
- `changelog-bodies.html` 37,117 → **22,489** bytes（−39.4%，38 → 23 則）⇒ 餘裕 18KB ≒ **150+ 版**
- `changelog-archive.html` 356 → 371 則（歷史一則都沒少）

### 【二】守衛：`test-v6264` 新增第四種合法情況 **F0c 批次縮減**

這一版不是常規的「一進一出」，F1（首頁只多最新一則）／F2（封存只多一則）本來就會紅。
⚠ 但它的**意圖**（搬運必須無損）完全沒變 ⇒ 依 IRON_RULES **Rule 40** 第 3 類處理：
**把斷言改到「意圖級」，而且要比原本更嚴**，不是放寬。

F0c 三條（除了掉出去的那 15 則，三個檔全部**逐位元相同**）：
- **F0c-1** 首頁保留的 35 則逐位元等於 BASE 的前 35 則；head 也逐位元；
  掉出去的必須是**最舊的連續那一段**（`deepStrictEqual(droppedVers, wantDropped)`）。
- **F0c-2** 封存頁恰好多 15 則、順序正確、每一則 ＝ BASE 首頁標題（脫 `data-ver`）＋ BASE bodies 內文；
  其餘 356 則逐位元不變。
- **F0c-3** bodies 只少了那 15 則、其餘逐字不變、而且**必須真的變小**（否則整件事白做）。

⚠ 只要多動一個位元組就落回 else 分支走 F1~F3（那時一定紅）⇒ 這是**收緊不是放寬**。

**突變測試 6 個、全部翻紅在指定位置**：
① 搬進封存頁的某則改一個字 → F0c-2 ② bodies 少移除一則 → A4 ＋ F0c-3
③ 首頁保留的第 20 則改一個字 → F0c-1 ④ 首頁少一則 → 0-1／A1／A2
⑤ bodies 沒變小 → F0c-3 ⑥ 首頁最後兩則對調 → F0c-1
⚠ 誠實標記：F0c-1 的 `deepStrictEqual(droppedVers, wantDropped)` 目前是**冗餘的第二道**
（各種「挑著刪」的形狀都會先被 0-1／A2／A4 抓到），它便宜且無害，但不宣稱有獨佔偵測面。

### 【三】另加一條「提前預警」：`test-changelog-size-and-archive` ①b

`assert slack > 20 * 120` —— bodies 的剩餘空間必須夠再撐 20 版。
⇒ **下次逼近時會提前紅，而不是等到撞牆才發現**。這是這一輪真正的結構性收穫：
原本的守衛只有「< 40KB」這個**硬牆**，撞到才知道；現在多了一道**護欄**。

### 【三之二】⭐⭐⭐ 順手抓到的 Rule 38 實例：「50」被抄在**三支**守衛裡

改政策時 `test-v6223-layout-fixes.mjs`（**版面守衛**）莫名其妙紅在
`check(... === 50, 'changelog 條目數 = 50')` —— 改的人完全不會想到要去看版面守衛。
另外兩份在 `test-v6264` 與 `test-changelog-size-and-archive`。

⇒ 新增 **`scripts/lib/changelog-policy.mjs`** 當單一來源
（`N_HOME` / `N_INLINE` / `MAX_KB` / `BODIES_GROWTH_PER_VERSION` / `BODIES_MIN_SLACK_VERSIONS`），
五支守衛全部改成 import。
⚠ 第一版連 `MAX_KB` 都做成了**死 export**（宣告了卻沒人用，40 仍寫死在四處）——
審查者抓到，已把那四處也改成 import。**「抽出單一來源」如果沒有把消費端接上去，就只是多一份複本。**
⭐ 這正是 IRON_RULES **Rule 38** 的實例：**判準有 N 份複本時，改政策就會有 N-1 支誤紅，
而且任何一份被改壞都測不出來。**

### 【三之三】⭐⭐⭐ 更大的一族：**斷言「某一則公告必須在首頁」** —— 遲早必然過期

把首頁縮到 35 則之後，另外**三支**守衛紅在完全無關的地方：
- `test-v6244` ⑭「首頁 changelog 少了 v6.244」
- `test-v6246`「首頁找不到 v6.246 那一則」＋「首頁則數不是 50」
- `test-v6248` ⑩「首頁則數不是 50」

⭐⭐ **首頁 changelog 是滾動視窗**（固定 N 則、舊的擠進封存頁）
⇒ 任何「某一則永遠在首頁」的斷言都是**時間炸彈**，而且引爆時紅在與行為無關的地方。
⚠ 諷刺的是 `test-v6246` 的註解自己就寫過「原本寫死首頁第一則必須是 v6.246，下一版就必紅」，
v6.247 修掉了「第一則」卻**留下了「在首頁」**——只修了一半。

⇒ 判準改到**意圖級**：
- 「那則公告存在且文字正確」→ 搜 **changelog.html ∪ changelog-bodies.html ∪ changelog-archive.html**
  （封存頁是逐位元保留的，F0c-2 有證明 ⇒ 與原意等價，而且不會再過期）
- 「則數」→ 讀 `scripts/lib/changelog-policy.mjs` 的 `N_HOME`
- ⚠ `test-v6246` 的「恰一則展開」必須只數**首頁**（封存頁本來就有 71 則 `<details open>`）

### 【四】⚠ 我這一輪自己踩的坑
`test-v6264` 的 `BASE_SHA` 忘了前移（還指著 v6.330），於是它拿 v6.330 當基準
⇒ 封存頁差了 v6.331 搬進去的那一則 ⇒ `_clBatchShrink` 的則數條件對不上、
一路走到 else 分支紅在 F1/F2，而我還去 debug 了半天判斷式。
⭐ **bump 四配套（version.ts／admin.html／test-v6272 PREV_SHA＋PREV_ALLOWED／test-v6264 BASE_SHA）
要在動守衛之前先做完**，否則會拿舊基準除錯。
...
改動檔：`static/cards/M6.json`（**刪 3 筆停用卡**，79 → 76）／`static/cards/M-P-J.json`（+2）／
`static/cards/M-P-I.json`（+1）／`static/cards/index.json`（手術式：只動這三包，**全站總數 4938 不變**）／
`static/card-set-map.json`／`src/lib/cards/visibility.ts`（移除 3 筆停用登記）／
`src/lib/game/effects.ts`（+1 行 regPre）／`scripts/test-card-db-integrity.mjs`（圖檔例外表移除 3 筆）／
`scripts/test-v6241-*`（卡包張數對帳）／`scripts/test-v6328-*`（改寫成「歸還完成」版）／
**新增** `scripts/test-v6329-official-id-returned.mjs`／`package.json`（chain 645 → **646** 步）／
首頁 changelog 三步／`src/lib/version.ts`／`oracle-admin/admin.html`／
`scripts/test-v6272-*`（PREV_SHA）／`scripts/test-v6264-*`（BASE_SHA，本版**有**動首頁 ⇒ 走完整 F 比對）／本檔。

### 【零】這一版是 v6.328 的下半場，三件事**必須同版**

v6.328 只做換號（右半 → `19621-1`／`19622-1`／`19623-1`），舊 id 的卡片資料留著當停用卡。
本版把 id 讓出去，三件事一起做：

1. 刪掉 `M6.json` 那三筆停用的舊右半（79 → 76）
2. 移除 `visibility.ts` 的那三筆停用登記
   ⇒ `RETIRED_DUP_TO_TW_ID`（由 `HIDDEN_FROM_PLAYERS` 推導）與 `mergeTwoCardStadiumEntries`
     的舊 id fail-safe（由 `RETIRED_DUP_TO_TW_ID` 推導）**自動失效** —— 兩處都不是寫死清單，不必手動拆
3. 才把 19624/19625/19626 加進 `M-P-J`／`M-P-I`

⚠⚠ **順序反了就是 v6.328 第一版設計的那個 bug**（遷移表還在就先加官方卡 ⇒ 玩家放的新卡
會被 `migrateCardId` 靜默換成傳說場地卡的右半，`engine.ts` 的 createGame 咽喉點也會）。
`test-v6328` 的 **B1** 現在反過來鎖這件事：那三個 id 必須「不是競技場／不在停用表／不在配對表／
`migrateCardId(id) === id`」。

### 【一】張數對帳（總數刻意不變，方便驗證沒有多生或漏掉）

| 卡包 | v6.328 | v6.329 | 說明 |
|---|---|---|---|
| M6 | 79 | **76** | 刪掉 3 筆停用的舊右半（Trainer 17 → 14） |
| M-P-J | 101 | **103** | ＋膽小蟲 209、超級米立龍ex 210（Pokemon 75 → 77） |
| M-P-I | 50 | **51** | ＋麻麻小魚 211（Pokemon 35 → 36） |
| **全站** | 4938 | **4938** | 不變 |
| 停用卡 | 5 | **2** | 回到 v6.194 的兩張港版重複卡 |
| 對玩家可見 | 4933 | **4936** | ＋3（就是新收的這三張） |

### 【二】卡面查證（逐欄取自台灣官方卡牌檢索 detail 頁的原始 DOM）

⚠ detail 頁是 client-rendered，`web_fetch` 回空白 ⇒ 用瀏覽器頁內 same-origin `fetch` ＋ `DOMParser`。
守衛 `test-v6329` 的 `OFFICIAL` 表就是這份查證的落地，**動它之前要先開官方頁核對，不要照著實作改**。

- **19624 膽小蟲**（J，209/M-P）基礎／HP 70／【草】／特性「懦弱」：
  若對手的場上有「寶可夢【ex】」，則這隻寶可夢【撤退】所需的能量全部消除。／
  衝撞【草】10／弱點【火】×2／抵抗力 --／撤退【無】×3／No.767 疾行寶可夢／繪師 Tonji Matsuno
  → 與既有印刷 19529（148/M-P）、19558（M6 008/076）**逐欄相同**，只有繪師不同。
- **19625 超級米立龍ex**（J，210/M-P）**基礎**／HP 260／【水】／
  三貫頭擊【水】【水】【水】`150×`：擲3次硬幣，造成正面出現的次數×150點傷害。／
  弱點【雷】×2／抵抗力 --／撤退【無】×1／No.978（官方無分類名 ⇒ 不寫 `species`）／繪師 Keisuke Azuma
  - ⭐ **stage 是 `Basic` 不是 Stage2**：官方 h1 的 `evolveMarker` 逐字「基礎」，進化欄只有自己一項。
    這個對照用 19550 超級艾路雷朵ex（h1「2階進化」，站內 stage=Stage2）與 19627 探探鼠（h1「基礎」）
    **雙向校準**過，不是憑印象。米立龍本來就是單階段寶可夢。
    卡池裡本來就有 40 張 `stage=Basic` 的 Mega ex，不是新形狀。
- **19626 麻麻小魚**（I，211/M-P）基礎／HP 40／【雷】／紋絲不動【無】（無傷害）：
  將這隻寶可夢恢復「10」HP。／弱點【格鬥】×2／抵抗力 --／**撤退 --**（⇒ 不寫 `retreatCost`，
  與既有印刷 14707 一致）／No.602 電魚寶可夢／繪師 Saboteri

### 【三】效果實裝：只有一行

- **超級米立龍ex｜三貫頭擊** ＝ 既有中央 helper **`coinHeadsMultiplyPre(3, 150, '三貫頭擊')`**
  （`effects.ts`；同家族站內已有 56 種去重、105 張含重印，例：卡比獸ex 翻身壓制 `coinHeadsMultiplyPre(3, 120)`）。
  ⭐ **不另寫一份擲幣邏輯**。
- **膽小蟲｜懦弱＋衝撞**、**麻麻小魚｜紋絲不動** 是既有卡名的另一印刷，
  reg key 是**卡名**（`'麻麻小魚|紋絲不動'` 在 `effects.ts:12323`；`懦弱` 走 `m6_wave8` 的特性名索引）
  ⇒ **卡片進 DB 就自動生效，不必再登錄**（v6.191 已立的慣例）。
  守衛 C1 逐字比對招式／特性集與效果，確認「另一印刷」這個前提真的成立。
- **超級進化寶可夢【ex】** 不必另外登錄：中央述詞 `isMegaExCard`（`selection-filter.ts:92`）
  ＝`subtype === 'ex'` ∧ 卡名以「超級」開頭 ⇒ KO 給 3 張獎賞、玳蘿、飯匙蛇｜激動力量、
  deck-search `'MegaEx'` filter 全部自動涵蓋。

### 【四】守衛

**`test-v6329-official-id-returned.mjs`**
A1 三張卡逐欄與官方卡面相符（含「抵抗力 --／撤退 -- ⇒ 欄位缺席」「無分類名 ⇒ 不寫 species」）／
A2 三包張數與 supertypeCounts 與 index.json、card-set-map 對帳／
**B1 走完整 `applyAction` ATTACK 流程**（不是只餵 `ATTACK_PRE`）：全正面 450／一正兩反 150／
**全反面 0**（⭐ 若 `damage: "150×"` 被當成固定 150，這條會是 150 ⇒ 專門用來抓字串誤解析）／
B2 招式有註冊／**B3 對【水】弱點 900**（證明走的是中央傷害公式，弱點／抵抗力／減傷／免疫才會跟著生效）／
B4 `isMegaExCard` 認得它、且不會誤判膽小蟲／C1 另外兩張的招式特性集與既有印刷逐字相同。

**`test-v6328-*` 改寫**：A 區（右半 id 結構性不變量）不變；
B1 從「舊右半必須是停用卡」反轉成「**必須已完全退出競技場邏輯**」；
B2 的豁免清單在本版之後應為 0 筆；B3 加「M6 不得再有停用卡」；
C 區移除過渡期的舊 id fail-safe 斷言，改成 **C4：那三個官方 id 不可以再被合併回競技場左半**。

### 【五】⛔⛔ 對抗性審查（Opus 5）抓到的真回歸：舊牌組會被**靜默**改壞

我原本宣稱「舊牌組裡若仍有舊右半 id，牌組驗證的『左右張數相等』會擋下並提示」。
**那是錯的。** 審查者實跑、我自己複驗：

```
v6.329 修正前：19621(傳說的海溝)×1 , 19621-1(傳說的海溝)×1 , 19624(膽小蟲)×2
               總張數 60、legal: true、issues: []          ← 零提示
v6.328  對照：19621(傳說的海溝)×2 , 19621-1(傳說的海溝)×2   ← 正確
```

**成因**：`migrateCardId` 不再把 19624 換成 19621-1 之後，接著跑的
`splitTwoCardStadiumEntries` 找不到右半 entry（`d.entries.some(x => x.cardId === rightId)` 為 false），
就把玩家**原本已經拆好**的左半 2 張**再拆一次**成 1+1 —— 左右張數仍然相等 ⇒ 驗證通過。
玩家一套傳說場地卡憑空消失、多出兩張膽小蟲。
影響三條路徑：`storage.ts:37` 本機牌組、`cloud.ts:41` 雲端牌組、`deck-posts/+page.svelte:333` 公布欄匯入。

⚠ 而且「等一陣子讓玩家的牌組被載入改寫」**不能根治** —— 雲端牌組只有玩家**主動存檔**才寫回。

### 【六】⛔⛔ 第一版修法（時間戳閘門）**又被第二輪審查打穿**，最終改用「牌組形狀」

我第一版寫的是「只對 `updatedAt` 早於 v6.329 發布的牌組套用」。**第二輪對抗性審查實跑證明它有三條破口**：

| 破口 | 情形 | 後果 |
|---|---|---|
| **假陰 A** | 公布欄匯入 `deck-posts/+page.svelte:333`：`{ ...newDeck(名稱), entries: 伺服器上的舊資料 }` | **新時間戳 ＋ 舊 entries** ⇒ 判成「新牌組」不修 ⇒ 靜默改壞原封不動重現。而且公布欄投稿是**伺服器永久資料**，不會因玩家存檔而自癒 |
| **假陰 B** | 匯入 JSON 檔 `decks/+page.svelte:917` 同一形狀 | 同上 |
| **假陽 C** | 官網代碼匯入 `decks/+page.svelte:1177` 寫的是 `updatedAt: Date.now()`（**數字**） | `String(178…) < '2026…'` 恆真 ⇒ 玩家剛匯入的**超級米立龍ex 被換成競技場右半** |

⭐⭐⭐ **通則：時間戳描述的是「牌組什麼時候被存」，不是「entries 是哪個年代的格式」——
這兩件事在匯入路徑上會脫鉤。** 用「存檔時間」推論「資料格式」是這一版最貴的一課。
（我在第一版的內部文件裡還寫了「官網代碼匯入走 `newDeck()`」——那是**事實錯誤**，它根本沒走。）

**最終修法（站長裁定）：判準改成牌組形狀** —— 換舊右半的條件是
**「這副牌同時含對應的左半，而且還沒有新右半」**（`repairLegacyStadiumRightIds`，
掛在 `migrateDeck` 的 `splitTwoCardStadiumEntries` **之前**；左半由 `TWO_CARD_STADIUM_SPLIT`
反查，不寫第三份清單）。

⭐ **為什麼形狀優於時間：失效方向不同。**
形狀判錯 ⇒ 左右張數不等 ⇒ `validateDeck` **明確提示**；時間戳判錯 ⇒ **靜默改壞、零提示**。

七種形狀實跑（守衛 D1~D8 鎖住，每個形狀都跑過 7 種 `updatedAt`：舊 ISO／新 ISO／數字／缺席／空字串／非 ISO／帶時區）：

| 形狀 | 結果 |
|---|---|
| 舊格式 左2 ＋ 舊右2（含公布欄／JSON 匯入的新時間戳版） | → 左2 ＋ **新右2**（修好） |
| 新牌組 左2 ＋ 新右2 ＋ 膽小蟲2 | → 原樣，**膽小蟲保留** |
| 只有官方那三張卡（含 `updatedAt` 是數字） | → 原樣保留 |
| 拆卡前 只有左半 4 張 | → 左2 ＋ 新右2 |

⚠ **已知取捨**：「只有右半、沒有左半」的壞資料不再自動換
（v6.328 會換完再由驗證提示；v6.329 起 19624 本來就是一張合法的卡）。

⚠⚠ **為什麼不能放進 `migrateCardId`**：那支是無條件、永久的，而且 `engine.ts:2298` 的
createGame 咽喉點也會呼叫 ⇒ 會把玩家新放的官方卡靜默吃掉（＝ v6.328 第一版設計的那個 bug）。

**順手修的既有 bug**：`decks/+page.svelte:1177` 的 `updatedAt: Date.now()` 改成 ISO 字串 ——
`Deck.updatedAt` 的型別是 `string`，數字會讓同檔雲端合併的 `d.updatedAt > existing.updatedAt`
永遠比不出正確結果（守衛 D8 鎖住）。

⚠ **引擎端不套這條**（`createGame` 拿到的是 deckEntries、沒有牌組物件）：
部署當下**進行中的對局**與**部署前拍下的錦標賽報名快照**仍有殘留風險，
處置同 v6.328 ——**挑離峰、沒有錦標賽進行中時部署**。

⚠ **已歸檔的對戰回放不經任何遷移**（存的是盤面快照 ＋ cardId）：
v6.093~v6.328 期間用過傳說競技場的回放，右半會**永久**渲染成膽小蟲／超級米立龍ex／麻麻小魚。
不會壞掉、不會 throw（id 仍在卡池），只是圖與名字錯。這是 id 語義變更無法回頭的部分。

### 【七】審查同時抓到的其他項（已一併修）

- ⛔ **`test-v6272` ⑩ 的 `PREV_ALLOWED` 只列了 `version.ts`**，本版動了 12 個 `src/`＋`static/` 檔。
  ⚠⚠ **沙盒沒有歷史 commit 會走 shallow-skip ⇒ 那條在沙盒是假綠**，正式 repo 一定會紅。
  ⭐ 通則：**shallow-skip 會剛好蓋掉唯一會抓到你的那條** —— 沙盒全綠在動到 `src/`／`static/` 的版本不算數。
- `scripts/test-v6101-*.mjs` 檔頭殘留 v6.328 漏改的混寫（純註解）。
- 首頁 changelog 摘要 88 字 → 縮到規格內的 40~80 字。
- ⚠ 待辦：`static/changelog-bodies.html` 已 37KB／上限 40KB，再搬 2~3 版就會爆，下次要規劃搬運策略。

### 【六】⚠ 部署順序（server 先）

本版改了卡表也改了 `effects.ts` ⇒ **`update-tournament.bat` 先跑**
（它會 `git fetch` ＋ `reset --hard origin/main`，重建 `server-engine.cjs` 與 `tournament-pool.json`），
再跑 `redeploy-oracle.bat`。順序反了的話，client 送 `19625` 上去而錦標賽伺服器的卡池還沒有這張卡。
⚠ `update-tournament.bat` 會 pm2 restart（斷線 2~5 秒）⇒ 挑離峰、沒有錦標賽進行中時跑。

## v6.328 「傳說」競技場右半 id 換號 —— 把被本站佔用的三個**官方 id** 讓出來（第一版：只換號、不補卡）

BASE `c2cfb8e8501cca7784902633b6a20d602adf9c35`（v6.327，遠端 main；`git ls-remote` 確認；一律以 BASE blob 為準）。
改動檔：`static/cards/M6.json`（**純新增** 3 筆）／`static/cards/index.json`（手術式：只動 M6）／
`static/card-set-map.json`／`src/lib/cards/visibility.ts`／`src/lib/decks/validation.ts`／
`src/lib/game/effects/_shared.ts`／`src/lib/decks/cardIdMigration.ts`／`src/routes/decks/+page.svelte`（註解）／
**新增** `scripts/test-v6328-two-card-stadium-id-renumber.mjs`／`package.json`（chain 644 → **645** 步）／
`scripts/test-card-db-integrity.mjs`・`test-v6194`・`test-v6232`・`test-v6241`（張數對帳數字）／
`src/lib/version.ts`／`oracle-admin/admin.html`（SITE_VERSION_HINT，維持 LF）／
`scripts/test-v6272-*`（PREV_SHA）／`scripts/test-v6264-*`（BASE_SHA）／本檔。
**首頁 changelog 不放**（玩家完全無感：選不到的卡換了內部編號、舊牌組自動換號、對玩家的可見張數 4933 一張都沒變）。

### 【零】⛔ 事故：v6.093 造右半 id 時，捏到了官方的 id

v6.093 把「傳說」競技場（海溝／山頂／熔岩洞）拆成左右兩張獨立卡片時，右半需要新的 `cardId`，
當時**自己捏了 `19624` / `19625` / `19626`**。

那三個號碼**其實是台灣官方 M-P 209~211**：

| 官方 id | 實際是哪張卡 | 標記 | 編號 |
|---|---|---|---|
| 19624 | 膽小蟲 | J | 209/M-P |
| 19625 | **超級米立龍ex** | J | 210/M-P |
| 19626 | 麻麻小魚 | I | 211/M-P |

證據有三層，互相獨立：
1. repo 自己的 `scripts/data/official-set-manifest.json` 的 `sets['M-P'].ids` **本來就有這三個 id**
   （`sets['M6'].ids` 只有 19621/19622/19623）。
2. 站內 M-P 的 collectorNumber **正好缺 209/210/211**（208 = id 19550、212 = id 19627）。
3. 直接開官方卡牌檢索 `detail/19624`、`/19625`、`/19626` 逐欄取原始 DOM 核對（見下方【三】）。

後果：**那三張官方卡永遠補不進來**（補了會撞 `test-card-db-integrity` 的「cardId 全站唯一」），
其中 **超級米立龍ex 是站內完全沒有、但標準賽可用的卡**（J 標，260 HP，三貫頭擊 150×）。

### 【一】⭐⭐⭐ 為什麼一個多月沒有人發現 —— 兩層安慰劑

**① 守衛只驗「id 在不在」，不驗「是不是同一張卡」。**
`scripts/test-official-set-completeness.mjs:79` 的判準是：

```js
if (OURS.has(String(id))) continue;   // 官方快照的這個 id 我方有收 → 過
```

19624 一直在卡庫（是傳說的海溝右半）⇒ **一路綠燈**。
本版的 **B2** 補的就是這個維度：*官方快照說某 id 屬於某卡包 → 我方若也收了，`setCode` 必須對得起來*。
在 BASE 上跑 B2 會直接把三筆逐字印出來。

**② 本檔自己寫過一句錯誤結論，把線索封死了。**
`docs/changelog-internal.md`（v6.191 那一節）原本寫著「官方**同時掛在 M-P 與 M6** 底下 ⇒ 不是缺卡」——
**那是錯的**，官方快照從來沒有把它們掛在 M6。本版已就地訂正（保留原句加刪除線，附訂正說明）。
⭐ 通則：**內部紀錄裡的「已排除」結論，日後要當成假設重驗一次**，它會變成下一個人的盲點。

### 【二】修法：右半改成「左半 id ＋ `-1`」，舊 id 先停用、下一版再歸還

新右半 id：`19621-1` / `19622-1` / `19623-1`。
⭐ **官方 id 一律是純數字**，所以「帶非數字字元」這種形式**結構上不可能**再被官方佔用
（守衛 A1 從**產品端的配對表**推導右半集合再驗，不在測試檔自己抄清單）。

⚠⚠ **舊 id 的卡片資料刻意留著**，只是登記進 `$lib/cards/visibility` 的 `HIDDEN_FROM_PLAYERS`
（沿用 v6.194 既有機制，不另造新表）：

- **部署當下進行中的對局**、與**部署前拍下的錦標賽報名快照**裡仍帶著舊 id ——
  資料留著就仍然解析成傳說場地卡，**零破壞**；拿掉的話那些盤面上的競技場會變成「查不到的卡」。
- 玩家在牌組編輯器**選不到**舊 id（`/cards` 的卡包摘要也會扣掉）。
- 舊牌組載入時由 `migrateCardId`（走本表推導出的 `RETIRED_DUP_TO_TW_ID`）換成新 id。
- 對玩家的可見張數：M6 檔案 76 → **79**、全站 4935 → **4938**，但停用卡從 2 張變 5 張
  ⇒ **對玩家仍是 4933 張，一張都沒變。**

`mergeTwoCardStadiumEntries`（送去台灣官網牌組工具前把右半併回官方那張）新增舊 id 的 fail-safe，
且**不寫死清單**——從 `RETIRED_DUP_TO_TW_ID` 推導，v6.329 把那三筆刪掉之後這段自動失效。

### 【三】⛔⛔⛔ 第一版設計被對抗性審查推翻（Opus 5）—— 記下來，這是本版最重要的教訓

**我原本要一版做完**：換號 ＋ 同版把三張官方卡加進 `M-P-J.json` / `M-P-I.json`，
並加一張永久的 `LEGACY_STADIUM_RIGHT_ID`（19624→19621-1 …）掛在 `migrateCardId` 最前面。

審查者實跑證明**那個設計自相矛盾、招牌成果實際效果是 0**：

```
19625 -> 19622-1
牌組載入後: [{"cardId":"19622-1","count":2}]           ← 玩家放的超級米立龍ex 被吃掉
createGame 後 P1 牌堆: {"18519":56,"19622-1":4}         ← 開局也一樣（engine.ts:2298 也呼叫 migrateCardId）
```

`migrateCardId` 是**永久、無條件**的，掛在 `storage.ts`／`cloud.ts`／**`engine.ts` 的 createGame 咽喉點**／
`deck-posts`。只要那張遷移表還在，那三個 id 就**永遠不能同時是官方卡**。

⭐⭐⭐ **通則：「同一個識別碼在新舊語義之間切換」時，遷移表與新語義不可以同版共存。**
我自己在註解裡寫了「光看 id 無法分辨舊右半與新膽小蟲」，卻只把這個推論套用在
「部署當下的對局／快照」，**沒有套用到未來所有牌組**——推論只做了一半是最危險的形態。

⇒ 站長裁定拆兩版。而且拆完之後**連我原本說「沒辦法救」的部署當下風險也一起消失了**
（因為 v6.328 的 19624 仍然是傳說的海溝）。

### 【四】🔨 v6.329 待辦（下一版接著做）—— 三件事必須**同版**

1. 刪掉 `static/cards/M6.json` 裡那三筆停用的右半（79 → 76）。
2. 移除 `visibility.ts` 的那三筆（`RETIRED_DUP_TO_TW_ID` 與 merge fail-safe 隨之自動失效）。
3. 才可以把 19624/19625/19626 讓給官方那三張卡（`M-P-J` +2、`M-P-I` +1），
   並登錄 `regPre('超級米立龍ex|三貫頭擊', coinHeadsMultiplyPre(3, 150, '三貫頭擊'))`
   （另兩張是既有卡名的另一印刷，reg key 是卡名 ⇒ 自動生效）。

⚠⚠ **順序反了**（先加官方卡、遷移表卻還在）＝ 重演【三】那個 bug。
本版守衛 **D1** 就是交接鎖：只要有人在遷移表還在的情況下把那三個 id 換成別的卡，D1 立刻紅。

⚠ **殘留風險（站長已知悉）**：v6.328 到 v6.329 之間**從未被載入過**的舊牌組，
裡面若仍有舊右半 id，在 v6.329 之後會變成官方那三張卡。
不過牌組驗證要求「左右張數相等」，所以是**會被明確擋下並提示**，不是靜默壞掉。

### 【五】卡面查證（v6.329 要用，先記在這裡；逐欄取自台灣官方卡牌檢索原始 DOM）

- **19624 膽小蟲**（J，209/M-P）基礎／HP 70／【草】／特性「懦弱」：
  若對手的場上有「寶可夢【ex】」，則這隻寶可夢【撤退】所需的能量全部消除。／
  衝撞【草】10／弱點【火】×2／抵抗力 --／撤退【無】×3／No.767 疾行寶可夢／繪師 Tonji Matsuno
  → 與既有印刷 19529（148/M-P）、19558（M6 008/076）**逐欄相同**，只有繪師不同。
- **19625 超級米立龍ex**（J，210/M-P）**基礎**（官方 h1 `evolveMarker` 逐字「基礎」，進化欄只有自己；
  已用 19550「2階進化」與 19627「基礎」雙向校準過這個對照）／HP 260／【水】／
  三貫頭擊【水】【水】【水】`150×`：擲3次硬幣，造成正面出現的次數×150點傷害。／
  弱點【雷】×2／抵抗力 --／撤退【無】×1／No.978（官方無分類名 ⇒ 不寫 `species`）／繪師 Keisuke Azuma
  → `subtype: 'ex'` ＋ 卡名以「超級」開頭 ⇒ 中央述詞 `isMegaExCard` 自動認得（KO 給 3 張獎賞）。
- **19626 麻麻小魚**（I，211/M-P）基礎／HP 40／【雷】／紋絲不動【無】（無傷害）：
  將這隻寶可夢恢復「10」HP。／弱點【格鬥】×2／抵抗力 --／**撤退 --**（⇒ 不寫 `retreatCost`，
  與既有印刷 14707 一致）／No.602 電魚寶可夢／繪師 Saboteri

### 【六】守衛 `test-v6328-two-card-stadium-id-renumber.mjs`（10 條）

A1 右半 id 必須是本站自造格式（**清單從產品端 `TWO_CARD_STADIUM_PAIR_IDS` 推導**）／
A1b 反安慰劑（與 A1 呼叫**同一個** `isSiteMintedId`）／A2 左右同一張卡、配對雙向／
B1 舊右半四件事：資料還在＋登記停用＋指向對應新右半＋已離開配對表／
B2 官方快照 id 的卡包一致性（**BASE 上會紅**，唯一豁免是「排定歸還的停用右半」且豁免本身也被斷言）／
B2b 反安慰劑（同一個 `setCodeMatchesOfficial`）／B3 M6 張數與 index.json 對帳（不寫死魔術數字）／
C1~C4 行為端：舊牌組載入換號且張數守恆、拆卡前的更舊牌組仍攤開、連跑三次冪等、送官網合併（新舊右半都要）／
D1 交接鎖（見【四】）。

⚠ **第一版守衛的 A1 是安慰劑，是審查者用突變證明的**：當時 `RIGHT_IDS` 是**寫死在測試檔裡的三個字串**，
所以他「新增第 4 組配對、右半用既有純數字 id 19620」（＝完全重演 v6.093 的事故）之後，
**11 條全綠、全站沒有任何一支守衛翻紅**。已改成從產品端推導。
⭐ 這是「守衛安慰劑」的一種新型態：**判準是對的，但資料來源是測試檔自己抄的一份清單** ——
它只能守住「今天這三張」，守不住「同一類事故再發生一次」。

## v6.327 純守衛版：補回 v6.326 自己造成的偵測缺口 —— `damageTakenLastOppTurn` **唯一重置點**的行為＋結構斷言，並訂正 v6.326 的五處文件錯

BASE `427fdd3b3e894c84543b741ebea5586148ff567b`（v6.326，遠端 main；`git ls-remote` 確認；一律以 BASE blob 為準）。
改動檔：**新增** `scripts/test-v6327-damage-taken-clear.mjs`／`package.json`（chain 643 → **644** 步）／
`scripts/test-lib-strip-comments.mjs`（**只加 7-a 的定位註記**）／`src/lib/version.ts`／
`oracle-admin/admin.html`（SITE_VERSION_HINT，維持 LF）／`scripts/test-v6272-*`（PREV_SHA）／
`scripts/test-v6264-*`（BASE_SHA）／本檔。
⚠ **完全沒有動 `src/lib/game/engine.ts`** —— 那六行是**對的**，本版是補守衛不是修 bug。
零新依賴、`src/` 只動 version.ts、首頁 changelog 不放（純守衛）。

### 【零】⛔ 這是 v6.326 自己造成的偵測缺口（複驗全部成立）

v6.326 把 **`test-v6255` C7 的 `total` 5→3** 與 **`test-v6256` C1 的 `totalMentions` 6→4** 同時放寬，
而**這兩條是被同一段真碼撐著的**：`engine.ts` END_TURN 的清除區塊
...
剝註解後它恰好貢獻 **2 行／2 次提及** ⇒ **放寬的 slack 2 剛好被這一個真回歸吃光**。

**主證明（沙盒實跑，`applyAction(st, {type:'END_TURN'})` 真管線）**

| | `P1.active.damageTakenLastOppTurn`（END_TURN 後） |
|---|---|
| BASE `427fdd3b` | `undefined`（正確清除）／bench `[undefined, undefined]` |
| 刪掉那六行 | **`120`（永遠不清）** ／bench **`[50, 70]`** |

`src/lib/game/effects/_shared.ts:2771` 是**純累加** `(inst.damageTakenLastOppTurn ?? 0) + actual`
⇒ 那六行是**全站唯一的重置點** ⇒「超級赫拉克羅斯ex｜重裝角擊」（`100 + 上個對手回合受到的傷害`）
會跨回合無限累加 ⇒ **傷害灌水級 bug**。

**偵測力對照（實跑 rc；同一個突變，只換下限值）**

| 下限版本 | `total` | `totalMentions` | 結果 |
|---|---|---|---|
| **v6.325**（≥5／≥6） | 5→3 | 6→4 | v6255 **rc=1**、v6256 **rc=1** |
| **v6.326**（≥3／≥4） | 5→3 | 6→4 | **兩支都綠**（實得剛好等於新下限） |
| **v6.327**（本版，下限不動） | 5→3 | 6→4 | 新守衛 **rc=1**，紅在 B2／B4／B5／B6／C1 |

⭐ **v6.326 的 21 個突變為什麼一個都沒抓到**：MS1~MS9 **全部**是「把掃描器打殘」型
（實得 `0`／`3`／`5`／`23`），**沒有一個落在放寬的邊際帶**。

### 【一】⭐⭐⭐ 本輪最有價值的判準（已一併寫進 v6.326 段落的訂正裡）

> **放寬下限之前，先問「有沒有第二條下限是同一段代碼撐著的」；**
> **並且突變必須落在「放寬的邊際帶」——**
> **用「把掃描器打殘」型的突變，證明不了下限值本身的價值。**

⚠ 同型形狀在站內還有（**本版只列，不修**，見【六】）。

### 【二】修法：**不收回下限**，改補行為端＋結構端斷言（兩條都加）

**為什麼不把 `total`／`totalMentions` 收回 5／6**：那會退回「任何一次合法收斂都翻紅」的橡皮圖章
（v6.326 的分類是對的）。正確的做法是**把偵測力放在它該在的地方** ——
下限的職責是「掃描器沒壞」，不是「這一段功能碼還在」。

**為什麼行為端與結構端都要**（逐條給理由，不是求保險）：

| | 抓得到 | 抓不到 |
|---|---|---|
| **行為端**（B 段，真管線 `applyAction(END_TURN)`） | 清了但**沒寫回**（M3）／只清 active **漏 bench**（M2）／條件寫反（M4）／加死 gate（M6）——這些**結構斷言全都看不出來** | harness 本身失效（fixture 卡下架、`applyAction` 簽名變動）時會靜默不測 |
| **結構端**（C 段，END_TURN 區塊內的清除樣式） | 清除點被**刪除／改名／搬出 END_TURN 區塊**；行為 harness 失效時仍守得住 | 語義壞掉但字面還在（M2／M3 對它都是綠的） |

⇒ 實測 **M2 與 M3 在 C1 是綠的、只有 B 段紅**；**M7／M8 只有 C1 紅** —— 兩邊各自都有獨佔的偵測面，
不是重複。（承 v6.322 的教訓：「手工 decision 餵 case body 的守衛守不到『路徑可達否』⇒ 必加真管線斷言」。）

**新守衛 `scripts/test-v6327-damage-taken-clear.mjs`（11 條）**
- **A1** 卡面逐字：`重裝角擊` live H/I/J 恰 2 張印刷（`14322`／`18578`），`100+`、effect 逐字。
- **A2** ⭐⭐⭐ **前提**：中央寫入點是**純累加** ⇒「沒有第二個地方會歸零」（＋正對照＋反對照）。
  這一條讓「本檔存在的理由」自己也被守著：哪天寫入點改成覆寫，A2 會紅、提醒回來重判整支守衛。
- **B1** 前提：真管線攻擊確實會寫入（fixture 有效，否則 B2~B6 等於沒測）。
- **B2** ⭐⭐⭐ **主**：受傷方自己 END_TURN ⇒ active 與**全部** bench 都必須是 `undefined`。
- **B3** ⭐ **正對照（方向性）**：**對手** END_TURN **不得**清掉 ⇒ 防「修過頭」（清錯邊＝重裝角擊永遠讀 0）。
- **B4** bench **多隻**（3 隻）都要清 —— 防「只清第 0 隻」。
- **B5** ⭐⭐⭐ **端到端**：連續兩個對手回合各挨 50 ⇒ 第二輪必須是 **50 不是 100**。
- **B6** ⭐⭐⭐ **端到端傷害數字**：第二輪的重裝角擊 ＝ **100+50＝150**，不是 100+100＝200。
- **B7** 正對照（因果鏈）：`dmgTaken` 0／50／100／270 → 傷害 100／150／200／370
  ⇒ 白紙黑字寫下「累計灌水**就是**傷害灌水」。
- **C1** ⭐⭐⭐ END_TURN 區塊內必須有清除樣式：**三種等價寫法全枚舉**
  （`delete` 型／`: undefined` 型／rest 解構型 —— 安慰劑型態 8「字面樣式排除」），
  ＋ anchor 唯一性斷言 ＋ **行距上限 1500 行**（實測 ~777；型態規則 8「anchor 失效要能被發現」）
  ＋ 三個合規樣本的正對照 ＋ 一個「只讀取／寫 0」的反對照。
- **C2** 元守衛：本檔必須真的在 `package.json` 的 test chain 上（寫了卻沒跑＝安慰劑）。

⚠ fixture 細節記一筆：B5 的 bench 目標**必須**用 HP280 的赫拉克羅斯ex，不能用 HP60 的烏波 ——
挨兩輪各 50 會把烏波打死而離場，測到的就變成「被 KO 移走」而不是「跨回合累加」。

### 【三】驗證

**(a) 主證明**：刪掉 `engine.ts` 那六行 ⇒ 新守衛 `rc=1`，紅在 **B2／B4／B5／B6／C1**；還原 ⇒ `11 PASS / 0 FAIL`。
**同一個突變在 BASE（v6.326）下**：`test-v6255` ✅／`test-v6256` ✅／`test-v6253` ✅／
`test-heavy-impact-damage-taken` ✅ —— **全綠**，證明 v6.327 這一條是**新增**的偵測力。

**(b) 邊際帶突變**（⚠ 這是 v6.326 缺的那一類）

| 突變 | `total` | `totalMentions` | v6255／v6256 | **新守衛** | 判定 |
|---|---|---|---|---|---|
| **EDGE-1** 刪掉那六行（語義壞） | 5→**3**（＝下限） | 6→**4**（＝下限） | 都綠 | **紅**（B2/B4/B5/B6/C1） | ✅ 接住了 |
| **EDGE-2** 只刪 early-return 那一行（**合法等價重構**） | 5→**4** | 6→**5** | 都綠 | **綠** | ✅ 沒製造摩擦 |

⇒ 兩條合起來才叫「落在放寬的邊際帶」：語義壞的要紅、合法收斂的要綠。

**(c) 突變 11 個，全部紅在預期那一條**（⚠ 只捕 `assert.AssertionError`；紅在編譯失敗／crash 不算數）

| # | 突變 | 預期 | 實得 |
|---|---|---|---|
| M1 | 刪掉那六行 | B2,B4,B5,B6,C1 | ✅ 同 |
| M2 | 只清 active、不清 bench | B2,B4,B5 | ✅ 同（**C1 綠** ⇒ 結構斷言單獨不夠）|
| M3 | 清了但**沒寫回**（作用在淺拷貝副本上） | B2,B4,B5,B6 | ✅ 同（**C1 綠**）|
| M4 | 條件寫反（`===` → `!==`） | B2,B4,B5,B6 | ✅ 同 |
| M5 | 改成寫 `0` 而不是 `delete` | B2,B4,C1 | ✅ 同 |
| M6 | 加死條件 gate（`turn > 100` 才清） | B2,B4,B5,B6 | ✅ 同 |
| M7 | 掃描器打殘：C1 的 END_TURN anchor 打錯 | C1 | ✅ 同（**B 段全綠** ⇒ 行為斷言單獨也不夠）|
| M8 | 掃描器打殘：`CLEAR_RES` 換成永不匹配 | C1 | ✅ 同 |
| M9 | 中央寫入點改成直接覆寫（不再純累加） | A2 | ✅ 同 |
| M10 | fixture 打殘：重裝角擊 `regPre` key 改名 | B6,B7 | ✅ 同 |
| EDGE-2 | 合法等價重構 | **全綠** | ✅ 綠 |

**(d) HEAD-FAIL**（改過的檔各自還原成 BASE blob）

| 還原的檔 | 哪支守衛紅 | 紅在哪一條 |
|---|---|---|
| `package.json` | `test-v6327` | `C2 元守衛`（`test-v6327 不在 package.json 的 test chain 上 ⇒ CI 不會跑它`）|
| `src/lib/version.ts` | `test-v6160-checkin-version-gate` | `★admin 的 SITE_VERSION_HINT 與 src/lib/version.ts 的 VERSION 一致` |
| `scripts/test-v6327-*`（＝整支不存在） | chain 少一步 | 644 → 643（`npm test` 索引檢查）|
| `scripts/test-lib-strip-comments.mjs` | **無**（本版對它是**純註解**改動）| ⚠ 老實說：7-a 的定位註記沒有「還原就會紅」的守衛，它是文件性質的訂正 |

**(e) 數字**
- **完整 `npm test`：644 步、0 紅**（沙盒 `/tmp` `git clone --shared` 帶完整歷史，**序列**分批跑）。
  ⚠ 拆 chain 時 `steps.txt` 最後一行先補換行（v6.326 踩過：`while read` 會吃掉最後一步）。
- `tsc`（先 `npx svelte-kit sync`）：**TS2304 ＝ 0、總錯誤 54**（與基準相同）。
- `anti-pattern-lint`：**rc=0、✅ 無違規**。
- 免疫網三支：`test-damage-immunity-matrix` ✅／`test-attack-effect-immunity-matrix` ✅／`test-selection-ui` ✅。
- 零新依賴。

### 【四】v6.326 的五處文件訂正（都在 v6.326 段落就地標記）

1. 「**放寬之後原意突變仍然會紅**」⇒ **有反例**，已劃掉並附實測表 ＋ 判準。
2. **判準寫進去**（【一】那三行）。
3. A/B 表 **`bytes` 的「舊下限」欄**：`1,500,000` 是 **v6.324** 的值 ⇒ 改成 **2,500,000**，全表基準統一為 v6.325。
4. 「**19 條**」⇒ 集合定義講清楚（㊀ 有 A／B 註解的＝**20**／㊁ 表格列出的＝19／㊂ v6.325 改過值的＝15），
   **補上漏掉的 `v6262 SUPS ≥82`**，slack 統計更正為「19 條 ≤1、6 條 ＝0」。
5. `test-lib-strip-comments` **7-a** 旁註明「**這是實話存證、不是設計目標**；日後護欄若變強，改的是這一條不是護欄」。

### 【五】三配套 ＋ pin 掃描
① `oracle-admin/admin.html` `SITE_VERSION_HINT = '6.327'`（維持 **LF**，全檔 CRLF ＝ 0）
② `scripts/test-v6272-*` `PREV_SHA` → `427fdd3b…`；`PREV_ALLOWED` 維持 `['src/lib/version.ts']`（本版 `src/` 只動 version.ts）
③ `scripts/test-v6264-*` `BASE_SHA` → `427fdd3b…`
④ **pin 掃描**：與 v6.326 結論相同 —— 唯一命中仍是 `test-v6270-delta-put-client.mjs:658/673` 的
   `if (VERSION === '6.270')`，它自 v6.271 起走 else 分支並**印出「整檔 sha 錨定已停用」**（宣告而非靜默），
   同一條 `T()` 有不分版本的無條件斷言兜著 ⇒ 不是靜默失效，本版不動。
   其餘命中（`measure-v6321`／`test-v6184`／`test-v6189`／`test-v6213`／`test-v6261`）都是
   **fixture 資料裡的版本字串**，不是斷言 pin。

### 【六】⭐ 附帶掃描：站內還有哪些「兩條下限被同一段來源撐著」（**只列不修**）

| 組合 | 關係 | 危害 | 說明 |
|---|---|---|---|
| **v6255 C7 `total`（≥3） ＋ v6256 C1 `totalMentions`（≥4）** | **同 token、同檔組** | **⚠⚠ 高（已實現）** | 本版處理的那一組。兩條 slack 都是 2，而真碼恰好貢獻 2 ⇒ 剛好吃光 |
| **v6202 20a `sites`（≥90） ＋ 20c `g`（≥74）** | `g ⊆ sites`（守衛自己的註解就寫了）| ⚠ 中 | 同型：數的都是**功能碼消費點**；兩條 slack 都是 3 ⇒ 一次刪 3 個已接閘的消費點，兩條同時綠。⭐ 審查者已驗過這一族目前**守得住**（proximity 視窗 6→4 時 v6.326 仍紅），但**形狀一樣**，記一筆 |
| **v6255 C7 `total`（≥3） ＋ `readSites`（≥1）** | `readSites ⊆ total`，同一次掃描 | 低 | `readSites` 是 **B 類 slack 0** ⇒ 刪掉唯一讀取點一定紅，被保護住了 |
| **v6257 E2 `files.length`（≥191） ＋ v6258 L2 `srcFiles.length`（≥191）** | **同一個 `src/**` walk（.ts/.svelte）**，數值也相同 | 低 | 數的是**檔案數**不是功能碼 ⇒ 刪一段真碼不會讓它掉（除非整檔刪）。但兩條 slack 都是 3 且來源相同 ⇒ 一次刪 4 個檔會**同時**紅（不會出現「一條紅一條綠」的交叉檢查） |
| **v6211 `EFFECT_FILES.length`（≥95） ＋ `bytes`（>2.5M）** | 同一組 `effects/**` 檔案，同一個 `T()` 內 | 低 | 同上，檔案級；`bytes` 的百分比餘裕更寬 ⇒ 實務上先紅的是檔案數 |
| **v6257 E2 `files.length` ＋ 同段 `totalHits`（≥14）** | `totalHits` 在同一組 `files` 上計數 | 低 | 掃描目錄壞掉兩條同時掉（打殘型）；功能碼被刪只掉 `totalHits` ⇒ 不是同型 |

⇒ **只有第 1 組真的把偵測力吃掉了**（本版補），第 2 組是**同型但目前守得住**（列管），其餘三組數的是檔案／卡池不是功能碼。

## v6.326 純守衛版【批 3】：anti-pattern-lint（CI 第 1 步）的貪婪區塊正則遷移 ＋ 19 條下限逐條分 A／B 類 ＋ v6.325 三處文件訂正

BASE `e9430084ed96c4270f675b3e440c775540136c3c`（v6.325，遠端 main；`git ls-remote` 確認；一律以 BASE blob 為準）。
改動檔：`scripts/anti-pattern-lint.mjs`（Check Y 遷移）／`scripts/lib/strip-comments.mjs`（**只補檔頭**）／
`scripts/test-lib-strip-comments.mjs`（新增【7】）／8 支下限守衛（`test-v6202`／`v6211`／`v6253`／`v6255`／`v6256`／`v6257`／`v6258`／`v6262`）／
新量測腳本 `scripts/measure-v6326-checky.mjs`／`src/lib/version.ts`／`oracle-admin/admin.html`（SITE_VERSION_HINT，LF）／
`scripts/test-v6272-*`（PREV_SHA／PREV_ALLOWED）／`scripts/test-v6264-*`（BASE_SHA）／本檔。
零新依賴、`src/` 只動 version.ts、首頁 changelog 不放（純守衛）。
⚠ **不做**（留給批 4 以後）：`oracle-admin` 家族（`server_admin_patch.js` 543 行／`admin.html` 511 行，**伺服器端**）、
其餘 scripts 小洞、39 支無洞的衛生批、A 類 22~23 處 fail-open 全修、`test-v6277:609` 的 `blockRe`、
`lastIndexOf('<style')` 那族、`new Function` 實跑型、純 CSS 剝除、anti-pattern-lint 的「禁新增區塊正則」棘輪、
「chain 裡 2 支重複／25 支不在 chain 上」。

### 【一】批 3 主體：`anti-pattern-lint.mjs:919` 的 `/\/\*[\s\S]*?\*\//g`

⚠⚠ 這一支是 `npm test` 的**第 1 步**，改壞＝每一次 push 都被擋，所以下面每一步都寫清楚。

**① 到底有幾個 Check 會用到那份剝除器？＝ 恰好 1 個（Check Y）。**
`grep` 全檔，`/\/\*[\s\S]*?\*\//g` 只出現在 `:919` 一處，變數 `src` 的作用域就在 Check Y 的 `for (const f of files)` 裡。
⇒ ⚠ 交辦文字說它是「全站 lint 的地基」，就**影響面**而言是**誇大**的：它只影響 Check Y。
（真正誇大不了的是另一件事：它是 CI 第 1 步，所以改壞的**代價**很大。兩件事要分開講。）
順帶列出全檔其餘的剝註解寫法（本版**不動**，它們都是單行、不跨行，開不了洞）：
`:340`／`:418`／`:442`／`:464`／`:1063`／`:1100` 的 `lines[i].replace(/\/\/.*$/, '')`、`:1103` 的 `/\/\/[^\n]*/g`、
以及 Check V 自己的 `vStrip`（字元級狀態機，會追字串／樣板字面，`'/api/x/*'` 騙不開它）。

**② ⚠ 分清楚兩件事**（交辦文字提醒的）：
· **它掃別人時的剝除器**＝`:919`，**這次改的就是它**。
· **它自己被掃時的洞**＝v6.324 報的「`anti-pattern-lint.mjs` 有 573 行的洞」，那是**別的守衛拿舊剝除器掃它**時的事，
  跟 `:919` 無關，本版不處理（批 4 的衛生批）。

**③ 遷移內容**：`:919` 改成
`stripCommentsBlankChecked(src, { label: rel(f) }).replace(/(^|[^:])\/\/.*$/gm, '$1')`。
· 用**等長留白**版不是刪行版 —— Check Y 的違規訊息帶行號。
· 行尾 `//` 那一條**逐字保留**（含 `(^|[^:])`，避免把 `https://` 當註解）—— **不放寬任何斷言**。
· 護欄炸掉時**轉成 `violations.push`（exit 1）**，不是 `continue` 跳過 ⇒ 不是 fail-open。

#### (a) 命中集合逐項相同（不是只比總數）
`node scripts/measure-v6326-checky.mjs`：
```
① 命中數：舊 177 → 新 177
   (檔|卡名) 多重集合逐項相同：✅ 是
   只在舊：(無)      只在新：(無)
...
#### (b) ⭐ 順帶修好的事：行號本來 172/177 是錯的
```
② 行號正確筆數：舊 5/177、新 177/177；舊版最大偏差 1170 行
   （effects.ts 的 regG('貴重手推車') 實際 :17174，舊版報 :16004）
```
原因：舊剝除器 `.replace(/\/\*[\s\S]*?\*\//g, '')` 把區塊註解**連換行一起刪掉**，後面每一行都往上位移。
⇒ 遷移前 Check Y 若真的抓到違規，**印出來的行號多半打不開那一行**。

#### (c) ⭐⭐⭐ 這一支其實**有**真 HEAD-FAIL —— 交辦文字說「沒有」，被我推翻

「今天 177 = 177 ⇒ 沒有 HEAD-FAIL」只證明了「**現況**沒有違規落在洞裡」，不等於「造不出 HEAD-FAIL」。
把一個**真違規**（`regG('皮卡丘', …)`，皮卡丘是寶可夢 ⇒ 註冊進 `TRAINER_GUARDS` 是死碼）注入 `effects.ts` 的假區塊之內，
用**真的 lint 子行程**跑（BASE 版 vs v6.326 版）：

| 注入位置 | BASE 的 lint | v6.326 的 lint |
|---|---|---|
| `effects.ts:56`（落在 `:27` 假區塊 `:27~:147` 內） | **rc=0、`[Y]` 0 條 ← 假綠** | rc=1，`[Y] effects.ts:56 — regG('皮卡丘') 的對象是**寶可夢**` |
| `effects.ts:8126`（落在 `:7905` 假區塊 `:7905~:8203` 內） | **rc=0、`[Y]` 0 條 ← 假綠** | rc=1，`[Y] effects.ts:8126 …` |
| `effects.ts:9001`（洞外，對照組） | rc=1（抓得到，但行號報成 **:8019**，錯 982 行） | rc=1，行號 `:9001` ✅ |
| 未注入（對照） | rc=0 | rc=0 |

⇒ **差別確實只在「洞」**，而且洞外那一列順帶再證一次行號問題。

#### (d) ⭐ 事故形狀突變（M11b′）：遷移真的接上了護欄
見下面【三】。⚠ 這一條同時是「Check Y **真的**接上中央 helper」的管線斷言 ——
沒接上的話，突變 helper 完全不會改變 lint 的行為（「只驗字串存在的守衛擋不住接線沒接上」）。

#### (e) ⭐⭐ 怎麼確定沒改壞 CI 第 1 步
1. **未注入時 `node scripts/anti-pattern-lint.mjs` rc=0、輸出「✅ 無違規」**（遷移前後都是）。
2. **命中集合逐項相同**（上面 (a)）⇒ Check Y 的判定輸入沒有變。
3. **斷言一個字都沒放寬**：`ySeen < 10`、`pokeNames.size < 500`、「同名同時是寶可夢與訓練家 ⇒ 放行」三條原樣不動；
   行尾 `//` 正則逐字保留。
4. **新增的兩條只會「多紅」不會「少紅」**（正對照／反向對照），而且它們在真掃描**之前**跑 ⇒ 剝除器回歸時確定性地先紅在自己那一條。
5. **護欄炸掉不吞**：轉成 violation（exit 1），不是 `continue`。
6. **子行程實跑**：`test-lib-strip-comments`【7】每次 `npm test` 都會跑一次**真的** lint（未突變版必須 rc=0）
   ⇒ 「lint 本身跑得起來」這件事現在有守衛在守，不只靠 CI 那一次。
7. 全站 1,025 支檔案在**預設護欄**下全綠（`test-lib-strip-comments` 5-1／5-2），
   ⇒ `stripCommentsBlankChecked` 不會在 CI 上對某個檔意外炸掉。
   ⚠ 附帶數字漂移一筆：5-2 印的「留存率 < 50% 的檔」從 59 變 **60** —— 因為本版給 `strip-comments.mjs`
   自己加了 14 行檔頭註解，它自己掉到 50% 以下。那是 `console.log` 的診斷值不是斷言值（斷言是 `under50 >= 10`）。

### 【二】19 條下限逐條分成 A／B 兩類（站長裁定：「幫會掉的那幾條留 2~3」）

**分類原則（不一刀切，逐條給理由）**
· **A 類「收斂就會掉」＝ 數的是「程式碼消費點／檔案數／位元組」** —— 中央收斂、合併卡檔、刪一條管線都會讓它**合法下降**。
  歷史上三次實證下降**全部**在這一類：`sites 90→88`（v6.258）、`totalHits 20→17`（v6.258）、`lit 55→50`（v6.204）。
· **B 類「結構性最小值」＝ 數的是「卡池枚舉／註冊表條目／結構定義」** —— 中央收斂再怎麼做都不會讓卡池少一張、
  註冊表少一條。日常方向只增不減 ⇒ **維持 slack 0**，掉了就是真的少了東西。
  ⚠ B 類唯一會掉的情形是「整批卡包輪替下架」，那是掉一大截（不是 1~3），而且那時本來就該人工回來重判整支守衛
  ⇒ 留 2~3 的餘裕**對它沒有用**，只會白白降低偵測力。

| # | 守衛 | 斷言 | 類別 | 理由（為什麼歸這一類） | 舊下限 | 實測 | 新下限 | 新 slack |
|---|---|---|---|---|---|---|---|---|
| 1 | v6202 20a | `sites.length` | **A** | passive **消費點**數；v6.258 實證 90→88 | 92 | 93 | **90** | 3 |
| 2 | v6202 20b | `lit` | **A** | 字面量消費點；v6.204 實證（6 支 helper 收斂 ⇒ 下限被迫 55→50） | 58 | 59 | **56** | 3 |
| 3 | v6202 20b | `reg` | **A** | registry 查表消費點，同族 | 33 | 34 | **31** | 3 |
| 4 | v6202 20c | `g`（已接閘） | **A** | `sites` 的**子集**，sites 掉它一定跟著掉 | 76 | 77 | **74** | 3 |
| 5 | v6211 | `EFFECT_FILES.length` | **A** | `effects/**` 的**檔案數**，合併／刪一支卡檔就 −1 | 97 | 98 | **95** | 3 |
| 6 | v6211 | `bytes` | **A** | 位元組數；**唯一非整數計數**，餘裕要看百分比 | 2,500,000 | 2,548,259 | **2,500,000（不動）** | 48,259（1.9%） |
| 7 | v6253 C1 | `abNames.size` | **B** | live H/I/J **卡池**裡「不會昏厥」的特性種類 | 4 | 4 | **4（不動）** | 0 |
| 8 | v6253 C1 | `toolNames.size` | **B** | 同上，道具型；結構上至少要有 1 | 1 | 1 | **1（不動）** | 0 |
| 9 | v6255 A3 | `wide.length` | **B** | **卡池**裡符合寬鬆措辭的印刷筆數；真斷言是下面的 `deepEqual`（三個特性名逐字釘死） | 12 | 13 | **12（不動）** | 1 |
| 10 | v6255 C7 | `total` | **A** | 欄位名在**程式碼裡**的出現次數；少一個消費點就 −1。基數只有 5 ⇒ 餘裕給 2 | 5 | 5 | **3** | 2 |
| 11 | v6255 C7 | `readSites` | **B** | 「**唯一**讀取點（重裝角擊）」的結構最小值 | 1 | 1 | **1（不動）** | 0 |
| 12 | v6256 C1 | `totalMentions` | **A** | 同 #10，程式碼提及次數；基數 6 ⇒ 餘裕給 2。⚠ v6.325 是 slack 0 且漏了註明 | 6 | 6 | **4** | 2 |
| 13 | v6256 C2 | `calls.length` | **A（例外只留 1）** | ⚠⚠ 它性質上是消費端數（A），但**同時兼任「有管線被拔掉」的唯一偵測器**（這個 token 沒有 exact enumeration 兜底）。v6.325 已實證：把兩個 `withAttackDamageTaken(` 改成 `withAttackDamageTaken (`（12→10）時 `>=10` **是綠的**、`>=11` 才紅 ⇒ 放到 10 就抓不到那個 HEAD-FAIL | 11 | 12 | **11（不動）** | 1 |
| 14 | v6257 E2 | `files.length` | **A** | `src/**` **檔案數** | 193 | 194 | **191** | 3 |
| 15 | v6257 E2 | `RISKY.size` | **B** | **卡池**裡「同名不同印刷」的卡名數 | 99 | 100 | **99（不動）** | 1 |
| 16 | v6257 E2 | `totalHits` | **A** | ⭐ 有**直接歷史證據**：v6.258 這個數字 20→17（−3）⇒ slack 1 會讓那次合法收斂當場翻紅 | 16 | 17 | **14** | 3 |
| 17 | v6257 F2 | `getEvolveTimingBypass(` | **B** | 3 ＝ 定義 1 ＋ 兩端各 1，是中央閘存在所必需的最小結構；掉到 2 ＝ 某一端**繞過了中央閘**（正是本條要抓的），不是收斂 | 3 | 3 | **3（不動）** | 0 |
| 18 | v6258 L1 | `names.length` | **B** | `PASSIVE_ATTACK_BONUS` **註冊表條目數**，只會因新卡而增加；減少＝有人刪掉一張卡的被動加成 | 12 | 12 | **12（不動）** | 0 |
| 19 | v6258 L2 | `srcFiles.length` | **A** | `src/**` **檔案數** | 193 | 194 | **191** | 3 |

⚠ **一處事實訂正**：交辦文字說「19 條下限全部收到 slack ≤ 1」——
`bytes`（#6）的 slack 是 **48,259（1.9%）**，不是 ≤1，所以正確講法是 **18 條 ≤1、6 條 ＝0**。

⛔ **v6.327 訂正（本表的兩個瑕疵，都是我們自己寫錯的）**

**(甲) 「舊下限」欄的基準不一致。** 第 1~5 行寫的是 **v6.325 的值**，
第 6 行 `bytes` 卻寫了 **1,500,000** —— 那是 **v6.324** 的值（v6.325 已改成 2,500,000，
見 `test-v6211` 的 `⭐v6.325：40 → 97（實測 98）、1_500_000 → 2_500_000` 註解）。
⇒ 已就地改成 **2,500,000**，全表「舊下限」一律 ＝ **v6.325（本表 BASE）的值**。

**(乙) 「19 條」這個分母對不上任何集合。** 把集合定義講清楚：

| 集合定義 | 條數 |
|---|---|
| ㊀ **v6.326 在旁邊寫了【A 類】／【B 類】註解的下限斷言**（本表想描述的就是這一個） | **20** |
| ㊁ 上表實際列出的行數 | 19（**漏了 `v6262` 的 `SUPS.length >= 82`**）|
| ㊂ v6.325 在 8 支守衛裡改動過下限**值**的斷言 | 15 |

⇒ 正解是 **㊀＝20 條**，補上漏掉的第 20 條：

| # | 守衛 | 斷言 | 類別 | 理由 | 舊下限 | 實測 | 新下限 | 新 slack |
|---|---|---|---|---|---|---|---|---|
| 20 | v6262 | `SUPS.length` | **B** | live H/I/J **卡池**裡的支援者張數；中央收斂不會讓卡池少一張 | 82 | 83 | **82（不動）** | 1 |

⇒ slack 統計相應更正為：**19 條 ≤1、6 條 ＝0**（`bytes` 的 1.9% 仍是唯一的非整數餘裕）。

⭐ A 類的訊息都改寫成可操作的：
`只掃到 N 個（A 類下限 90／實測基準 93；掉 1~3 多半是合法收斂，確認後改守衛這一行）`。
B 類的訊息附 `（B 類：結構性最小值，slack 0）`／`（B 類：卡池枚舉，不留收斂餘裕）`。

~~**放寬之後原意突變仍然會紅**（＝沒放到抓不到），見【三】的 MS／MB 段。~~

⛔ **v6.327 訂正：這句話有反例，是錯的。**
【三】的 MS1~MS9 **全部**是「把掃描器打殘」型（實得 `0`／`3`／`5`／`23`），
**沒有一個落在放寬的邊際帶**（5→3、6→4）。
> ⭐⭐⭐ **任何下限都會紅的突變，證明不了下限值本身的價值。**

真正的反例：#10 `v6255 C7 total` 與 #12 `v6256 C1 totalMentions`
**是被同一段真碼撐著的** —— `src/lib/game/engine.ts` END_TURN 的
`damageTakenLastOppTurn` 清除區塊（六行），剝註解後恰好貢獻 **2 行／2 次提及**。
實測（BASE `427fdd3b`，把那六行刪掉）：

| 下限版本 | `total` | `totalMentions` | 結果 |
|---|---|---|---|
| v6.325（≥5／≥6） | 5→**3** | 6→**4** | v6255 **rc=1**、v6256 **rc=1** |
| v6.326（≥3／≥4） | 5→**3** | 6→**4** | **兩支都綠**（剛好等於新下限） |

而 `_shared.ts:2771` 的中央寫入點是**純累加** ⇒ 那六行是全站**唯一**的重置點，
刪掉會讓「超級赫拉克羅斯ex｜重裝角擊」跨回合無限累加（**傷害灌水級 bug**）。
⇒ 放寬本身沒錯（合法收斂不該翻紅），但**偵測力必須另外補回來** —— v6.327 補上（見該版段落）。

### 【三】突變（21 個；⚠ 只捕 `assert.AssertionError`，紅在編譯失敗不算數）

**Check Y／剝除器（9）**
| # | 突變 | 期望 | 實得 |
|---|---|---|---|
| M11a | 只把 `t.startsWith(open)` → `line.includes(open)` | ⭐ **綠**（護欄①③⑤ 都攔不到） | lint **rc=0** ✅ |
| M11b′ | M11a ＋ 把規則 (a) **搬到區塊判斷之後** | 紅在 `[Y] 正對照失敗` | lint rc=1，訊息符合 ✅ |
| M-inj1 | 真違規注入 `effects.ts:56`（洞內） | BASE 綠／新版紅 | rc 0 → 1 ✅ |
| M-inj2 | 真違規注入 `effects.ts:8126`（洞內） | BASE 綠／新版紅 | rc 0 → 1 ✅ |
| M-inj3 | 真違規注入 `effects.ts:9001`（洞外，反面對照） | 兩版都紅 | 都 rc=1 ✅（差別確實只在洞） |
| M-y1 | `yStrip` 改成 identity（完全不剝） | 紅在 `[Y] 反向對照失敗` | ✅ |
| M-y2 | `yStrip` 吐空 | 紅在 `[Y] 正對照失敗` | ✅（另外也紅了「只掃到 0 個 regG」） |
| M-y3 | `static/cards` 路徑打錯 | 紅在 `[Y] 掃描器異常 — 只讀到 0 個寶可夢卡名` | ✅ |
| M-y4 | Check Y 的掃描 regex 打錯（`regG`→`regGZZ`） | 紅在 `[Y] 掃描器異常 — 只掃到 0 個 regG 呼叫` | ✅ |

⚠ **M11a 綠是刻意的、也是這一版檔頭那句話的證據** —— 它證明「擋住事故 2 的是**規則順序**，不是護欄」。
（審查者第一版只做 M11a 就宣稱守住了，實際上那個突變根本沒紅。）

**A 類放寬後的原意突變（9，全部仍紅）**
| # | 守衛 | 突變（弄壞掃描器） | 新下限 | 實得 |
|---|---|---|---|---|
| MS1 | v6202 | pattern1 的 `.name === '…'` regex 打錯 | `lit>=56` | `pattern1 只 0` ✅ |
| MS2 | v6202 | 掃描目錄打錯 | `sites>=90` | `只掃到 23 個` ✅ |
| MS3 | v6202 | `ABILITY_REGISTRY_RE` 換成永不匹配 | `reg>=31` | `pattern2 只 0` ✅ |
| MS4 | v6211 | `EFFECT_FILES` 收集上限 3 | `>=95` | `只掃到 3 個 .ts` ✅ |
| MS5 | v6255 C7 | 掃描檔清單清空 | `total>=3` | `實得 0` ✅ |
| MS6 | v6256 C1 | `strip()` 回傳固定字串 | `totalMentions>=4` | `實得 0` ✅ |
| MS7 | v6257 E2 | 檔案收集上限 5 | `files>=191` | `只找到 5 個` ✅ |
| MS8 | v6257 E2 | `NAME_EQ` 換成永不匹配 | `totalHits>=14` | `只掃到 0 個` ✅ |
| MS9 | v6258 L2 | 檔案收集上限 5 | `srcFiles>=191` | `只找到 5 個` ✅ |

**B 類「少一個就要紅」的驗證（3）**
| # | 守衛 | 突變 | 實得 |
|---|---|---|---|
| MB1 | v6257 F2 | 模擬少一個 `getEvolveTimingBypass` 呼叫端 | `呼叫點只有 2（預期 ≥3）` ✅ |
| MB2 | v6258 L1 | 模擬註冊表少一個條目 | `只註冊了 11 個（預期 ≥12）` ✅ |
| MB3 | v6253 C1 | 模擬卡池少一個特性型防 KO | `特性型防 KO 應 ≥4（實得 3）` ✅ |

### 【四】檔頭補的那一句（`scripts/lib/strip-comments.mjs`）

寫進去的是**實話不是保證**：
> 真正擋住事故 2（`//` 註解裡的 `/*` 一路吃掉真碼）的是**規則順序** ——
> 規則 (a)「不在區塊內、行首 `//` ⇒ 丟整行」排在區塊判斷之前，所以那一行裡的 `/*` 根本沒機會開區塊。
> ⚠ 護欄①（留存率）③（區塊長度）⑤（連續丟棄行數）對這個形狀**都攔不到**。

由 `test-lib-strip-comments`【7】的 **7-a（M11a 必須綠）／7-b（M11b′ 必須紅）** 實跑撐著，
⚠ **v6.327 補註（7-a 的定位）**：7-a 斷言的是「**某個突變必須是綠的**」——
它是**實話存證**（記錄「今天的護欄①③⑤ 攔不到事故 2 的形狀」這個事實），**不是設計目標**。
⇒ 日後若把護欄做強到連 M11a 都攔得下來，那是**改進**，要改的是**7-a 這一條**（連同檔頭那句話），
**不是**回頭把護欄調弱去迎合它。（把「抓不到」鎖成規格，會讓「變強」看起來像「回歸」。）
已就地寫進 `scripts/test-lib-strip-comments.mjs` 的 7-a 上方。
外加 **7-c**（不靠突變的行為端表述：行首 `//` 裡的 `/*` 不開區塊、行首 `/*` 會開、收尾後尾巴保留）
與 **7-0**（未突變時同一條路徑必須綠 ⇒ 突變 harness 不是恆紅）。
⚠ 【7】只寫兩個 `scripts/.y6326-*` dot-file、**絕不覆寫 repo 既有檔**，離場（含 SIGINT／SIGTERM）一律刪。

### 【四之二】⚠ 遷移當場弄紅了一支既有守衛：`test-lint-crlf-neutral`（照它說的修，沒有繞過）

`scripts/test-lint-crlf-neutral.mjs` 把 lint **原封不動搬進臨時工作區**、用 LF／CRLF 各真跑一次。
它原本只 `cpSync` 了 `scripts/anti-pattern-lint.mjs` 一個檔 —— 因為在 v6.325 之前，lint 是**零相依單檔**。
本版給它加了 `import … from './lib/strip-comments.mjs'` ⇒ 臨時工作區的 lint `ERR_MODULE_NOT_FOUND` 直接掛掉
⇒ 該守衛 **PASS 4 / FAIL 3**（②的兩條正對照與路徑那條）。

修法（不放寬任何判準）：
1. 一併 `cpSync(join(ROOT, 'scripts/lib'), join(WORK, 'scripts/lib'), { recursive: true })`。
2. ⭐ **順手補一條前置**：臨時工作區的 lint **必須真的跑得起來**
   （輸出不得含 `ERR_MODULE_NOT_FOUND`／`Cannot find module`／`SyntaxError`，且要印出「反模式 lint」）。
   ⚠ 沒有這一條的話，日後 lint 再新增相依而漏搬時，本守衛會退化成
   **「兩邊都掛掉 ⇒ exit code 相同 ⇒ ① 全綠」的假綠**（第 13 種安慰劑的近親）。
修後 **PASS 7 / FAIL 0**。

⭐ 這一次翻紅本身就是好事：它證明「Check Y 真的改成走中央 helper」——
如果只是改了註解、沒真的接線，這支守衛根本不會有反應。

### 【五】HEAD-FAIL（改過的檔各自還原成 BASE blob）

| 還原的檔 | 哪支守衛紅 | 紅在哪一條 |
|---|---|---|
| `scripts/anti-pattern-lint.mjs` | `test-lib-strip-comments` | 7-0／7-a／7-b 三條同時紅在 `anti-pattern-lint.mjs 沒有從中央 helper import ⇒ Check Y 的剝除器沒接上中央 helper（接線沒接上）`（52 passed / **3 failed**；還原後 55/0） |
| `src/lib/version.ts` | `test-v6160-checkin-version-gate` | `★admin 的 SITE_VERSION_HINT 與 src/lib/version.ts 的 VERSION 一致`（70/**1**；還原後 71/0） |
| 8 支下限守衛 | 它們自己 | 見【三】MS／MB 段（下限本身就是斷言） |
| `scripts/test-lint-crlf-neutral.mjs` | 它自己 | 還原成 BASE（沒搬 `scripts/lib`）⇒ `PASS 4 / FAIL 3`，紅在「LF 沒抓到注入的違規：(無)」（＝臨時工作區的 lint 掛掉了）；修後 7/0 |
| `scripts/lib/strip-comments.mjs` | **無**（本版對它是**純註解**改動） | ⚠ 老實說：檔頭那句話沒有「還原就會紅」的守衛，它的證據是【7】的 7-a／7-b —— 那兩條在 `test-lib-strip-comments.mjs` 裡 |

### 【五之二】驗證數字
- **完整 `npm test`：643 步、0 紅**（沙盒 `/tmp` 用 `git clone --shared` 建帶完整歷史的樹，序列分批跑）。
  ⚠ **索引無缺的檢查**：把 chain 拆成 `steps.txt` 時最後一行沒有換行，`while IFS= read -r` 會**吃掉最後一步**
  （實測迴圈只跑到 642）。第 643 支 `test-v6321-undo-snapshot-cross-game` 已**單獨補跑**：`43 PASS / 0 FAIL`。
  ⭐ 這一條記下來：日後拆 chain 一律先 `printf '\n' >>` 補尾行換行，或用 `mapfile`。
- `tsc`（先 `npx svelte-kit sync`）：**TS2304 ＝ 0、總錯誤 54**（與基準相同）。
- `anti-pattern-lint`（本版就是在改它）：**rc=0、✅ 無違規**。
- 免疫網三支：`test-damage-immunity-matrix` ✅／`test-attack-effect-immunity-matrix` ✅／`test-selection-ui` PASS 35 / FAIL 0。
- 零新依賴。

### 【六】三配套 ＋ pin 掃描
① `oracle-admin/admin.html` `SITE_VERSION_HINT = '6.326'`（維持 LF）
② `scripts/test-v6272-*` `PREV_SHA` → `e9430084…`，`PREV_ALLOWED` 維持 `['src/lib/version.ts']`（本版 `src/` 只動 version.ts）
③ `scripts/test-v6264-*` `BASE_SHA` → `e9430084…`
④ **pin 掃描**：全 `scripts/`＋`oracle-admin/` 掃「pin 死版本號／整檔 sha256」——
   唯一命中是 `test-v6270-delta-put-client.mjs:658/673` 的 `if (VERSION === '6.270')`，
   但它**自 v6.271 起就走 else 分支並印出「整檔 sha 錨定已停用 —— 這是宣告不是靜默」**，
   而且同一條 `T()` 裡有不分版本的無條件斷言兜著 ⇒ **不是靜默失效**，本版不動（列出以備查）。

### 【七】批 4（`oracle-admin` 家族）的地圖 —— ⚠ 伺服器端，部署順序要另外想
（見本版最後一段，與交辦的「不做」清單一致。）
...
> **⭐v6.326 訂正：上表的「⭐C 新版吃更多 ＝ 0」名實不符** —— `.ts` 檔確實是 0，
> 但 `.svelte` 檔新版**確實多吃了 711 行**，只是被 `measure-v6325-equiv.mjs` 的
> `inHtml.has(i + 1)` 這個 carve-out 併進 B 欄了。表格補一欄：
>
> | 副檔名 | A 救回 | B1 保守保留 | **B2 HTML 註解（新版多吃）** | ⭐C 新版吃更多 |
> |---|---|---|---|---|
> | `.ts` | 273 | 51 | **0** | 0 |
> | `.svelte` | 72 | 154 | **711** | 0 |
>
> 那 711 行的前幾名：`src/routes/game/+page.svelte` **528**、`MobilePortraitBattle.svelte` **77**、
> `decks/+page.svelte` **48**、`friends/+page.svelte` 18、`FriendsPanel.svelte` 17、`routes/+page.svelte` 12、
> `cards/+page.svelte` 7、`deck-posts/+page.svelte` 2、`DmPanel.svelte` 2。
> **全部是 `<!-- … -->` HTML 註解**（舊的 C 型剝除器根本不處理 HTML 註解），方向無害 —— 剝掉註解是對的。
> ⭐ 但**別讓「C＝0」被日後的人讀成「新版絕不會多吃」**：正確的說法是
> 「新版多吃的 711 行**全部**是 HTML 註解，`.ts` 側才是 0」。
> ⚠ 順帶記一筆：那個 carve-out 自己用的是 `/<!--[\s\S]*?-->/g` —— **正是這一整批在修的同族貪婪正則**。
> 今天沒被它騙，但它是量測腳本、不是守衛 ⇒ 值得記著：**量測腳本自己用著被量測的反模式**。
...
| v6256 | `totalMentions` | 5 | 6 | 6（⭐v6.326 訂正：當時 slack ＝ 0 卻**漏了註明**，其餘五條都有寫「slack 0」；v6.326 已改歸 A 類並放寬到 4，見下一節） |
...
⚠ v6202 的 `sites` 從 91 變 93，**不是掃描器變強，是那 273 行真碼重新可見** —— 洞裡本來就有 **2 個** passive 消費點，
而且它們**全部都已經接了中央閘**（20d/20e 遷移後仍綠）⇒ 沒有因此挖出新的卡片 bug。

> **⭐v6.326 訂正（原句寫成「88 → 93、洞裡有 5 個」，兩個數字都錯）**
> · **BASE 實測是 91 不是 88**。88 是**兩版前註解裡記的舊值**（v6.258 當時的量測），被當成 BASE 直接抄了。
>   複驗方式：把 `test-v6202` 的 `stripComments` 換回舊的自寫區塊正則、把三條下限頂到 999999 讀失敗訊息 ⇒
>   `sites 91`／`lit 57`／`gated 75`（v6.325 版是 `93`／`59`／`77`）。
> · **洞裡是 2 個消費點不是 5 個**：逐筆 diff 兩種剝除器的 site 集合（`檔:行|特性|kind|gated`），
>   只在新版出現的恰好兩筆 —— `src/lib/game/effects.ts:7969|繁茂|lit|gated=true`、
>   `src/lib/game/effects.ts:7974|繁茂|lit|gated=true`；「只在舊版」是空集合。
>   兩筆都是**繁茂**、都 `gated=true`、都不在 `EXEMPT` 表裡。
> · ⭐ **結論不變**：救回來的兩個消費點都已接中央閘 ⇒ 這次遷移沒有挖出新的卡片 bug。
>   （+2 的差值與 `lit`／`gated` 各 +2 完全對得起來，這也是「5」不成立的算術證據。）
...
改動檔（全在 `scripts/`＋三配套）：`scripts/lib/strip-comments.mjs`／新 `scripts/test-lib-strip-comments.mjs`／`scripts/test-v6277-*`（helper 突變錨點改到新形狀）／
`scripts/test-v6179-*`／`test-v6146-*`／`test-v6159-*`／`test-v6175-*`／`test-v6180-*`／`test-v6246-*`／`test-v6210-central-*`／`test-v6111-*`／`test-v6182-*`／`test-v6189-*`／`test-v6198-*`／
`package.json`（chain 加 test-lib-strip-comments）／`src/lib/version.ts`／`oracle-admin/admin.html`（SITE_VERSION_HINT）／`scripts/test-v6272-*`（PREV_SHA／PREV_ALLOWED）／`scripts/test-v6264-*`（BASE_SHA）／本檔。
零新依賴、`src/` 只動 version.ts、首頁 changelog 不放（玩家看不到）。⚠ **不做**批 2 以後（engine／effects／_shared 家族、多檔掃描器、伺服器端、39 支無洞的）；不動 test-v6277:609 的 `blockRe` 反面對照、`lastIndexOf('<style')` 28 處、`new Function`／esbuild 實跑型、純 CSS 剝除、test-admin-helper-scope 的 IIFE 弱點；不在 anti-pattern-lint 加棘輪。

### 背景（行為端 hook 複驗上游枚舉）
hook `String.prototype.replace` 實跑 7 支：7 支都把 `/\/\*[\s\S]*?\*\//g` 套在 `game/+page.svelte`（1,010,707 字元）上，最大一段匹配 **:208～:384（11,268 字元）** —— 起點是 :208 的 `// … /api/tournament/* 回應帶 X-Srv-Ms` 那行行中的 `/*`。洞內有 :342 `let _rtSegN = 0;` 等真程式碼。

### 【H0】strip-comments.mjs：同一台狀態機、兩種渲染
- 狀態機 `scanCommentLines()` 只跑一次，產出 `keepFrom[i]`（每行從第幾個字元起保留；-1＝整行丟）這份**唯一真相**；`renderDropped`（刪行，＝v6.312 輸出逐位相同）與 `renderBlanked`（等長留白：丟掉的字元換空格、`\r`／`\n`／每行長度不變）都從它導出。**沒有第二台狀態機。**
...
### 【批 0】test-v6179：`_rtSegN = 0;` 恰一處是靠洞校準出來的
- 真檔兩處：:342 `let _rtSegN = 0;`（洞內）＋ :6042 歸零。舊剝除器吃掉宣告 ⇒ 剩 1 ⇒ 綠。**只換剝除器不收緊** ⇒ 立刻誤紅（實跑 NEW 計數 2）。
- 收緊：`grabFn(BARE,'_tResetResSeg')` 本體恰 1 處 ＋ 全檔 `/^\s*_rtSegN = 0;/m` 恰 1 處 ＋ 全檔 `_rtSegN = 0;` 恰 2 處（宣告＋歸零，從 BASE blob grep 量出手抄）。新增自驗：洞內宣告剝完還在。
- 原意突變：① tEnterMatch（:5509 之後）加 `_rtSegN = 0;` ⇒ 修前紅／修後紅（`reset 本體 1／行首歸零 2／全檔 3`）；② **洞內** :209 之後加 ⇒ 修前**綠**／修後紅。

### 【批 1】game 頁六支：`stripCommentsChecked`／`stripCommentsBlankChecked` ＋ 各檔原有的行尾 `//` 正則（單行、不跨行 ⇒ 不會形成洞）
每支：洞外（:386 之後）塞違規 ⇒ 修前紅／修後紅（原意守住）；**洞內**（:209 之後）塞同一違規 ⇒ 修前綠／修後紅。
| 守衛 | 違規 snippet | 修後紅在 |
|---|---|---|
| v6146（等長版；mustKeep startTournamentPoll／tPollDesiredMs、mustDrop「是降頻不是停」） | `startTournamentPoll();` | 呼叫點 8 次 |
| v6159 | `new PerformanceObserver(() => {})` | 最多兩顆 — 3 |
| v6175 E | `resolveSelection(payload, sid)`（缺 token） | 每一處都帶 pendingToken |
...
| 桌機縮窄 820×1180 滑鼠 | **none** | — | — | — |
  ...
  並在測試站部署後實跑（下表）。
- **部署後補記（測試站 suenz001.github.io，commit d96780ac，playwright isMobile+hasTouch，本機 AI 對戰進入戰鬥後量）**：

| 情境 | `.rotate-prompt` 有無渲染 | display | 說明 |
|---|---|---|---|
| 820×1180 直立，開關關 | 有 | flex（z 99999） | 新文案七段全在、guide 265×86 @ y=648 未裁切；⚙️ 設定鈕被遮罩蓋住（elementFromPoint 落在遮罩）⇒ 文案「先橫放」是必要的 |
| 820×1180 直立，開關開 | **無**（`.mp` 手機版分支亮著） | — | 勾選後直立整塊不渲染 ⇒ 不需要內層條件（查證成立） |
| 375×812 真手機（開關關／開） | **無** | — | 零位移（本來就不顯示） |
| 1180×820 橫放 | 有 | **none** | CSS orientation 層擋住，不可見 |
...
（v6.313 尚未上正式站 ⇒ 不新增條目、不搬運，test-v6264 走 F0b）、version.ts、SITE_VERSION_HINT、test-v6272 PREV_SHA→0c7422e5／PREV_ALLOWED 三項、test-v6264 BASE_SHA→0c7422e5。

### 為什麼要補這一句（v6.313 部署後在測試站用真開關量到的）

- 820×1180 開關關（＝現況）：桌機分支＋`.rotate-prompt` display=flex ⇒ **連 ⚙️ 設定鈕都被遮罩蓋住點不到**（playwright click 30 秒逾時，被 rotate-prompt 攔截）。
  ⇒ 玩家要打開這個開關，**必須先橫放**（橫放時遮罩消失、開關在設定內可勾），再轉直立才會切到手機版。
- 820×1180 開關開 → 在設定內取消勾選：立刻切回桌機版（localStorage '0'），**遮罩隨即蓋回來、設定 modal 也被蓋住** ⇒ 取消之後同樣要橫放才能再進設定。
- 這兩件都是 BASE 既有行為（遮罩 z-index 99999 在版面分支之外，v2.206 起如此），本版不動它（動了就是把「設定 modal 高於遮罩」擴到所有 601～950 直式觸控裝置，超出站長裁定的範圍）；
  🔨 若站長要根治，下一版候選：讓設定 modal（或只有這個開關）浮在 rotate-prompt 之上，需另做量測。

### v6.313 部署後真開關量測（不偽造；測試站 v6.313）

| 尺寸 | 開關 | 分支 | rotate-prompt | 溢出／重疊／水平捲動 | 內容高度 | 設定內開關 |
|---|---|---|---|---|---|---|
| 744×1133 | 開 | 手機版 | 不渲染 | 0／0／無 | 885～905 | 有、已勾 |
| 820×1180 | 開 | 手機版 | 不渲染 | 0／0／無 | 885～905 | 有、已勾；取消 ⇒ 立刻桌機版 |
| 820×1180 | 關 | 桌機版 | flex（＝現況） | — | — | 被遮罩擋住點不到 |
| 834×1194 | 開 | 手機版 | 不渲染 | 0／0／無 | 885～905 | 有 |
| 1024×1366 | 開 | 手機版 | 不渲染 | 0／0／無 | 885～905 | 有 |
| 1180×820（橫） | 開 | 桌機版 | none | — | — | 有、已勾（橫放時勾選的入口） |
| 375×812 | 關 | 手機版 | 不渲染 | 0／0／無 | 812（100%） | **沒有**（真手機不顯示） |
| 375×812 | 開 | 手機版 | 不渲染 | 0／0／無 | 812 | 沒有 |

375×812 開關關 vs BASE（v6.312 偽造前的原生量測）：setup／playing 兩個時點的 9～10 個主區塊 rect、6 種元件尺寸、關鍵區 rect 逐項相同、溢出 0／0、重疊 0／0 ⇒ 零位移。
共用 zoom-modal（戰鬥位 sheet › 查看詳情）在四種 iPad 直式：z 200 蓋在 .mp 之上、完整在視窗內（483.7 高）、不需捲動。
偽造量測 vs 真開關量測：四種 iPad 直式的 usedH（885／905）、sizes、overflow、overlap 逐項相同 ⇒ 「偽造 min(w,h)」等價於真開關（守衛 G1 的前提成立）。
...
- ~~19624/19625/19626（傳說的海溝／山頂／熔岩洞）官方同時掛在 M-P 與 M6 底下，
  ...
  ⚠⚠ **v6.328 訂正：這一句是錯的。** 官方快照的 `sets['M6'].ids` 只有 19621/19622/19623，
  `sets['M-P'].ids` 才有 19624/19625/19626（＝膽小蟲 209／超級米立龍ex 210／麻麻小魚 211）
  —— **從來就不是「同時掛在兩包」**，是本站 v6.093 造右半 id 時佔用了官方 id 空間。
  這句錯誤結論正是這件事被埋了一個多月的原因（詳見 v6.328 那一節）。
...
### 【五】⭐⭐⭐ 對抗性審查（Opus 5）抓到的**真回歸**：封存頁副標寫死「最近 50 次更新」

`static/changelog-archive.html:28` 的 `<p class="sub">`：
> 首頁只顯示最近 **50** 次更新；這裡保留全部紀錄…

那是玩家點「查看完整更新歷史」之後看到的**第一行**，首頁改成 35 則之後就直接對玩家講錯話。
⚠⚠ **當時沒有任何守衛蓋得到**：`F0c-2`／`F2` 只逐位元 pin `<details>` 區塊，
封存頁的 `<head>`／`<style>`／`<p class="sub">`／tail **完全在 pin 範圍之外**
（審查者的突變 M16／M17 竄改 head 與 style，六支相關守衛全綠）。

⇒ 修法兩件事：
1. 改成**不綁數字**的說法（沿用 v6.100 當年對首頁頁尾做過的處理）。
2. ⭐ **新增守衛 `test-v6264` A9**：首頁與封存頁的**非條目區**（head ＋ tail）
   不得出現 `最近 N 次更新／則／版` 這種寫死的則數，附正對照與反對照。
   ⚠ 刻意**不**做成「head 逐位元 pin」—— 那會讓任何合法的文案／樣式調整誤紅（安慰劑型態 9 的變體）。
   **只鎖真正會過期的東西。**
   ⭐ A9 一上就抓到**第二處**（`changelog-archive.html:1232` 的 HTML 註解「只保留最近 50 版」），
   而那一處連審查者都判斷「玩家看不到、可不動」⇒ 守衛比人可靠。

⭐⭐⭐ **這一輪最有價值的一句**：記憶裡從 v6.100 就寫著「**頁尾不要寫死數字**」，
但那條規則**從來沒有守衛化**，所以它在 v6.332 準時失效。
**寫在記憶／註解裡的規則，如果沒有守衛，就只是一句遲早會被忘記的話。**

### 【六】審查者其他指正（均已修）
- `MAX_KB` 死 export（見【三之二】）。
- 「50 被抄在三支守衛裡」實際是**五支**（敘述錯，實作端五支都改了）。
- `test-v6248` 的註解指向一個**不存在的 `GAP_TOL`**；實際上批次縮減搬的是**連續**的最舊一段，
  首頁最舊與封存最新仍相鄰 ⇒ 容差 5 自然成立，**不必放大**。已改正註解。
- 本檔的改動檔清單漏列五支守衛與新檔（已補）。
- `src/lib/changelog-lazy.ts` 與 `src/service-worker.ts` 的**現在式**過期註解（已改）；
  `src/routes/+page.svelte:92` 的「v6.100：只留最近 50 則」是**歷史陳述**，維持原樣。
- ⚠ **已知且接受的守備面縮小**：`test-changelog-size-and-archive` ⑥（技術用語黑名單）／⑬
  與 `test-v6110` 只掃首頁＋bodies，搬進封存頁的那 15 則從此不在掃描面內。
  那些是**凍結的歷史內容**、原本就綠，風險低；但這是真實的守備面縮小，記在這裡。

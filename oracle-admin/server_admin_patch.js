  ...
  try {
    ...
    app.post('/api/tournament/action', async (req, res) => {
      try {
        ...
        if (doc.matchId && newGs.turn !== gs.turn) snapshotTurn(doc.matchId, doc.eventId, newGs.turn, newGs);  // v0.81 回放:回合邊界快照(fire-and-forget,不 await,不影響回應)
    ...
    function snapshotTurn(matchId, eventId, turn, gs) {
      if (!REPLAY_SNAPSHOT_ENABLED || !matchId || !gs) return;
      try {
        const state = Object.assign({}, gs); state.log = [];  // strip log(佔~73%;逐回合文字由 finalLog 供)
        TREPLAY.updateOne({ _id: matchId + '_t' + turn }, { $setOnInsert: { matchId, eventId: eventId || null, turn, state, createdAt: Date.now(), expireAt: new Date(Date.now() + REPLAY_TTL_MS) } }, { upsert: true }).catch(() => { /* fire-and-forget,缺回合回放端 fallback */ });
      } catch (e) { /* 絕不影響對戰 */ }
    }
    ...
    app.get('/api/tournament/replay', async (req, res) => {
      try {
        ...
        const snaps = await TREPLAY.find({ matchId }).sort({ turn: 1 }).toArray();
        let fs = null; if (m.finalState) { fs = Object.assign({}, m.finalState); fs.log = []; }  // strip log(finalLog 另回)
        res.json({
          ...
          finalLog: Array.isArray(m.finalLog) ? m.finalLog : [],
          finalState: fs,
          snapshots: snaps.map((x) => ({ turn: x.turn, state: x.state })),
        });
      } catch (e) { res.status(500).json({ error: e.message }); }
    });

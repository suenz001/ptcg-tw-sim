...
(async () => {
  try {
    ...
    //   ══ v1.41（v6.295）好友備註名 ＋ 可信的「最新暱稱」（守衛 scripts/test-v6295-friends-alias-nick.mjs 行為端實跑）══
    //     【A】備註名（LINE 那種「我自己幫這位好友取的名字」）：存 friendships **同一列**的 aliasByA／aliasByB
    //         （依既有 a/b 排序語義：a 那一側的人給對方取的名字存 aliasByA）。新端點 POST /api/friends/alias {fid, alias}。
    //       ⚠⚠⚠ **對方絕對看不到我給他取的備註名** —— 這是本版最重要的安全性質：
    //         ・_frPublic 只回「我這一側」那一個欄位（doc.a === me ? aliasByA : aliasByB），對方那一側連讀都不讀；
    //         ・admin 端點（_frDmAdminRow／dm 總覽／dm 展開）用的是**逐欄 projection**，一律不含 alias
    //           （站長不需要看玩家的私人備註，也避免誤洩）。
    //       ⚠ 只准 status === 'accepted'；被封鎖方一律靜默 200 且 DB 一個位元都不動（與 accept／reject／remove 同紀律）。
    //       ⚠ 空字串／全空白 ＝ 清除備註名（$unset），且**只 $unset 我自己那一側**，絕不碰對方的欄位。
    //     【B】「最後一次使用的暱稱」：唯一可信來源＝tournamentRegistrations（TREGS）以 **email** 為鍵取最新一筆。
    //       ⭐ 自己重新查證過（2026-09-03，BASE eba051fc）：
    //         ・TREGS 的三支寫入點（/register、/register-and-checkin、社群賽 /propose 的發起者自動報名）
    //           都寫 `email: id.email || null`，而 `tournIdentity` **只有 Firebase ID token 驗過**那條分支才給 email；
    //           未驗證的 playerId fallback 寫死 `email: null`。
    //         ・掃過本檔全部 179 個歷史版本：有 tournIdentity 的 161 個版本**全部**是
    //           `if (pid) return { uid: String(pid), email: null, ...`；另外 18 個版本連 tournamentRegistrations 都還不存在。
    //         ⇒ **冒名報名寫進去的列 email 必定是 null**，用 email 當鍵一筆都撈不到它們
    //           ⇒ v6.291 之前的舊資料同樣安全，**不需要時間下限**（加了反而砍掉老玩家真正的最新暱稱）。
    //       ⚠ TREGS 的 email 沒有正規化（直接來自 token），本查詢走等值 $in ⇒ 大小寫不同就查無 ⇒ 退回快照（fail-graceful）。
    //       ⚠⚠ TREGS 只有 {uid:1}／{eventId:1} 兩個索引（v6.119），**沒有 email 索引** ⇒ 本版 lazy 建
    //         {email:1, registeredAt:-1, name:1}（把 name 也放進去 ⇒ 查詢被索引涵蓋，不必 FETCH 帶著 60 張牌表的大 doc），
    //         並比照 v6.266 deckStatsIndexReady 做**索引自驗**：查不到索引就整段跳過（暱稱退回快照），**絕不 COLLSCAN**。
    //       ⚠⚠ 絕不拖累：list 已有 FR_LIST_CAP 上限，TREGS 是**一發** $in 批次查詢（絕不 per-friend 各查一次），
    //         逐筆迴圈掛中央 adminScanYield。
    //     ⭐ list 的顯示優先序＝① 我給的 alias → ② TREGS 最新報名暱稱 → ③ friendships 的 nickA/nickB 快照 → ④「玩家」；
    //       ②③④ 收在 `nick` 這一個欄位，① 另外用 `alias` 欄位回傳（**伺服器不合併**，下一版 UI 才能顯示「原暱稱：xxx」）。
    //     ⚠⚠ 絕不退回吃 playerIdentity.nick（v6.286【2】的裁定：那張表由零身分驗證的 /api/match-result 寫入）。
    //   ══ v1.44（v6.300）好友清單多回一個布林 `inTournament`（守衛 scripts/test-v6300-friends-intournament.mjs 行為端實跑）══
    //     站長需求：好友選單要加【加入房間】鈕；好友正在**錦標賽**對戰時不給加入，改提示「正在錦標賽對戰中」。
    //     ⭐ 站長裁定：本版**只做伺服器端這一個布林**（打開清單當下的快照即可，不要即時）；按鈕與提示是下一版 client 的事。
    //   ⚠⚠⚠ 隱私：`inTournament` **只是布林** —— 絕不回賽事名稱／對手／房號／matchId／uid。
    //     只有 status === 'accepted' 的關係才帶這個欄位（被封鎖／待確認／送出中一律不帶，維持既有可見度規則）。
    //   ⭐⭐ 怎麼從 email 走到「正在錦標賽對戰中」（⚠ 這裡刻意走**反方向**，理由見下）：
    //     ① tournamentEvents `{ status: 'running' }`         → 進行中的賽事 id（既有索引 `{status:1}`）
    //     ② tournamentMatches `{ eventId: $in, status: 'playing' }` → 正在打的 p1uid/p2uid（既有索引 `{eventId:1,status:1}`）
    //     ③ tournamentRegistrations `{ eventId: $in, uid: $in }`    → uid → email（既有索引 `{eventId:1}`）
    //   ⚠⚠ **為什麼不是 email → uid**：v6.295 建的 TREGS 索引是 `{email:1, registeredAt:-1, name:1}`，**不含 uid**
    //     ⇒ 拿 email 去查 uid 會 FETCH，而 TREGS 每一列都帶 60 張牌表、且是玩家的**全部歷史報名**（無上限）。
    //     反過來從「進行中賽事」出發，被 `{eventId:1}` 索引框住 ⇒ 只碰到這幾場賽事的報名列（≈ 賽事人數），
    //     **完全不必動任何索引**（本版一支索引都沒有新增，v6.295 的 D8 逐字斷言不受影響）。
    //   ⚠⚠ 絕不 N+1：三發查詢的次數與好友人數**完全無關**（100 個好友仍是 1／1／1 發），
    //     而且沒有賽事在進行時 ① 之後直接收工（②③ 零查詢）—— 那是絕大多數時間的情況。
    //   ⚠⚠ 絕不新增輪詢：只搭在既有的 GET /api/friends/list 那一發上（v6.118 的教訓：
    //     `game/+page.svelte` 的 `!isTournament` gate 就是「每個開著錦標賽頁的玩家每 2 秒兩支 /api/rooms」的事故修正）。
    //     再加一層**全站共用的 5 秒快照**（FR_TOURN_TTL_MS）＋ in-flight 合併 ⇒ 同時段多少人開清單都只打一次。
    //   ⚠⚠ 索引自驗（比照 v6.266 deckStatsIndexReady／v6.295 _frRegIndexReady）：三支索引**任一不在 ⇒ 整段跳過**，
    //     `inTournament` 一律 false，**一次查詢都不發**（絕不 COLLSCAN 這三張永久累積的大表）。
    //   ⚠ 查不到 uid→email 對照（例如那筆報名的 email 是 null＝未驗證身分的 playerId fallback 寫的）⇒ false（fail-closed）。
    //     TREGS 的 email 沒有正規化 ⇒ 這裡一律過 `_frNormEmail`（trim＋小寫）再比對。
    //   ⚠ 只認 `status: 'playing'`（已配到房間、真的在打）；`pending`＝已配對但還沒進場，不算「正在對戰中」。
    //   == v1.45（v6.302）好友清單多回 `roomId`（守衛 scripts/test-v6302-friend-room-by-email.mjs 行為端實跑）==
    //     ⚠⚠ 站長實測回報：好友明明在一般休閒對戰中，好友列的【🚪 加入房間】鈕卻是暗的、按不下去。
    //     ⭐ 複驗過的真因（v6.301 是**純瀏覽器端**比對：拿 playerIdentity 的 uid 去對 openRooms[].seats[].uid）：
    //       ① `recordPlayerIdentity` 的**唯一**接點是 /api/match-result 的 PTCG-MATCH-EMAIL-ENRICH 段
    //          ⇒ 一個人的 uid **只有在「一場對局結束」時**才會被寫進對照表；
    //       ② 正式站 seats[].uid 是 **Oracle 匿名 JWT 的 per-瀏覽器 uid**（oracle-client.ts 的
    //          /api/auth/anonymous ＋ localStorage `ptcg_oracle_uid`）⇒ 換裝置／清資料／401 續簽都會換一個；
    //       ③ `playerIdentity` 2026-09-02（v6.282）才上線，當時資料只累積了兩天。
    //       ⇒ 拿「上一場結束時記到的 uid」去比對「這一場正在進行的房間」，命中率本來就很差。
    //     ⭐⭐ 站長裁定的修法：**改由伺服器用 email 比對**。伺服器手上一直都有 `rooms.seats[].email`
    //       （v1.20／v6.220 只是不再**下發**給瀏覽器，DB 裡從來沒少過），而好友關係本來就以 email 為 key
    //       ⇒ 完全不受 uid 漂移影響。client 拿到 roomId 之後自己去 `openRooms` 找那一間房，
    //       房名／房主／status（lobby／playing）都是**當下這一秒**的值（大廳本來就每 2 秒更新一次）。
    //     ⚠⚠⚠ 隱私：回應**只多一個 `roomId` 字串** —— 絕不回房名／房主／對手／任何 email。
    //       房名與房主一律由 client 從它自己的 `openRooms` 取（那份資料它本來就有）。
    //     ⚠ 只有 status === 'accepted' 的關係才帶（維持既有可見度規則，與 `inTournament` 同一條紀律）。
    //     ⭐⭐ **「欄位不存在」與「欄位是 null」語義不同**（client 就是靠這個分辨的）：
    //       ・**欄位缺席** ＝ 伺服器沒有能力回答（舊伺服器／本版索引自驗沒過／查詢出錯）
    //         ⇒ client 退回 v6.301 的 uid 比對（那只是相容退路，不是常態路徑）；
    //       ・**null** ＝ 伺服器查過了，這位好友此刻**不在**任何 lobby／playing 房 ⇒ client 直接把按鈕灰掉。
    //       ⇒ 快照拿不到（`_frRoomsByEmail` 回 null）時**一個 roomId 欄位都不寫**，絕不寫成 null。
    //     ⭐ 查詢＝**大廳列表端點本來就在打的同一條**（v1.17 的 `_roomsCombinedMw`）：
    ...
    //       ⚠⚠ projection 只取 `_id` 與 `seats.email` —— **絕不撈 `gameState`**（v6.240 的教訓：
    //         房間 doc 的大宗是 gameState.log，第 9 回合就約 29KB，整批讀進 node ＝ 阻塞事件迴圈）。
    //     ⚠⚠ 絕不 N+1：**固定一發**查詢，次數與好友人數完全無關（100 個好友仍然只有 1 發）。
    //     ⭐ 沿用 v6.300 的全站共用 5 秒快照 ＋ in-flight 合併 ⇒ 同時段多少人開好友清單都只打一次。
    //     ⚠⚠ 絕不新增輪詢：只搭在既有的 GET /api/friends/list 那一發上（v6.118 的教訓）。
    //     ⚠ 只看 p1／p2 兩個座位（與 client 的 `FRIEND_SEAT_SLOTS` 一致）：座位 2 以後是觀戰位，
    //       好友在觀戰位時他自己也只是旁觀者，把那種情況標成「他在這間房」會誤導。
    //     ⚠⚠⚠ **安全性並沒有變好**（⭐ 更正 v6.301 `src/lib/friends/friend-rooms.ts` 註解裡那一段的說明）：
    ...
    //       ⇒「有人把自己房間的座位填上別人的 email、讓那間房被標成好友的房」這件事**仍然做得到**，
    //       而且 email 比不透明的 Oracle uid **更容易知道**。
    //       ⇒ **配對到的房間一定要把房名與房主名顯示出來**讓玩家自己判斷（站長裁定，也是好體驗）——
    //         這條規定不因為換成 email 比對而放寬。
    ...
    // ⭐v1.41【B】「最後一次使用的暱稱」的唯一可信來源（理由見區塊開頭）。⚠ 用字面而不是引用錦標賽區塊的 TREGS 常數：
    //   那個常數宣告在本區塊**之後**，而守衛會單獨抽本區塊出來實跑（引用會 TDZ／ReferenceError）——
    //   與 _frPurgeDm 直接用字面 'tournamentChat' 是同一條紀律。
    const FR_REG_COLL = 'tournamentRegistrations';
    const FR_REG_IDX = { email: 1, registeredAt: -1, name: 1 };   // ⭐ 含 name ⇒ 查詢被索引涵蓋，不必 FETCH 帶 60 張牌表的大 doc
    const FR_REG_IDX_KEY = '{"email":1,"registeredAt":-1,"name":1}';
    const FR_REG_IDX_TTL_MS = 60000;    // 索引自驗：成功結果快取 60 秒
    const FR_REG_IDX_RETRY_MS = 10000;  // 索引自驗：失敗退避 10 秒（剛開機索引可能還在建）
    const FR_ALIAS_MAX_LEN = 20;        // ⭐v1.41【A】備註名上限（站長裁定）
    // ⭐v1.44【v6.300】「好友正在錦標賽對戰中」：同樣用字面而不是引用錦標賽區塊的 TEVENTS／TMATCH／TREGS 常數
    //   （那些宣告在本區塊**之後** ⇒ 引用會 TDZ；守衛也會單獨抽本區塊出來實跑）。
    const FR_EV_COLL = 'tournamentEvents';
    const FR_MATCH_COLL = 'tournamentMatches';
    const FR_EV_IDX_KEY = '{"status":1}';                  // 既有索引（錦標賽區塊建）
    const FR_MATCH_IDX_KEY = '{"eventId":1,"status":1}';   // 既有索引（錦標賽區塊建）
    const FR_REG_EV_IDX_KEY = '{"eventId":1}';             // 既有索引（錦標賽區塊建）
    const FR_TOURN_TTL_MS = 5000;      // 全站共用快照：站長裁定「打開時的快照即可，不要即時」
    const FR_TOURN_EV_CAP = 20;        // 同時進行中的賽事硬上限
    const FR_TOURN_MATCH_CAP = 500;    // 進行中賽事的 playing 對戰硬上限
    const FR_TOURN_REG_CAP = 2000;     // 進行中賽事的報名列硬上限
    // ⭐v1.45【v6.302】「好友此刻所在的休閒房」：沿用大廳列表端點的同一條查詢與同一條**既有**索引。
    const FR_ROOM_COLL = 'rooms';
    const FR_ROOM_IDX_KEY = '{"status":1,"updatedAt":-1}';   // 既有索引（v1.22／v6.240 在服務啟動時建，本版不新增）
    const FR_ROOM_STATUSES = ['lobby', 'playing'];           // ⚠ 與 _roomsCombinedMw 的 $in 逐字相同
    const FR_ROOM_CAP = 100;                                 // ⚠ 與大廳列表端點的 .limit(100) 同一個上限
    const FR_ROOM_SEAT_SLOTS = 2;                            // ⚠ 只看 p1／p2（與 client 的 FRIEND_SEAT_SLOTS 一致）
    const FR_ROOM_TTL_MS = 5000;                             // 全站共用快照（沿用 v6.300 的 5 秒）
    ...
    try {
      ...
      db.collection(FR_REG_COLL).createIndex({ email: 1, registeredAt: -1, name: 1 }).catch(() => { /* best-effort，已存在即略過 */ });
    } catch (e) { /* db 尚未就緒時略過，不影響服務啟動 */ }
    ...
    /**
     * ⭐v1.41【A】備註名淨化。回 '' 代表「清除備註名」（呼叫端 $unset）。
     *   ・只收字串（數字／物件一律當空 ⇒ 清除，不讓 '[object Object]' 落地，比照 _frDmText）；
     *   ・控制字元（C0／DEL／C1）換成空白後再折疊 —— 換行會把名單排版打爛；
     *   ・**零寬字元**（U+200B~U+200F／U+2060／U+FEFF）直接刪除 —— 看不見卻會讓字串永遠不相等（v6.116 教訓）；
     *   ・\s+ 折成單空白、trim；
     *   ・上限 FR_ALIAS_MAX_LEN 字，用 Array.from 以**碼位**計數，不會把 emoji 從中間切成半個。
     */
    function _frAlias(v) {
      if (typeof v !== 'string') return '';
      const t = v.replace(/[\u0000-\u001f\u007f-\u009f]/g, ' ').replace(/[\u200b-\u200f\u2060\ufeff]/g, '').replace(/\s+/g, ' ').trim();
      return t ? Array.from(t).slice(0, FR_ALIAS_MAX_LEN).join('') : '';
    }
    ...
    function _frPublic(doc, me, ident) {
      ...
      return {
        ...
        // ⚠⚠⚠ 只回**我這一側**的備註名（a 那一側的人取的名字放 aliasByA）。對方那一側連讀都不讀
        //   ⇒ 對方永遠看不到我給他取的名字（守衛 test-v6295【A3】兩側哨兵互掃）。
        alias: _frAlias(doc.a === me ? doc.aliasByA : doc.aliasByB) || null,
    ...
    // ── ⭐v1.41【B】索引自驗（比照 v6.266 deckStatsIndexReady）────────────────────────
    //   ⚠⚠ **沒有索引就不可以查**：tournamentRegistrations 是永久累積、而且每一列都帶 60 張牌表的大 collection，
    //   全表掃會把它整批吃進 node（＝阻塞事件迴圈，錦標賽與休閒對戰都在同一個行程）。
    //   成功快取 60 秒、失敗退避 10 秒（剛開機索引可能還在建，不要一次失敗就整天停用，也不要每發都打）。
    let _frRegIdxOk = { at: 0, ok: false };
    async function _frRegIndexReady() {
      const now = Date.now();
      if (_frRegIdxOk.ok && now - _frRegIdxOk.at < FR_REG_IDX_TTL_MS) return true;
      if (!_frRegIdxOk.ok && _frRegIdxOk.at && now - _frRegIdxOk.at < FR_REG_IDX_RETRY_MS) return false;
      try {
        const list = await db.collection(FR_REG_COLL).indexes();
        const ok = (list || []).some((i) => i && JSON.stringify(i.key) === FR_REG_IDX_KEY);
        _frRegIdxOk = { at: now, ok };
        if (!ok) console.warn('[friends] 缺 ' + FR_REG_COLL + ' ' + FR_REG_IDX_KEY + ' 索引 ⇒ 最新暱稱這一段自我停用（退回快照，絕不全表掃）');
        return ok;
      } catch (e) { _frRegIdxOk = { at: now, ok: false }; return false; }
    }
    /**
     * ⭐v1.41【B】批次取「每個 email 最後一次報名時填的暱稱」。
     *   ⚠⚠ **一發** aggregate（$in 陣列），絕不 per-friend 各查一次（N+1）；hint 綁死上面那支索引 ⇒ 走索引掃描。
     *   ⚠ 索引不在 ⇒ 回空 Map（呼叫端自然退回 friendships 快照）—— fail-closed，不是 fail-open 去掃全表。
     *   ⚠ 取回的 name 一律過 _frAuthNick（＝不得等於自己 email 的前綴）—— 全站「絕不用 email 前綴當顯示名」的同一條紀律。
     *   @param {string[]} emails 已正規化（小寫）的對方 email，長度 ≤ FR_LIST_CAP
     *   @param {(n:number)=>Promise|null} y 中央讓路節拍
     */
    async function _frLatestNicks(emails, y) {
      const out = new Map();
      if (!Array.isArray(emails) || !emails.length) return out;
      if (!(await _frRegIndexReady())) return out;
      const rows = await db.collection(FR_REG_COLL).aggregate([
        { $match: { email: { $in: emails } } },
        { $sort: { email: 1, registeredAt: -1 } },
        { $group: { _id: '$email', name: { $first: '$name' } } },
      ], { hint: FR_REG_IDX, allowDiskUse: false }).toArray();
      let k = 0;
      for (const r of (rows || [])) {
        const key = (r && typeof r._id === 'string') ? r._id.trim().toLowerCase() : null;
        const nm = key ? _frAuthNick(r.name, key) : null;
        if (key && nm) out.set(key, nm);
        const w = y(++k); if (w) await w;
      }
      return out;
    }
    // ── ⭐v1.44【v6.300】「正在錦標賽對戰中」：索引自驗 ＋ 三發固定查詢 ＋ 全站共用 5 秒快照 ──────
    //   ⚠⚠ **三支索引任一不在就不可以查**：tournamentEvents／tournamentMatches／tournamentRegistrations
    //   都是永久累積（TREGS 每列還帶 60 張牌表），全表掃會把它們整批吃進 node ＝ 阻塞事件迴圈，
    //   而錦標賽與休閒對戰都在同一個行程（pm2 fork_mode 單 instance）。
    //   成功快取 60 秒、失敗退避 10 秒（沿用 v6.295 的兩個常數，剛開機索引可能還在建）。
    let _frTournIdxOk = { at: 0, ok: false };
    async function _frTournIdxReady() {
      const now = Date.now();
      if (_frTournIdxOk.ok && now - _frTournIdxOk.at < FR_REG_IDX_TTL_MS) return true;
      if (!_frTournIdxOk.ok && _frTournIdxOk.at && now - _frTournIdxOk.at < FR_REG_IDX_RETRY_MS) return false;
      let ok = false, miss = '';
      try {
        const has = async (coll, key) => ((await db.collection(coll).indexes()) || []).some((i) => i && JSON.stringify(i.key) === key);
        if (!(await has(FR_EV_COLL, FR_EV_IDX_KEY))) miss = FR_EV_COLL + ' ' + FR_EV_IDX_KEY;
        else if (!(await has(FR_MATCH_COLL, FR_MATCH_IDX_KEY))) miss = FR_MATCH_COLL + ' ' + FR_MATCH_IDX_KEY;
        else if (!(await has(FR_REG_COLL, FR_REG_EV_IDX_KEY))) miss = FR_REG_COLL + ' ' + FR_REG_EV_IDX_KEY;
        else ok = true;
      } catch (e) { ok = false; miss = '（indexes() 失敗）'; }
      _frTournIdxOk = { at: now, ok };
      if (!ok) console.warn('[friends] 缺 ' + miss + ' 索引 ⇒ 「正在錦標賽對戰中」這一段自我停用（inTournament 一律 false，絕不全表掃）');
      return ok;
    }
    /**
     * ⭐v1.44【v6.300】掃出「此刻正在錦標賽對戰中」的 email 集合。
     *   ⚠⚠ 三發**固定**查詢（與好友人數完全無關）；沒有賽事在進行 ⇒ ② ③ 零查詢。
     *   ⚠ 索引不在 ⇒ 回 null（呼叫端一律 false）—— fail-closed，不是 fail-open 去掃全表。
     *   @param {(n:number)=>Promise|null} y 中央讓路節拍
     */
    async function _frTournPlayingScan(y) {
      if (!(await _frTournIdxReady())) return null;
      const evs = await db.collection(FR_EV_COLL).find({ status: 'running' }, { projection: { _id: 1 } }).limit(FR_TOURN_EV_CAP).toArray();
      const evIds = (evs || []).map((e) => e && e._id).filter((v) => v !== undefined && v !== null);
      if (!evIds.length) return new Set();   // 沒有賽事在進行 ⇒ 全部 false（② ③ 一發都不打）
      const ms = await db.collection(FR_MATCH_COLL).find({ eventId: { $in: evIds }, status: 'playing' },
        { projection: { p1uid: 1, p2uid: 1 } }).limit(FR_TOURN_MATCH_CAP).toArray();
      const uids = new Set();
      let k = 0;
      for (const m of (ms || [])) {
        if (m && typeof m.p1uid === 'string' && m.p1uid) uids.add(m.p1uid);
        if (m && typeof m.p2uid === 'string' && m.p2uid) uids.add(m.p2uid);
        const w = y(++k); if (w) await w;   // ⚠ 每 200 筆讓路（v6.242）
      }
      if (!uids.size) return new Set();
      const regs = await db.collection(FR_REG_COLL).find({ eventId: { $in: evIds }, uid: { $in: [...uids] } },
        { projection: { uid: 1, email: 1 } }).limit(FR_TOURN_REG_CAP).toArray();
      const out = new Set();
      let j = 0;
      for (const r of (regs || [])) {
        // ⚠ 再驗一次 uid（$in 是索引外的殘餘過濾，這裡不靠 mongo 幫我保證）＋ email 正規化後才進集合。
        if (r && typeof r.uid === 'string' && uids.has(r.uid)) {
          const e = _frNormEmail(r.email);
          if (e) out.add(e);
        }
        const w = y(++j); if (w) await w;   // ⚠ 每 200 筆讓路（v6.242）
      }
      return out;
    }
    let _frTournSnap = { at: 0, emails: null };
    let _frTournInflight = null;
    /** 全站共用的 5 秒快照 ＋ in-flight 合併：同時段多少人開好友清單，都只打一輪那三發查詢。 */
    async function _frTournPlayingEmails(y) {
      const now = Date.now();
      if (_frTournSnap.at && now - _frTournSnap.at < FR_TOURN_TTL_MS) return _frTournSnap.emails;
      if (_frTournInflight) return _frTournInflight;
      _frTournInflight = (async () => {
        let emails = null;
        try { emails = await _frTournPlayingScan(y); } catch (e) { emails = null; }   // ⚠ 出錯一律 fail-closed
        _frTournSnap = { at: Date.now(), emails };
        _frTournInflight = null;
        return emails;
      })();
      return _frTournInflight;
    }
    // ── ⭐v1.45【v6.302】「好友此刻所在的休閒房」：索引自驗 ＋ 一發固定查詢 ＋ 全站共用 5 秒快照 ──────
    //   ⚠⚠ **沒有索引就不可以查**：rooms 已累積 8 萬多筆（v6.240 實測），而這條查詢帶 sort
    //   ⇒ 沒索引會退化成整集合掃描＋記憶體排序，那是把休閒對戰與錦標賽一起卡住
    //   （pm2 fork_mode 單 instance，同一個 node 行程）。
    //   ⚠⚠ 索引不在時回 **null**，而呼叫端會因此**完全不寫 roomId 欄位** ⇒ client 退回 v6.301 的 uid 比對，
    //     **不是**把所有好友的按鈕都灰掉。這正是「欄位不存在」與「欄位是 null」必須分開的理由。
    //   成功快取 60 秒、失敗退避 10 秒（沿用 v6.295／v6.300 的兩個常數，剛開機索引可能還在建）。
    let _frRoomIdxOk = { at: 0, ok: false };
    async function _frRoomIdxReady() {
      const now = Date.now();
      if (_frRoomIdxOk.ok && now - _frRoomIdxOk.at < FR_REG_IDX_TTL_MS) return true;
      if (!_frRoomIdxOk.ok && _frRoomIdxOk.at && now - _frRoomIdxOk.at < FR_REG_IDX_RETRY_MS) return false;
      let ok = false;
      try {
        const list = await db.collection(FR_ROOM_COLL).indexes();
        ok = (list || []).some((i) => i && JSON.stringify(i.key) === FR_ROOM_IDX_KEY);
      } catch (e) { ok = false; }
      _frRoomIdxOk = { at: now, ok };
      if (!ok) console.warn('[friends] 缺 ' + FR_ROOM_COLL + ' ' + FR_ROOM_IDX_KEY + ' 索引 ⇒ 「好友所在的房間」這一段自我停用（回應不帶 roomId ⇒ client 退回舊比對，絕不 COLLSCAN）');
      return ok;
    }
    /**
     * ⭐v1.45【v6.302】掃出「email → 此刻所在的休閒房房號」。
     *   ⚠⚠ **一發**固定查詢（與好友人數完全無關）；projection 只取 _id 與 seats.email，**絕不撈 gameState**。
     *   ⚠ 索引不在 ⇒ 回 null（呼叫端**不寫欄位**）—— fail-graceful，不是 fail-open 去掃全表。
     *   ⚠ 只看 p1／p2 兩個座位；同一個 email 理論上不會同時坐兩間房，真的重複時採「先到先得」，
     *     而查詢已依 updatedAt 由新到舊排序 ⇒ 留下的是最近有活動的那一間，行為穩定不抖動。
     *   ⚠ 逐座位走中央 adminScanYield（100 房 × 2 座位 ＝ 200 ⇒ 滿載時正好讓路一次）。
     *   @param {(n:number)=>Promise|null} y 中央讓路節拍
     */
    async function _frRoomScan(y) {
      if (!(await _frRoomIdxReady())) return null;
      const rooms = await db.collection(FR_ROOM_COLL)
        .find({ status: { $in: FR_ROOM_STATUSES } }, { projection: { _id: 1, 'seats.email': 1 } })
        .limit(FR_ROOM_CAP).sort({ updatedAt: -1 }).toArray();
      const out = new Map();
      let k = 0;
      for (const r of (rooms || [])) {
        const code = (r && (typeof r._id === 'string' || typeof r._id === 'number')) ? String(r._id) : '';
        const seats = (r && Array.isArray(r.seats)) ? r.seats : [];
        const n = code ? Math.min(FR_ROOM_SEAT_SLOTS, seats.length) : 0;
        for (let i = 0; i < n; i++) {
          const e = _frNormEmail(seats[i] && seats[i].email);
          if (e && !out.has(e)) out.set(e, code);
          const w = y(++k); if (w) await w;   // ⚠ 每 200 個座位讓路（v6.242）
        }
      }
      return out;
    }
    let _frRoomSnap = { at: 0, byEmail: null };
    let _frRoomInflight = null;
    /** 全站共用的 5 秒快照 ＋ in-flight 合併：同時段多少人開好友清單，都只打一發那個 rooms 查詢。 */
    async function _frRoomsByEmail(y) {
      const now = Date.now();
      if (_frRoomSnap.at && now - _frRoomSnap.at < FR_ROOM_TTL_MS) return _frRoomSnap.byEmail;
      if (_frRoomInflight) return _frRoomInflight;
      _frRoomInflight = (async () => {
        let m = null;
        try { m = await _frRoomScan(y); } catch (e) { m = null; }   // ⚠ 出錯一律當「答不出來」⇒ 呼叫端不寫欄位
        _frRoomSnap = { at: Date.now(), byEmail: m };
        _frRoomInflight = null;
        return m;
      })();
      return _frRoomInflight;
    }
    ...
    app.get('/api/friends/list', async (req, res) => {
      try {
        ...
        const cur = c.find(q, { projection: { _id: 1, a: 1, b: 1, status: 1, requester: 1, blockedBy: 1, nickA: 1, nickB: 1, aliasByA: 1, aliasByB: 1, addedVia: 1, createdAt: 1, updatedAt: 1, fid: 1 } }).limit(FR_LIST_CAP);
        const docs = []; let n = 0;
        ...
        // ⭐v1.41【B】「最後一次使用的暱稱」：一發批次查詢（⚠ 絕不 per-friend 各查一次）；沒索引 ⇒ 回空 Map ⇒ 全部退回快照。
        const latest = await _frLatestNicks(others, me.yield);
        // ⭐v1.44【v6.300】「正在錦標賽對戰中」：全站共用 5 秒快照（三發固定查詢，與好友人數無關；索引缺席 ⇒ null ⇒ 一律 false）。
        const tplay = await _frTournPlayingEmails(me.yield);
        // ⭐v1.45【v6.302】「這位好友此刻所在的休閒房」：全站共用 5 秒快照（一發固定查詢，與好友人數無關）。
        //   ⚠⚠ 回 null ＝ 伺服器答不出來（索引沒過／查詢出錯）⇒ 底下**一個 roomId 欄位都不寫**
        //     ⇒ client 退回 v6.301 的 uid 比對；有 Map ⇒ 查不到的人明確寫 null（client 據此把按鈕灰掉）。
        const rmap = await _frRoomsByEmail(me.yield);
        const friends = [], incoming = [], outgoing = [], blocked = [];
        for (const d of docs) {
          const p = _frPublic(d, me.email, ident);
          // ⭐v1.41 顯示優先序：① p.alias（我給的備註名，**伺服器不合併**，留給 UI 套用並同時顯示「原暱稱」）
          //   → ② TREGS 最新報名暱稱 → ③ friendships 的 nickA/nickB 快照（_frPublic 已填）→ ④「玩家」（_frPublic 已填）。
          const _oe = d.a === me.email ? d.b : d.a;
          const _ln = latest.get(_oe);
          if (_ln) p.nick = _ln;
          // ⭐v1.44【v6.300】⚠⚠ 只有 accepted 才帶 inTournament（被封鎖／待確認／送出中一律不帶這個欄位）；
          //   ⚠⚠⚠ 只回布林，絕不回賽事名稱／對手／房號／matchId／uid。
          if (d.status === 'accepted') {
            p.inTournament = !!(tplay && tplay.has(_oe));
            // ⭐v1.45【v6.302】⚠⚠⚠ 只回**房號字串**，絕不回房名／房主／對手／任何 email。
            //   ⚠⚠ 「欄位不存在」≠「欄位是 null」：rmap 為 null（伺服器答不出來）⇒ 連 key 都不寫；
            //     rmap 有值但這位好友不在任何 lobby／playing 房 ⇒ 明確寫 null。
    ...
    // POST /api/friends/alias {fid, alias}：⭐v1.41【A】我自己幫這位好友取的備註名（LINE 那種）。
    //   ⚠⚠⚠ **對方絕對看不到** —— 只寫／只讀我這一側的欄位（a 那一側寫 aliasByA），_frPublic 也只回我這一側。
    //   ⚠ 只准 accepted（pending／rejected／我封鎖的都不是「好友」⇒ 409）；被封鎖方靜默 200 且零寫入。
    //   ⚠ 空字串／全空白／非字串 ⇒ $unset **我自己那一側**（清除備註名），絕不動對方的欄位、也絕不動 status／nickA／nickB。
    app.post('/api/friends/alias', async (req, res) => {
      try {
        ...
        const cur = await _frFindMine(c, me.email, req.body && req.body.fid);
        if (!cur) return res.status(404).json({ error: '找不到這位好友', code: 'friends-not-found', friendsApi: 1 });
        if (cur.status === 'blocked' && cur.blockedBy !== me.email) return res.json({ ok: true, friendsApi: 1 });   // 靜默：一個位元都不寫
        if (cur.status !== 'accepted') return res.status(409).json({ error: '只能為好友設定備註名', code: 'friends-alias-not-friend', friendsApi: 1 });
        const alias = _frAlias(req.body && req.body.alias);
        const field = cur.a === me.email ? 'aliasByA' : 'aliasByB';
        const now = Date.now();
        if (alias) await c.updateOne({ _id: cur._id, status: 'accepted' }, { $set: { [field]: alias, updatedAt: now } });
        else await c.updateOne({ _id: cur._id, status: 'accepted' }, { $unset: { [field]: '' }, $set: { updatedAt: now } });
        res.json({ ok: true, friendsApi: 1, fid: cur.fid || _frFid(cur._id), alias: alias || null });
      } catch (e) { _frFail(res, e, 'alias'); }
    });
    ...
    console.log('[friends] endpoints registered (v1.45) enabled-by-default=false');
  ...
  //     守衛已補一條 acorn 作用域斷言（定義的函式鏈必須是每個呼叫點的前綴）。
    ...
    app.post('/api/tournament/unregister', async (req, res) => {
      try {
        const id = await tournIdentity(req);
        if (id.error) return res.status(id.code || 401).json({ error: id.error });
        if (tournRequireVerified(id, res, 'unregister')) return;   // ⭐⭐⭐v6.292 冒名閘（helper 定義在 tournIdentity 正下方，v6.291）
        const ev = await resolveEventFromReq(req);
    ...
    app.post('/api/tournament/cancel-proposal', async (req, res) => {
      try {
        const id = await tournIdentity(req);
        if (id.error) return res.status(id.code || 401).json({ error: id.error });
        if (tournRequireVerified(id, res, 'cancel-proposal')) return;   // ⭐⭐⭐v6.292 冒名閘（helper 定義在 tournIdentity 正下方，v6.291）
        const ev = await resolveEventFromReq(req);
    ...
    app.post('/api/tournament/chat', async (req, res) => {
      try {
        const id = await tournIdentity(req);
        if (id.error) return res.status(id.code || 401).json({ error: id.error });
        if (tournRequireVerified(id, res, 'chat')) return;   // ⭐⭐⭐v6.292 冒名閘（helper 定義在 tournIdentity 正下方，v6.291）
    ...
    app.post('/api/tournament/match/enter', async (req, res) => {
      try {
        const id = await tournIdentity(req);
        if (id.error) return res.status(id.code || 401).json({ error: id.error });
        if (tournRequireVerified(id, res, 'match/enter')) return;   // ⭐⭐⭐v6.292 冒名閘（helper 定義在 tournIdentity 正下方，v6.291）
    ...
    app.post('/api/tournament/match/forfeit', async (req, res) => {
      try {
        const id = await tournIdentity(req);
        if (id.error) return res.status(id.code || 401).json({ error: id.error });
        if (tournRequireVerified(id, res, 'match/forfeit')) return;   // ⭐⭐⭐v6.292 冒名閘（helper 定義在 tournIdentity 正下方，v6.291）
    ...
    app.post('/api/tournament/drop', async (req, res) => {
      try {
        const id = await tournIdentity(req);
        if (id.error) return res.status(id.code || 401).json({ error: id.error });
        if (tournRequireVerified(id, res, 'drop')) return;   // ⭐⭐⭐v6.292 冒名閘（helper 定義在 tournIdentity 正下方，v6.291）
        const ev = await resolveEventFromReq(req);
        if (!ev) return res.status(409).json({ error: '目前沒有賽事' });

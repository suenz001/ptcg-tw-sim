  ...
  try {
    ...
    app.get('/api/tournament/state', async (req, res) => {
      try {
        ...
        res.json({ gameState: _stateForSeat(doc.gameState, _vseat), version: doc.version, seats: doc.seats, names: doc.names, actorSeat: _actorSeat, waiting: !doc.gameState, lastActionAt: doc.lastActionAt || null, idleForfeitMin: doc.idleForfeitMin || 3, serverNow: Date.now() });
      } catch (e) { res.status(500).json({ error: e.message }); }
    });

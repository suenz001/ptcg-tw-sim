...
import('firebase-admin').then(async ({ default: admin }) => {
  ...
  // ── v1.19 (v6.219) users 統計快取（/api/admin/stats 提速）────────────
  // 為什麼：nginx 計時 log 全站最慢前三筆全是 /api/admin/stats（14.7~15.9 秒、全在 node 內）。
  //   真因：users 統計用 adminAuth.listUsers(1000) 逐頁抓全部使用者（上限 50 頁），
  //   每頁是一趟「循序」HTTPS 往返 ⇒ ~50 趟 × ~300ms ≈ 15 秒。
  //   ⚠ 這 15 秒幾乎全是 await 的網路 I/O，事件迴圈沒被佔住（同步聚合段 50k 筆實測毫秒級）
  //   ⇒ 不會卡到玩家；但 admin 每開一次總覽分頁就白等 15 秒＋對 Firebase Auth 打 ~50 發。
  // 修法：比照 v0.95 /firebase/users-all 的先例 —— 結果快取 + single-flight，
  //   外加「過期先回舊值、背景刷新」：除了進程重啟後的第一發，admin 永遠不必等掃描。
  //   數字的**算法一字未動**（口徑不變），只是「什麼時候算」改變；
  //   回應多帶 at=計算時刻，admin 總覽會標示資料時間。
  const USERS_STATS_TTL_MS = 5 * 60 * 1000;   // 與 v0.95 users-all 的 5 分鐘一致
  const _usersStatsCache = { at: 0, data: null, inFlight: null };

  async function computeUsersStats() {
    // ↓ 掃描與聚合與 v0.4 以來的原始碼一字不差（loop 抓全部 — 上限 sane safety: 50,000）
    ...
    return {
      ...
      at: Date.now(),   // 資料時間（給 admin 畫面標示；純新增欄位，不影響既有數字）
    };
  }

  async function getUsersStatsCached() {
    const now = Date.now();
    if (_usersStatsCache.data && now - _usersStatsCache.at < USERS_STATS_TTL_MS) {
      return _usersStatsCache.data;                     // 新鮮：直接回
    }
    if (!_usersStatsCache.inFlight) {                   // single-flight：同時多發只掃一輪
      _usersStatsCache.inFlight = computeUsersStats()
        .then((data) => { _usersStatsCache.data = data; _usersStatsCache.at = data.at; return data; })
        .finally(() => { _usersStatsCache.inFlight = null; });
    }
    if (_usersStatsCache.data) {
      // 過期但有舊值：先回舊值，讓刷新在背景跑完（失敗只記 log，下一發再試）
      _usersStatsCache.inFlight.catch((e) => console.warn('[admin] users 統計背景刷新失敗:', e && e.message));
      return _usersStatsCache.data;
    }
    return _usersStatsCache.inFlight;                   // 冷啟動：等第一次掃描
  }
  ...
  const FEEDBACKS_TTL_MS = 5 * 60 * 1000;   // 與 USERS_STATS_TTL_MS 一致
    ...
    app.get('/api/deck-stats', async (req, res) => {
      ...
      try {
        ...
        const data = {
          ...
          // ⚠ 錦標賽勝率本版**還沒有資料來源**：要收得到得先讓 TREGS 的報名紀錄與
          //   tournamentArchives 的 players[] 也帶 deckId（那會動到錦標賽寫入路徑，
          //   站長最在意的區塊 ⇒ 另開一版做）。這裡回一個明確的旗標，讓 client
          //   顯示「累積中」，而不是顯示 0 勝 0 敗騙玩家。
    ...
    try {
      ...
      const _dpMw = async (req, res, next) => {
        try {
          ...
          if (_method === 'GET') {
            // 哨兵:只在啟用時、且回應是 { room } 形狀時加 deltaPut:1(404/204/list 都不動)

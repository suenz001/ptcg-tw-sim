...
(async () => {
  try {
    ...
    function currentActorSeat(gs) {
      ...
      if (gs.phase === 'setup') {
        // v0.60：先判 mulligan 子階段待辦(補抽 pendingMulliganDraw / 確認揭示 mulliganRevealConfirmed /
        //   補抽後加備戰 mulliganPostBenchOpen)。卡在 mulligan 的一方才是「該動作卻沒動」的人,對手只是在等。
        //   原本只看 setupDone → 雙方 setupDone 都 false 時回 -1 雙敗,把「等對方補抽」的人也一起誤判雙敗。
        ...
        const owesMulligan = (i) => (Number(pmd[i]) > 0) || !mrc[i] || !!mpb[i];
        if (owesMulligan(0) || owesMulligan(1)) {
          const b0 = owesMulligan(0), b1 = owesMulligan(1);
          if (b0 && !b1) return 0;
          if (b1 && !b0) return 1;
          return -1; // 兩邊都還欠 mulligan → 都該動
        }
        const d0 = !!(gs.setupDone && gs.setupDone[0]), d1 = !!(gs.setupDone && gs.setupDone[1]);
        if (d0 && !d1) return 1;
        if (!d0 && d1) return 0;
        return -1; // mulligan 都完成、但雙方都還沒選出場 → 都欠動作
      }

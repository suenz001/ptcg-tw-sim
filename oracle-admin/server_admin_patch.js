  ...
  try {
    ...
    app.post('/api/tournament/propose', async (req, res) => {
      try {
        ...
        await postSystemChat('📣 玩家發起社群賽「' + ev.name + '」（' + (fmt === 'swiss-then-cut' ? '瑞士制+TopCut' : '單淘汰') + '）！募集 ' + rallyMin + ' 分鐘、期間都可報名，時間到達 ' + minPlayers + ' 人以上就自動開賽（人越多越熱鬧），快來「響應」報名～');

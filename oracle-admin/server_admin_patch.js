...
import('firebase-admin').then(async ({ default: admin }) => {
  ...
  (function registerStatsEndpoints() {
    ...
    // cardId → 卡名對照。從錦標賽卡池 JSON lazy 載入一次（只留 name，省記憶體）。
    ...
    /** 多規則命中時取 priority 最小者為主原型；回 {rule, all[]}。零命中回 {rule:null}。 */
    function classifyDeck(sets, rules) {
      ...
      all.sort((a, b) => (a.priority || 0) - (b.priority || 0) || String(a._id).localeCompare(String(b._id)));
      return { rule: all[0], all };
    }
    // 供批次3 統計端點重用（同一份比對邏輯，避免兩處漂移）
    app.locals = app.locals || {};
    app.locals._deckRuleHelpers = { getCardNameMap, deckToSets, deckMatchesRule, classifyDeck, casualSideResult, tournSideResult };
    ...
    app.get('/api/admin/deck-archetype-stats', requireFirebaseAdmin, async (req, res) => {
      ...
      try {
        ...
        if (source === 'casual' || source === 'all') {
          const q = {};
          if (excludeAI) q.roomCode = { $type: 'string' };
          if (since) q.endedAt = { $gte: since };
          q.winReason = { $not: /中途離開|離開房間|斷線|斷開|退出|不在場|disconnect|技不如人|先行離開/i };
        ...
        const pack = (bucket) => {
          ...
          const topCards = [...u.cardFreq.entries()].sort((a, b) => b[1] - a[1]).slice(0, 15)
            .map(([name, n]) => ({ name, decks: n }));
          return { rows: out, unclassified: { usage: u.usage, wins: u.wins, losses: u.losses, draws: u.draws,
                   winRate: udec > 0 ? u.wins / udec : null, topCards } };
        };

        const data = { source, since, excludeAI, ruleCount: rules.length, scanned,

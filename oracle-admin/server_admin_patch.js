      ...
      try {
        ...
        res.json({
          ...
          rows: rows.map(function (r) {
            return { ts: r.ts, email: r.email || null, room: r.room || '', reason: r.reason || '', diag: r.diag || '' };
          }),
        });
      } catch (e) { res.status(500).json({ error: e.message }); }
    });

...
import('firebase-admin').then(async ({ default: admin }) => {
  ...
  (function registerMatchRecords() {
    ...
    //   ⚠ 暱稱來源＝match-result payload 的 p1/p2.name（同索引對應 seats[0]/[1]），
    //     刻意**不**動 enrich 段的 projection（test-v6266 C2 逐字鎖住那個 projection）。
    function recordPlayerIdentity(db, seats, names) {
      try {
        ...
        for (let i = 0; i < 2 && i < seats.length; i++) {
          ...
          const nick = (Array.isArray(names) && typeof names[i] === 'string' && names[i].trim()) ? names[i].trim().slice(0, 40) : null;
  ...
  try {
    ...
    function _frPublic(doc, me, ident) {
      ...
      return {
        ...
        status: doc.status,
        nick: _frNick(idn && idn.nick) || _frNick(otherNick) || '玩家',
    ...
    async function _frResolveTarget(body, me, res) {
      ...
      if (typeof b.email === 'string' && b.email.trim()) {
        ...
        const idn = await db.collection(FR_ID_COLL).findOne({ _id: te }, { projection: { nick: 1 } });
        if (idn) return { email: te, nick: _frNick(idn.nick), myNick: null, via: 'email' };
        ...
        return { email: te, nick: _frNick(au.name), myNick: null, via: 'email' };
      }
    ...
    app.get('/api/friends/list', async (req, res) => {
      try {
        ...
        const ident = new Map();
        if (others.length) {
          const ic = db.collection(FR_ID_COLL).find({ _id: { $in: others } }, { projection: { _id: 1, uid: 1, uids: 1, nick: 1 } }).limit(FR_LIST_CAP);
          let k = 0;
      ...
      } catch (e) { res.status(500).json({ error: e.message, code: 'friends-error' }); }
    });
    ...
    app.post('/api/friends/request', async (req, res) => {
      try {
        ...
        if (cur && cur.status === 'rejected' && typeof cur.rejectedAt === 'number' && now - cur.rejectedAt < FR_REJECT_COOLDOWN_MS) {
    ...
    // POST /api/friends/unblock {fid}：只有封鎖方可解除；解除＝真刪除那一列（關係歸零，要重新邀請）
    app.post('/api/friends/unblock', async (req, res) => {
      try {
        ...
        await c.deleteOne({ _id: cur._id, status: 'blocked', blockedBy: me.email });
        res.json({ ok: true, friendsApi: 1, removed: true });
      } catch (e) { res.status(500).json({ error: e.message, code: 'friends-error' }); }
    });
    ...
    console.log('[friends] endpoints registered (v1.36) enabled-by-default=false');

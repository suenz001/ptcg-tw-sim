  ...
  try {
    ...
    app.post('/api/tournament/propose', async (req, res) => {
      try {
        ...
        }, { excludeUid: id.uid, prefKey: 'notifyCommunity' });
     ...
     *
     * @param payload      推播內容
     * @param excludeUid   要排除的人（通常是發起者自己 —— 他不需要被自己開的賽事通知）
     * @param prefKey      訂閱文件上的偏好欄位名；用 `$ne: false` 判定
     *                     → **未設定過的舊訂閱視為開啟**（Wilson 裁定新通知預設開，
     *                       否則既有玩家全部收不到、功能等於沒上線）
     * @param cap          單次推播上限，防未來訂閱數暴增時一次送爆
     */
    async function broadcastPush(payload, { excludeUid = null, prefKey = null, cap = 500 } = {}) {
      if (!_webpush) return { sent: 0 };
      try {
        const q = {};
        if (excludeUid) q.uid = { $ne: excludeUid };
